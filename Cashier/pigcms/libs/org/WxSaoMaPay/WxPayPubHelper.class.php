	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null) 
			{
		   	...
		   	$this->parameters["mch_id"] = $this->mchid;//商户号
			if(WxPay_SUBMCHID){
				$this->parameters["sub_mch_id"] =WxPay_SUBMCHID;
			}
		//	if(defined('WxPay_SUBMCHID')){$this->parameters["sub_mch_id"] =WxPay_SUBMCHID; } //子商户号  会引起签名错误，但是刷卡支付那边这样写却没问题，回头看下。两种支付方式签名代码不一样，一个传参数，一个没有
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null && $this->parameters["transaction_id"] == null) {
		   	...
		   	$this->parameters["appid"] = $this->appid;//公众账号ID
		   	$this->parameters["mch_id"] = $this->mchid;//商户号
			//$this->parameters["sub_mch_id"] = '1392691402';
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
		}catch (SDKRuntimeException $e)
