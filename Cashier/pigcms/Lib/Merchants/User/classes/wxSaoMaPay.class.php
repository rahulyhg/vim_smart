...
class wxSaoMaPay {
    ...
    public function search($data){
      //  bpBase::loadOrg('WxPay/WxPay.MicroPay');
        ...
        $input->SetOut_trade_no($data['order_id']);
        $microPay = new MicroPay();
        $result=$microPay->searchOrder($input);
        return $result;
    }
            ...
            if (!empty($response)) {
                ...
                if ($response['return_code'] == 'SUCCESS') {
                    ...
                    if ($response['result_code'] != 'SUCCESS') {
						...
						M('meal_order')->update(array('refund'=>1), array('orderid' => $ordertmp['order_id']));	//对meal_order作退款处理
     ...
     * 退款处理(随机免单)
     */
    public function wxRefundTwo($ordid, $wx_user, $mid,$type='') {
        $orderDb = M('cashier_order');
        if($type=='micropay'){
            $ordertmp = $orderDb->get_one(array('order_id' => $ordid, 'mid' => $mid, 'pay_way' => 'weixin'), '*');
        }else{
