        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-10">
                ...
                <ol class="breadcrumb">
                    ...
                    <li>
                        <span><?php echo $this_group['group_name'];?></span>
                    </li>
                    <li class="active">
                        <strong>成员管理</strong>
                    </li>
...
<!--            --><?php //if($mid){echo "<label>当前商户：</label><label>$merchant_name</label><br/> ";}?>
<!--            <label>当前分组：--><?php //echo $this_group['group_name'];?><!--</label><br/>-->
<!--            --><?php //if($mid){echo '<button onclick="user_list()">员工列表</button>';}?>
<!--            --><?php //if($mid){echo '<button onclick="record()">充值记录</button>';}?>
<!--            --><?php //if($mid){echo "<button onclick='group()'>分组管理</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";}?>
            <?php if($mid){echo "<button class='fe' onclick='add_user()'>添加组员</button>";}else{
                echo "<button class='fe' onclick='add_user2()' >添加组员</button>";
            }?>

            <button onClick="all_del()" class="fe4">批量删除</button><div style="clear:both"></div>
            <form action="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=company&a=move_user" method="post" style="margin-top:20px;">
                ...
                <table width="50%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px; margin-bottom:20px;" class="table_list">
                    ...
                    <tr>
                        ...
                        <?php if($mid) echo "<td class='td_left' width='5%' style='font-size:14px; font-weight:bold; color:#000000;'>账户余额</td>"?>
                    ...
                    <?php if(!empty($user)){
                        foreach($user as $value){
                            ...
                            <tr>
                               ...
                               <?php if($mid){?><td class='td_left' width='5%'><?php if($value['money']){echo $value['money'];}else {echo '0.00';}?></td><?php }?>
                                ...
                                <td class="td_left" width="5%"><a href="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=company&a=user_del&company_id=<?php echo $company_id;?>&group_id=<?php echo $group_id;?>&uid=<?php echo $value['uid']?>&mid=<?php echo $mid;?>"><div class="ff2">删除</div></a></td>
                            </tr>
                ...
                <div class="Tpage"><?php echo $pagebar;?></div>
                </div>
...
//    }
    function add_user(){
        window.location.href ="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=company&a=add_user&company_id=<?php echo $company_id;?>&group_id=<?php echo $group_id;?>&mid=<?php echo $mid;?>";
    }
    function add_user2(){
        window.location.href ="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=company&a=add_user&company_id=<?php echo $company_id;?>&group_id=<?php echo $group_id;?>";
    }
//    function user_list(){
...
//    }
    function all_del(){
        ...
        if(uid_arr=="" || uid_arr=="null"){
            ...
            $.ajax({
                'url':"?m=User&c=company&a=all_del",
