...
<!--            <button onClick="record()" class="fe2">充值记录</button><div style="clear:both"></div>-->
            <form action="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=recharge&a=company_recharge_submit" method="post" style="margin-top:20px;">
...
//    }
    $('.fe3').click(function () {
        ...
        if(result==true){
            //alert(2);
            $.ajax({
                'url':"?m=User&c=recharge&a=company_recharge_submit",
                ...
                'success':function(ret){
                    if(ret.error==0){
                        alert(ret.msg);
                        window.location.href="<?php echo $this->SiteUrl;?>/merchants.php?m=User&c=recharge&a=up";
                    }else{
                        alert(ret.msg);
                    }
                },
