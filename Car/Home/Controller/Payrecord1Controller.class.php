...
//                $api_order_info=$jieshun->use_api_yueka_info($car_no);
//                $endTime=json_decode($api_order_info,true);
//                //dump ($endTime);exit;
//            }else{
//                //登录失败
...
//            }
        ...
        if($payrecored->check_and_show($pay_info['car_no'])){
            $this->assign('show_msg','请在缴费后15分钟离场，否则会产生额外费用');
        }else{
            $this->assign('show_msg','进场30分钟内不产生费用');
        }
        //调用模板
        $this->display();
    ...
    public function pay_now(){
        ...
        //设定微信官方订单详情参数
        $order_name=I('post.fee_name');  //订单名称
        $order_name='广发银行大厦停车费';
        ...
        $datas=array(
            ...
            'api_pay_no'=>$api_order_info['orderNo'],   //第三方接口订单
        );
            ...
            if($total_fee<=0){
                ...
                if($payrecored->where(array('pay_no'=>$order_no))->setField(array('pay_status'=>'1','cp_id'=>$cp_id))){
                    ...
                    if(D('coupon')->where(array('cp_id'=>$cp_id))->setField(array('is_valid'=>'2'))) {
                        ...
                        //通知第三方开门接口
                        $notice_jieshun_result = $jieshun_api->notice_api_pay_ok($api_order_info['orderNo']);
                        //记录日志文件，后期可查异常状态追责
    ...
    public function WeiXin_call_back(){
        ...
        //②：查看通知开门状态
        $serv_opendoor_info=D('servicerecord')->where(array('serv_id'=>$pay_result_info['serv_id']))->getField('open_door');
        if($pay_result_info['pay_status']=='1' && $serv_opendoor_info=='1'){
            //支付成功，并且第三方开门成功！
            echo json_encode(2);
        }elseif($pay_result_info['pay_status']=='1' && $serv_opendoor_info=='0'){
        ...
        //        //非法ip(仅限微信服务器ip访问)，禁止访问此方法，将页面引导到404页面
        //        $fp= fopen('Common/Log/WeiXin_notice/weixin_pay_ok_notice2.log', 'a+');
        //        fwrite($fp, '访问WeiXin_call_back方法时出错');
        //        exit;
        //    }
        ...
        if($xml_arr['result_code']=='SUCCESS' && $xml_arr['return_code']=='SUCCESS'){
    ...
    public function problem_test(){
        //$payrecored=new \Home\Model\PayrecordModel();
        //dump($payrecored->check_and_show('鄂-A23995'));
        $_SESSION['user_id']='325';
