...
class Weixinpay{
    ...
    public function __construct($order_info,$pay_type,$user_info,$is_mobile=0){
    ...
    public function authorize_openid($redirect='')
    {
        ...
        if (empty($_GET['code']) || empty($_SESSION['weixin']['state'])) {
            ...
            if (isset($_GET['code']) && isset($_GET['state']) && ($_GET['state'] == $GLOBALS['_SESSION']['weixin']['state'])) {
                ...
                $return = https_request('https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . $this->config['wechat_appid'] . '&secret=' . $this->config['wechat_appsecret'] . '&code=' . $_GET['code'] . '&grant_type=authorization_code');
    ...
    public function mobile_pay($notice_url,$pay_money){
        ...
        $unifiedOrder->setParameter("notify_url",C('config.site_url').'/source/'.$notice_url.'php');//通知地址
