...
class Weixinpay{
    ...
    public function mobile_pay($notice_url,$pay_money){
        ...
        //自定义订单号，此处仅作举例
        $unifiedOrder->setParameter("out_trade_no",$this->order_info['order_id']);//商户订单号
        /*if($_SESSION['user_id']==325){
            $pay_money=0.01;//阳志鹏测试
        }*/
    ...
    //测试
    public function mobile_pay_two($notice_url,$pay_money){
        ...
        $unifiedOrder = new UnifiedOrder_pub_two($this->config['pay_weixin_appid'],$this->config['pay_weixin_mchid'],$this->config['pay_weixin_key'],$this->config['pay_weixin_appsecret']);
    ...
    public function getToken(){
        ...
        if($token = $this->getCache($authname)){
            ...
            if (isset($Tokenarr['access_token'])) {
                ...
                //缓存
                $expire = $Tokenarr['expires_in'] ? intval($Tokenarr['expires_in'])-3600 : 3600;
