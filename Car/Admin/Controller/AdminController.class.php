...
class AdminController extends RbacController {
    ...
    public function showlist(){
        ...
        if($_GET['role_id']){
             $admin_infos=$admin->where(array('is_check'=>'1','role_id'=>$_GET['role_id']))->limit(500)->select();
        foreach ($admin_infos as $key => &$value) {
            $value['role_name']=M('role')->where(array('role_id'=>$value['role_id']))->getField('role_name');
        }
        unset($value);
        //dump($admin_infos);exit;
        //将数据返回模板
        $this->assign('admin_infos',$admin_infos);

        }else{
             $admin_infos=$admin->where(array('is_check'=>'1'))->limit(500)->select();
        foreach ($admin_infos as $key => &$value) {
            $value['role_name']=M('role')->where(array('role_id'=>$value['role_id']))->getField('role_name');
        }
        unset($value);
    ...
    public function add(){
        ...
        if(IS_POST){
            //判断是否已输入必须信息
            if( !$_POST['ad_name'] || !$_POST['ad_pwd'] || !$_POST['ad_tname'] ){
                $this->error('没有填入对应的必填信息',U('add'),1);
            }
            //检查两次密码是否一致
            if( $_POST['ad_pwd']!= $_POST['ad_pwd2']){
                $this->error('两次密码输入不一致，请检查！',U('add'),1);
            }

            //数据收集
            $data=$admin->create();
            //同步更新角色id到user表中
            M('user')->where(array('user_id'=>$data['ad_uid']))->data(array('user_role_id'=>$data['role_id']))->save();
            //数据制作并插入到数据库方法
            $z=$admin->get_and_add($data, $_FILES);

            //将数据插入到数据库
            //$z=$admin->add($data);
            if($z){
                $this->success('后台管理员添加成功！',U('showlist'),1);
            }else{
                $this->error('后台管理员添加失败，请检查！',U('add'),1);
            }

        }else{

            //查询车库编号，将停车对应的编号和名称返回模板
            $garage_info=D('garage')->field('garage_name,garage_no')->select();
            $this->assign('garage_info',$garage_info);

            //查询所有角色
            $role_infos=D('role')->select();
            //将角色数据返回模板
    ...
    * */
     ...
     * 自动填充
     * */
    public function auto_write_uid(){
        $user_wxnik = I('post.user_wxnik');
        $user_id = M('user')->where(array('user_wxnik'=>$user_wxnik))->getField('user_id');
        echo $user_id;
    }
    ...
    public function update_self(){
        ...
        //只能修改自己的信息
        if(session(admin_name)!=$admin_info['ad_name']){
            $this->error('抱歉，你只能修改自己的信息！',U('Index/index'),2);
            exit;
        }
        ...
        if(IS_POST){
            ...
            $data['ad_id']=$ad_id;
            $data['old_sfzimg1']=$_POST['old_sfzimg1'];//记录原有的第一张照片
            $data['old_sfzimg2']=$_POST['old_sfzimg2'];//记录原有的第二张照片
            ...
            if($z){
                session(null);
                $this->success('个人信息更新成功！请重新登录！',U('admlogin'),1);
            }else{
                $this->error('个人信息更新失败，请检查！',U('update_self',array('ad_id'=>$ad_id)),1);
            }
    ...
    public function update(){
        ...
        //超级管理员既能修改自己的信息，也能修改除自己之外所有的人信息
        //从admin配置文件中读取超级管理员的列表
        $supper_admin_list=C('SUPPER_ADMIN_LIST');
        if(!in_array(session(admin_name),$supper_admin_list) && in_array($admin_info['ad_name'],$supper_admin_list)){
            $this->error('抱歉，你无权限执行当前操作！',U('Index/index'),1);
            exit;
        }

        //除了创始人，禁止任何人操作创始人信息
        if(session(admin_name)!=C('FOUNDER') && $admin_info['ad_name']==C('FOUNDER')){
            $this->error('抱歉，你无权限执行当前操作！',U('Index/index'),1);
            exit;
        }
        ...
        if(IS_POST){
           ...
           // dump($_FILES);exit;
            ...
            if($z){
                $this->success('管理员信息更新成功！',U('showlist'),1);
            }else{
    ...
    public function delete(){
        ...
        $ad_id=I('get.ad_id');
        //将对应的记录进行审核取消操作
        $z=D('admin')->where(array('ad_id'=>$ad_id))->save(array('is_check'=>'0'));
        if($z){
            echo json_encode('1');//取消审核操作成功！
        }else{
            echo json_encode('2');//取消审核操作失败！
        }
    }
    ...
    public function destroy(){
        ...
        //将对应的记录进行逻辑删除
        $z=$admin->where(array('ad_id'=>$ad_id))->delete();
        if($z){
            //同时要删除它对应的照片
            $admin->destory_all_img($admin_info);
    ...
    public function recycle(){
        //查询所有已经被取消审核资格的管理员信息
        //为了减轻服务器的压力，建议使用条件进行查询，涉及默认全部查询时我们只显示最近500条（后期可设置到配置项）
        $admin_infos=D('admin')->where(array('is_check'=>'0'))->limit(500)->select();
    ...
    public function recover(){
        ...
        $ad_id=I('get.ad_id');
        //将对应的记录进行恢复
        $z=D('admin')->where(array('ad_id'=>$ad_id))->save(array('is_check'=>'1'));
        if($z){
            echo json_encode('1');//审核操作成功！
        }else{
            echo json_encode('2');//审核操作失败！
        }
    }

    //管理人员查看自己的信息详情页(除了超级管理员，只能看自己的详情页)
    public function self_detail(){

        //接收对应的ad_id
        $ad_id=I('get.ad_id');
        //查询对应的管理员详情信息
        $ad_info=D('admin')->find($ad_id);

        if(session('admin_name')!=$ad_info['ad_name'] && !in_array(session('admin_name'),C('SUPPER_ADMIN_LIST'))){
            $this->error('你只能查看自己的详情页',U('Index/index'),2);
            exit;
        }
