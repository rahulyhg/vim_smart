...
class WxPay{
	...
	public function wxpay($openid,$total_fee,$body,$order_sn){
		...
		$unifiedorder = array(
			...
			'body'			=> $body,
			'sub_mch_id' 	=> '1489279412',
			'out_trade_no'	=> $order_sn,
			'total_fee'		=> $total_fee * 100,
			'spbill_create_ip'	=> self::getip(),
			'notify_url'	=> 'http://www.hdhsmart.com/vendor/ceshi/notify.php',
			'trade_type'	=> 'JSAPI',
			'openid'		=> $openid
		);
		...
		//return $unifiedorder;
		$data = array(
			'info' => $unifiedorder['sign'],
			'status' => 2,
			);
		$re = M('ceshi')->data($data)->add();
...
//---------------------------------------------------------------用到的函数------------------------------------------------------
	...
	*/
	...
	public function paytest(){
		//订单信息
		// $order = M('meal_order')->where(array('order_id'=>'127749'))->find();
