...
<block name="table-toolbar-left">
    <div class="btn-group">
        ...
        <a href="#form_modal2" data-toggle="modal">
            <button  class="btn sbold green" >设置抄表日期
            </button>
        </a>
        <a href="javascript:">
            <button  class="btn sbold green" onclick="sub()">打印催款单
            </button>
        </a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">设置抄表时间</h4>
                </div>
                <div class="modal-body">
                    <form action="{pigcms{:U('Room/setTime')}" method="post" id="up_time" class="form-horizontal">
                        <div class="form-group">
                            <label class="control-label col-md-4">上面抄表日期</label>
                            <div class="col-md-8">
                                <input class="form-control input-medium date-picker" name="start_time" size="16" type="date" value="{pigcms{$start_time}" /> </div>
                        </div>
                        <div class="form-group">
                            <label class="control-label col-md-4">本月抄表日期</label>
                            <div class="col-md-8">
                                <input class="form-control input-medium date-picker" name="end_time" size="16" type="date" value="{pigcms{$end_time}" /> </div>
                        </div>
                    </form>
                </div>
...
<block name="body">
    <table class="table table-striped table-bordered table-hover" id="sample_1">
        ...
        <volist name="list" id="vo">
            <if condition="!empty($vo['oid'])">
                <tr style="background-color: #ffffff">
                    <if condition="$vo['is_all_record'] eq '0' and $vo['is_enter_list'] eq '0'">
                        <td>
                            <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                        ...
                        <td>
                            <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                <input type="checkbox" class="checkboxes" value="{pigcms{$vo.pigcms_id}" />
                                <span></span>
                            </label>
                        </td>
                    </if>
                    <td>
                        ...
                        <!--排序需要-->
                        {pigcms{$vo.room_names_format}
                    </td>
                    <td>{pigcms{$vo.tenantname}</td>
                    <td>{pigcms{:str_replace(',',"<br >",$vo['ownernames'])}</td>
                    <td class="row" style="width:20em">
                        <empty name="vo['stat']['format']">
                            <if condition="empty($vo['oid'])">
                                <a data-toggle="modal" data-target="#common_modal"
                                   href="{pigcms{:U('modal_tenant_bind_room')}&tid={pigcms{$vo.pigcms_id}">
                                    <span class="text-danger">未绑定业主（已迁出）</span>
                                </a>
                                <else />
                                <a data-toggle="modal" data-target="#common_modal"
                                   href="{pigcms{:U('modal_tenant_bind_meter')}&tid={pigcms{$vo.pigcms_id}">
                                    <span class="text-danger">未绑定设备</span>
                                </a>
                            </if>

                        </empty>
                        <foreach name="vo['stat']['format']" item="v" key="k">
                            <if condition="!empty($k)">
                                <div>
                                    <a data-toggle="modal" data-target="#common_modal"  href="{pigcms{:U('modal_meter_type_record')}&meter_type_id={pigcms{$k}&tid={pigcms{$vo.pigcms_id}&ym={pigcms{$ym}">
                                        {pigcms{$meter_type_list[$k]} : ({pigcms{$v})
                                    </a>
                                </div>
                            </if>
                        </foreach>
                    ...
                    <td>
                        <if condition="$vo['is_all_record'] eq '1' and $vo['is_enter_paylist'] eq '0' or $vo['is_all_record'] eq '1' and $vo['is_enter_list'] eq '0'">
                            <a data-toggle="modal" data-target="#common_modal"
                               href="{pigcms{:U('modal_pay_list',array('tid'=>$vo['pigcms_id'],'ym'=>$_GET['ym']))}">
                                <button type="button" class="btn green btn-sm">
                                    未确认出账
                                </button>
                            </a>
                            <elseif condition="$vo['is_all_record'] eq '1' and $vo['is_enter_list'] eq '1'"/>
                            <a data-toggle="modal" data-target="#common_modal"
                               href="{pigcms{:U('look_list',array('tid'=>$vo['pigcms_id'],'ym'=>$_GET['ym']))}">
                                <button type="button" class="btn blue btn-sm">
                                    已出账
                                </button>
                            </a>
                            <elseif condition="$vo['is_all_record'] eq '0' and $vo['is_enter_list'] eq '0'"/>
                            <a data-toggle="modal" data-target="#common_modal"  href="{pigcms{:U('modal_meter_type_record')}&meter_type_id={pigcms{$k}&tid={pigcms{$vo.pigcms_id}">
                                <button type="button" class="btn blue btn-sm">
                                    统计未完成
                                </button>
                            </a>
                    ...
                    <td class="button-column">
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu pull-left" role="menu" style="position: absolute;">
                                <!--                            <li>-->
                                <!--                                <a data-toggle="modal" data-target="#common_modal"  href="{pigcms{:U('modal_tenant_bind_room',array('tid'=>$vo['pigcms_id']))}">-->
                                <!--                                    <i class="icon-docs"></i> 绑定房间-->
                                <!--                                </a>-->
                                <!--                            </li>-->
                                <if condition="$vo['is_all_record'] eq '1' and $vo['is_enter_paylist'] eq '0'">
                                    <li>
                                        <a data-toggle="modal" data-target="#common_modal"  href="{pigcms{:U('modal_pay_list',array('tid'=>$vo['pigcms_id'],'ym'=>$_GET['ym']))}">
                                            <i class="icon-docs"></i> 出账
                                        </a>
                                    </li>
                                    <elseif condition="$vo['is_all_record'] eq '1' and $vo['is_enter_list'] eq '1'"/>
                                    <li>
                                        <a data-toggle="modal" data-target="#common_modal"  href="{pigcms{:U('look_list',array('tid'=>$vo['pigcms_id'],'ym'=>$_GET['ym']))}">
                                            <i class="icon-docs"></i> 查看详情
                                        </a>
                                    </li>
                                    <elseif condition="$vo['is_all_record'] eq '1' and $vo['is_enter_list'] eq '0'"/>
                                    <li>
                                        <a data-toggle="modal" data-target="#common_modal"  href="{pigcms{:U('modal_pay_list',array('tid'=>$vo['pigcms_id'],'ym'=>$_GET['ym']))}">
                                            <i class="icon-docs"></i> 出账</a>
                                    </li>
...
<!--    </script>-->
    ...
    <script>
        $("input[name='choose_time']").change(function(){
            ...
            window.location.href='/admin.php?g=System&c=Room&a=bill_preview&ym='+ym;
        });
        ...
        function sub() {
            ...
            if (ids.length == 0) {
                ...
                //if(confirm( '你确定打印选中码？')) {
                    var url = "{pigcms{:U('PropertyService/hydropower_account_do')}";
                    //用post方式传递
