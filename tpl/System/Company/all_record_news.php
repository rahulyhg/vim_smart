                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <foreach name="record" item="vo">
                                <tr class="odd gradeX">
                                    ...
                                    <td class="center">
                                        {pigcms{$vo.merchant_name}
                                    </td>
                                    <td class="center">
                                        {pigcms{$vo.money}
                                    </td>
                                    <td class="center">
                                        {pigcms{$vo.recharge_name}
                                    </td>
                                    <td class="center">
                                    ...
                                    <td class="center" >
                                        <button type="button" class="btn blue active" style="width:100px; margin:0px auto; text-align:center;" id="{pigcms{$vo.id}" onclick="check_order(this);">点击查询</button>
                                    </td>
                                </tr>
                            </foreach>
                            </tbody>
    ...
    function check_order(msg){
        ...
        $.ajax({
            ...
            success:function(res){
                if(res == 1){
                    swal({   title: "支付成功",   text: "支付已经完成！",   type: "success",  confirmButtonColor: "#DD6B55",   confirmButtonText: "好的",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('all_record_news')}" });
                }else if(res == 2){
                    swal({   title: "支付异常",   text: "系统出错，亲联系管理员",   type: "info",  confirmButtonColor: "#DD6B55",   confirmButtonText: "好的",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('all_record_news')}" });
                }else if(res == 3){
                    swal({   title: "账单还未到账",   text: "请稍后重试！",   type: "info",  confirmButtonColor: "#DD6B55",   confirmButtonText: "好的",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('all_record_news')}" });
                }else if(res  == 4){
                    swal({   title: "支付失败",   text: "请稍后重试！",   type: "info",  confirmButtonColor: "#DD6B55",   confirmButtonText: "好的",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('all_record_news')}" });
                }
            }
        });
    }
