<div class="modal-header">
    <button type="button" class="close" aria-label="Close" data-dismiss="modal"><span aria-hidden="true">&times;</span></button>
    <h4 class="modal-title" id="myModalLabel">线下支付</h4>
</div>
<form action="__SELF__" method="post" id="myForm">
<div class="modal-body" style="height: 30rem;overflow-y: scroll">

    <div class="panel panel-default">
        <div class="panel-heading">
            <span style="vertical-align: bottom;line-height: 33px;">请确定支付金额
                （ <small class="text-danger inline">线下缴费需全额缴完后确认</small>
                ）
            </span>
        </div>
        <div class="panel-body">
            <table class="table table-bordered table-hover">
                <tr>
                    <th colspan="5" style="color: blue">尊敬的客户：{pigcms{$userInfo.name}</th>
                </tr>
                <tr>
                    <th>收费项目</th>
                    <th>应收费用<small class="text-muted">（元）</small></th>
                    <th>已缴费用<small class="text-muted">（元）</small></th>
                    <th>未缴费用<small class="text-muted">（元）</small></th>
                    <th>本次实缴费用<small class="text-muted">（元）</small></th>
                    <!--                    <th>上月上报状态1</th>-->
                </tr>
                <foreach name="userInfo['nowPay']" item="v" key="k">
                    <tr>
                        <td>{pigcms{$k}</td>
                        <td>{pigcms{$v.0}元</td>
                        <td>{pigcms{$v.1}元</td>
                        <td>{pigcms{$v.2}元</td>
                        <if condition="$v.2 neq 0">
                            <td><input type="text" name="<php>echo $fieldArray[$k];</php>" value="{pigcms{$v.2}" placeholder="填入金额"/></td>
                        <else/>
                            <td>0.00</td>
                        </if>

                    </tr>
                </foreach>


            </table>
        </div>
    </div>

</div>
<div class="modal-footer">
    <button type="button" class="btn btn-primary" id="save">确认并缴费</button>
    <button type="button" class="btn btn-default" data-dismiss="modal" id="closeWid">关闭</button>
</div>
</form>
<script src="./static/js/vue.min.js"></script>
<script src="./static/js/vue-route.js"></script>
<script>
    $("#save").click(function(){
        $("#myForm").submit();
    });


</script>
