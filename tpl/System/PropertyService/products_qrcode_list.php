...
<div class="page-content-wrapper">
    ...
    <div class="page-content">
        ...
        <ul class="page-breadcrumb breadcrumb">
            <li>
                <a href="{pigcms{:U('Off/off_list_news')}">在线巡检</a>
...
<!--                        </div>-->
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <tr>
                                ...
                                <th  style="width: 250px; text-align: center;line-height: 40px;">项目名称</th>
                                <th  style="text-align: center;line-height: 40px;">楼层号</th>
                                <th  style="text-align: center;line-height: 40px;">巡检点编号</th>
                            ...
                            <foreach name="qrArr" item="vo" key="k">
                                <tr>
                                    ...
                                    <td style="line-height: 40px;">{pigcms{$vo.room_name}</td>
                                    <td style="line-height: 40px;">{pigcms{$vo.orientation}</td>
                                    <td style="line-height: 40px;">
                                    ...
                                    <td style="line-height: 40px;">
                                        <img src="{pigcms{$vo.adress}" style="width: 40px;height: 40px;" onmouseover="big({pigcms{$vo.id})" onmouseout="small()" id="henfan_{pigcms{$vo.id}" />
                                    </td>
                                    <td style="line-height: 40px;">
                                        <a href="{pigcms{:U('PropertyService/history_qrcode',array('id'=>$vo['id'],'cid'=>$cid))}">历史记录</a>
                                    </td>
                                    <td style="line-height: 40px;">
...
<script type="text/javascript">
    ...
    function delete_pr_info(obj) {
        layer.msg('你确定要删除么？本操作不可恢复！', {
            ...
            ,yes: function(index){
                ...
                //通过ajax异步删除
                $.ajax({
                    url:"{pigcms{:U('PropertyService/products_delete')}",
                    data:{'pro_id':pro_id},
                    type:'post',
    ...
    function txtblur(el,id) {
        ...
        var borrower = $(el).val();
        var url = "{pigcms{:U('PropertyService/products_operate_save')}";
        $.ajax({
            url:url,
            type:'post',
    ...
    function big(id) {
        $.ajax({
            url:"{pigcms{:U('PropertyService/get_url')}",
            type:'post',
            data:'id='+id,
            success:function(re){
                $("#bigdada").attr("src", re);
    ...
    function pixia() {
        var ids = '';
        var url = "{pigcms{:U('PropertyService/products_zip_z')}";
    ...
    function qr_print() {
        var ids = '';
        var url = "{pigcms{:U('PropertyService/products_qrcode_pre')}";
        $(".checkboxes").each(function() {
            if ($(this).is(':checked')) {
                ids += ',' + $(this).val(); //逐个获取id
            }
