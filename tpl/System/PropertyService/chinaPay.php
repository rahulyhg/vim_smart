    ...
    <div class="page-content">
        ...
        <div class="row">
            <form action="{pigcms{$pay_url}" method="post" class="form-horizontal" id="form_sample_1" target="_blank">
                ...
                <input type="hidden" name="MerBgUrl" value="http://www.hdhsmart.com/admin.php?g=System&c=PropertyService&a=bgReturn"/>
                <input type="hidden" name="MerPageUrl" value="http://www.hdhsmart.com/admin.php?g=System&c=PropertyService&a=pgReturn&company_id={pigcms{$company_id}&mid={pigcms{$mid}&type={pigcms{$type}&order_id={pigcms{$order_id}"/>
                ...
                <input type="hidden" name="MerResv" value="{pigcms{$MerResv}"/>
                        ...
                        <div class="portlet-body">
                            ...
                            <div class="form-body">
                                ...
                                <div class="form-group form-md-line-input">
                                    <label class="col-md-5 control-label" for="form_control_1" style="text-align: center"><span style="font-size:16px;">您正在缴费，请确认以下订单金额！</span>
                                ...
                                <div class="form-group form-md-line-input">
                                    ...
                                    <div class="col-md-9">
                                        <input type="text" class="form-control"  name="OrderAmt" value="{pigcms{$money}" readonly="readonly"/><input  type="text" id="money" style="display: none;width:100%; outline:none; border:1px #44b549 solid; background-color:#FFFFFF; color:#676a6c; font-size:13px; padding:10px;">
                                ...
                                <div class="form-actions">
                                    <div class="row">
                                        <div class="col-md-offset-2 col-md-9">
                                            ...
                                            <button type="button" class="btn green" id="jump" style="display: none">提交</button>
    ...
    $('#form_sample_1').submit(function () {
        ...
        if(result==true){
            $.ajax({
                ...
                'data':{'OrderAmt':OrderAmt,'MerOrderNo':MerOrderNo,'TranDate':TranDate,'TranTime':TranTime,'MerId':MerId,'MerSplitMsg':MerSplitMsg,'pid':pid,'mid':mid,'type':type,'MerResv':MerResv},
                'type':'POST',
                'dataType':'JSON',
                'async':false,
            ...
            swal({  title: "请在新打开的页面完成支付",
                ...
                if (isConfirm) {
                    $.ajax({
                        ...
                        data:{'MerOrderNo':MerOrderNo,'TranDate':TranDate,'TranTime':TranTime,'MerId':MerId},
                        type:'POST',
                        success:function (msg) {
                            if(msg == 1){
                                swal({   title: "支付成功",   text: "支付已经完成！",   type: "success",  confirmButtonColor: "#DD6B55",   confirmButtonText: "好的",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('tenant_list')}" });
                            }else if(msg==2&&msg==4){
                                swal({   title: "支付失败",   text: "您本次的支付失败了",   type: "error",  confirmButtonColor: "#DD6B55",   confirmButtonText: "完成",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('tenant_list')}" });
                            }else if(msg==3){
                                swal({   title: "查询失败",   text: "系统故障，请联系管理员解决",   type: "error",  confirmButtonColor: "#DD6B55",   confirmButtonText: "完成",   closeOnConfirm: false }, function(){ window.location.href="{pigcms{:U('tenant_list')}" });
                            }else{
                                console.log(msg);
                            }
                        }
