                ...
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div style="float: right; width:250px; margin-right: 15px;">
                                        ...
                                        <select style="width:150px; height:34px; float: right; padding:6px 12px; background-color:#FFFFFF; border: 1px solid #c2cad8;" id="select_status_id">
                                            ...
                                            <option value="1" <if condition="$status eq 1">selected</if>>新申请</option>
                                            <option value="2" <if condition="$status eq 2">selected</if>>已通过</option>
                                            <option value="3" <if condition="$status eq 3">selected</if>>库存不足</option>
                                            <option value="4" <if condition="$status eq 4">selected</if>>已驳回</option>
                        ...
                        <div id="form_modal" class="modal fade" role="dialog" aria-hidden="true">
                            ...
                            <!-- <form action="{pigcms{:U('Off/save_order')}" method="post" class="form-horizontal" id="modal_order" enctype="multipart/form-data"> -->
                                        ...
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu pull-left" role="menu" style="position:relative; margin-top: 35px;margin-left: -138px;">
                                                <li style="margin-bottom: 2px;">
                                                    ...
                                                    <!-- <a href="{pigcms{:U('Off/update_apply_status',array('status'=>2))}">
                                                ...
                                                <li style="margin-bottom: 2px;">
                                                    ...
                                                    <!-- <a href="{pigcms{:U('Off/update_apply_status',array('status'=>3))}">
                                                ...
                                                <li>
                                                    ...
                                                    <!-- <a href="{pigcms{:U('Off/update_apply_status',array('status'=>4))}">
                    ...
                    <!-- 模态框结束     -->
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <foreach name="apply_list" item="vo" id="change_data">
                                <tr>
                                    <td>
                                        <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                    ...
                                    <!-- <td>{pigcms{$vo.deptname}</td> -->
                                    <td>{pigcms{$vo.village_name} - {pigcms{$vo.company_name} - {pigcms{$vo.deptname}</td>
                                    <td>{pigcms{$vo.add_name}</td>
                                    <td>
                                        <a href="#form_modal" data-toggle="modal">
                                            <button id="sample_editable_1_new" value="{pigcms{$vo.id}" class="btn sbold green" onclick="getGoodsContent(this);">查看物品详情
                                                <!-- <i class="fa fa-plus"></i> -->
                                            </button>
                                        </a>
                                    </td>
                                    <!-- <td>
                                        <foreach name="vo['goods']" item="voi" id="change_data">
                                            <ol type="1">
                                                <li>
                                                物品名称:{pigcms{$voi.goods_name},规格:{pigcms{$voi.specification},数量:{pigcms{$voi.count},单价:{pigcms{$voi.price}</li>
                                            </ol>
                                        </foreach>
                                    </td> -->
                                    ...
                                    <td>
                                        <if condition="$vo['status'] eq 1">
                                            <button class="btn btn-sm blue btn-outline filter-submit margin-bottom" id="supplier_{pigcms{$vo.id}" value="{pigcms{$vo.status}">新申请</button>
                                            <!-- <input type="hidden" id="supplier_{pigcms{$vo.id}_id" value="{pigcms{$vo['id']}"> -->
                                        <elseif condition="$vo['status'] eq 2"/>
                                            <button class="btn btn-sm green btn-outline filter-submit margin-bottom" id="supplier_{pigcms{$vo.id}" value="{pigcms{$vo.status}">已通过</button>
                                            <!-- <input type="hidden" id="supplier_{pigcms{$vo.id}_id" value="{pigcms{$vo['id']}"> -->
                                        <elseif condition="$vo['status'] eq 3"/>
                                            <button class="btn btn-sm gray btn-outline filter-submit margin-bottom" id="supplier_{pigcms{$vo.id}" value="{pigcms{$vo.status}">库存不足</button>
                                            <!-- <input type="hidden" id="supplier_{pigcms{$vo.id}_id" value="{pigcms{$vo['id']}"> -->
                                        <else/>
                                            <button class="btn btn-sm red btn-outline filter-submit margin-bottom" id="supplier_{pigcms{$vo.id}" value="{pigcms{$vo.status}">已驳回</button>
                                            <!-- <input type="hidden" id="supplier_{pigcms{$vo.id}_id" value="{pigcms{$vo['id']}"> -->
                                        </if>
                                    </td>
                                    <td>
                                        <if condition="$vo['storage'] eq 1">
                                            <button class="btn btn-sm green btn-outline filter-submit margin-bottom" id="good_{pigcms{$vo.id}" value="{pigcms{$vo.storage}" onclick="changeType(this)">已出库</button>
                                            <input type="hidden" id="good_{pigcms{$vo.id}_id" value="{pigcms{$vo['id']}">
                                        <else/>
                                            <button class="btn btn-sm red btn-outline filter-submit margin-bottom" id="good_{pigcms{$vo.id}" value="{pigcms{$vo.storage}" onclick="changeType(this)">未出库</button>
                                            <input type="hidden" id="good_{pigcms{$vo.id}_id" value="{pigcms{$vo['id']}">
                                        </if>
                                    </td>
                                    <td>
...
<script>
    function getGoodsContent(obj){
        ...
        $.ajax({
            url:"{pigcms{:U('Off/get_goods_detail')}",
            data:{'id':id},
            // dataType: "json",
            type:'post',
...
function update_apply_status(obj) {
    ...
    layer.msg('你确定要执行此操作么？', {
        ...
        ,yes: function(index){
            ...
            //通过ajax异步删除
            $.ajax({
                url:"{pigcms{:U('Off/update_apply_status')}",
                data:{'status':status},                    
                type:'post',
                success:function(delmsg){
                    if(delmsg==='1'){
