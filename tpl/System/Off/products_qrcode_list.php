...
<!--                                    </div>-->
                                    ...
                                    <div class="btn-group">
                                        <a onclick="download_page()">
                                            <button id="sample_editable_1_new" class="btn sbold green">全部下载
                                                <i class="fa fa-plus"></i>
                                            </button>
                                        </a>
                                    </div>
                                    ...
                                    <div class="btn-group">
                                        <a href="{pigcms{:U('Off/products_custom_list')}">
                                            <button id="sample_editable_1_new" class="btn sbold green">返回二维码管理页
                                            </button>
                                        </a>
                                    </div>
...
<!--                                        <h4 class="modal-title">创建二维码个数</h4>-->
<!--                                    </div>-->
...
<!--                                                <div class="col-md-8">-->
<!--                                                    <input class="form-control input-medium date-picker" placeholder="请输入整数" name="num" id="modal_tan" type="text" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"  /> </div>-->
<!--                                            </div>-->
<!--                                        </form>-->
<!--                                    </div>-->
...
<!--                        </div>-->
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <foreach name="qrArr" item="vo" key="k">
                                <tr>
                                    ...
                                    <td style="line-height: 40px;">
                                        <img _src="{pigcms{$vo.adress}" style="width: 40px;height: 40px;" onmouseover="big({pigcms{$vo.id})" onmouseout="small()" id="henfan_{pigcms{$vo.id}" />
                                    </td>
                                    <td style="line-height: 40px;">
                                        <a href="{pigcms{:U('Off/history_qrcode',array('id'=>$vo['id'],'cid'=>$cid))}">历史记录</a>
                                    </td>
                                    <td style="line-height: 40px;">
                                        <a onclick="downloadIamge({pigcms{$vo.id})">二维码下载</a>
                                    </td>
                                    <!--                                    <td style="line-height: 40px;">-->
                                    <!--                                        <div class="btn-group">-->
...
<script type="text/javascript">
    ...
    table.dataTable({
        ...
        ajax: {
            url: '{pigcms{:U("ajax_qrcode_list",array('cid'=>$cid))}',
            type: 'POST'
        },
        "bStateSave": true, // save datatable state(pagination, sort, etc) in cookie.
        searchDelay: 700,//防抖延迟
        "lengthMenu": [
            [5, 15, 20, -1],
        ...
        "aoColumns": [
            ...
            {
                "sTitle" : '<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"> <input type="checkbox" class="group-checkable" data-set="#sample_1 .checkboxes"> <span></span> </label>',　　　//显示的列名
                "mDataProp": "check_id",　　//对应的数据中的字段名
                "sDefaultContent" : "",
                "sClass" : "center"
            },
            {
                "sTitle" : "二维码编号",　　　//显示的列名
                "mDataProp": "pro_qrcode",　　//对应的数据中的字段名
                "sDefaultContent" : "",
                "sClass" : "center"
            },
            {
            ...
            {
                "sTitle" : "责任人",
                "mDataProp": "admin_name",
                "sDefaultContent" : "",
                "sClass" : "center"
            },
            {
                "sTitle" : "时间",
                "mDataProp": "trans_time",
                "sDefaultContent" : "",
                "sClass" : "center"
            },
            {
    ...
    function pixia() {
        var ids = '';
        var url = "{pigcms{:U('Off/products_zip_z')}";
    ...
    function qr_print() {
        var ids = '';
        var url = "{pigcms{:U('Off/products_qrcode_pre')}";
        $(".checkboxes").each(function() {
            if ($(this).is(':checked')) {
                ids += ',' + $(this).val(); //逐个获取id
            }
    ...
    function download_page() {
        for(var i=1;i<={pigcms{$page_num};i++){
            $("body").append("<iframe src='{pigcms{:U('Off/products_zip_z',array('cid'=>$_GET['cid']))}&page="+i+"' style='display:none'></iframe>");
            sleep(500);
        }
    }
    // function pixia() {
