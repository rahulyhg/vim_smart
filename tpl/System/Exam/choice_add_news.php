...
<!--                                <div class="col-md-6">-->
<!--                                    <div class="btn-group">-->
<!--                                        <a href="{pigcms{:U('Exam/question_add_news')}">-->
<!--                                            <button id="sample_editable_1_new" class="btn sbold green">添加选题-->
...
<!--                                </div>-->
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <tr>
                                ...
                                <th  style="width: 100px; text-align: center;">题库序号</th>
                    ...
                    <div class="form-actions">
                        <div class="row">
                            <div class="col-md-offset-2 col-md-9">
                                <button type="submit" onclick="sub()" class="btn green">确认添加</button>
                                <button type="reset" class="btn default" onclick="window.location.href='http://www.hdhsmart.com/admin.php?g=System&c=Exam&a=choice_news&id={pigcms{$s_id}'">返 回</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function delete_pr_info(obj) {
        layer.msg('你确定要删除么？本操作不可恢复！', {
            ...
            ,yes: function(index){
                ...
                //通过ajax异步删除
                $.ajax({
                    url:"{pigcms{:U('Exam/question_del_news')}",
                    data:{'id':id},
                    type:'post',
    ...
    function sub() {
        var ids = '';
        var s_id = "{pigcms{$s_id}";
        var p_id = "{pigcms{$Think.get.id}";
        var score = "{pigcms{$score}";
        var totalper = "{pigcms{$totalper}";
        ...
        if (ids.length == 0) {
            ...
            if (confirm("确定添加？")) {
                $.ajax({
                    type: "get",
                    url: "{pigcms{:U('Exam/choice_add_save_news')}",
                    data: "idStr="+ids+"&sid="+s_id+"&pid="+p_id+"&score="+score+"&totalper="+totalper,
                    success: function(delmsg) {
                        if(delmsg==='1'){
                            //逻辑删除成功！
                            ...
                            //逻辑删除失败！
                            layer.msg("第"+delmsg+"条添加失败！<br />可能分值过大，超过总分!", {icon: 5});
                            setTimeout(function(){
                                window.location.reload()
                            },2000);
                        }
