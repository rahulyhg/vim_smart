                    ...
                    <div class="portlet-body">
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{pigcms{:U('Exam/choice_add_news',array('id'=>$s_id))}">
                                            <button id="sample_editable_1_new" class="btn sbold green">手动选题
                                                <i class="fa fa-plus"></i>
                                            </button>
                                        </a>
                                    </div>
                                    <div class="btn-group">
                                        <a href="{pigcms{:U('Exam/choice_zidong_add_news',array('id'=>$s_id))}">
                                            <button id="sample_editable_1_new" class="btn sbold green">自动选题
                                                <i class="fa fa-plus"></i>
                                            </button>
                                        </a>
                                    </div>
                                    <div class="btn-group" style="color: #cc463d !important;">
                                        <button id="sample_editable_1_new" onclick="sub()" class="btn default">批量移除
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <tr>
                                ...
                                <th  style="width: 100px; text-align: center;">选题号</th>
                                <th  style="width: 250px; text-align: center;">所属题库</th>
                                <th  style="width: 150px; text-align: center;">题型</th>
                                <th  style="width: 150px; text-align: center;">难度</th>
                                <th  style="width: 100px; text-align: center;">分数</th>
                                <th  style="text-align: center;">题目</th>
                                <th  style="width: 150px; text-align: center;">操作</th>
                            </tr>
                            </thead>
                            <foreach name="questionArr" item="vo">
                                <tr>
                                    <td>
                                        <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                    ...
                                    <td>{pigcms{$vo.id}</td>
                                    <td>{pigcms{$vo.subject_name}</td>
                                    <td>
                                        <if condition="$vo['type'] eq 1">单选<elseif condition="$vo['type'] eq 2" />
                                            多选<elseif condition="$vo['type'] eq 3" />主观</if>
                                    </td>
                                    <td>
                                        <if condition="$vo['difficulty'] eq 1">低<elseif condition="$vo['difficulty'] eq 2" />
                                            中<elseif condition="$vo['difficulty'] eq 3" />高</if>
                                    </td>
                                    <td>{pigcms{$vo.fraction}</td>
                                    <td>{pigcms{$vo.question}</td>
                                    <td><span onclick="delete_pr_info(this)" id="{pigcms{$vo.id}">
                                            <a href="javascript:;">
                                                <i class="icon-tag"></i> 取消选题 </a>
                                        </span>
                                    </td>
                                </tr>
                            </foreach>
                    ...
                    <div class="form-actions">
                        <div class="row">
                            <div class="col-md-offset-2 col-md-9">
                                <button type="reset" class="btn default" onclick="window.location.href='http://www.hdhsmart.com/admin.php?g=System&c=Exam&a=paper_news'">返 回</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    function delete_pr_info(obj) {
        layer.msg('你确定要取消么？', {
            ...
            ,yes: function(index){
                layer.close(index);
                var id=$(obj).attr('id');
                var pid = "{pigcms{$Think.get.id}";
                //通过ajax异步删除
                $.ajax({
                    url:"{pigcms{:U('Exam/choice_del_news')}",
                    data:{'id':id,'pid':pid},
                    type:'post',
    ...
    function sub() {
        ...
        if (ids.length == 0) {
            ...
            if (confirm("确定全部移除？")) {
                $.ajax({
                    type: "get",
                    url: "{pigcms{:U('Exam/choice_all_del_news')}",
                    data: "idStr="+ids+"&pid="+pid,
                    success: function(delmsg) {
                        if(delmsg==='1'){
                            //逻辑删除成功！
