                    ...
                    <div class="portlet-body">
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{pigcms{:U('Exam/paper_add_news')}">
                                            <button id="sample_editable_1_new" class="btn sbold green">新建试卷
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            <thead>
                            <tr>
                                <th  style="width: 120px; text-align: center;">试卷编号</th>
                                <th  style="width: 200px; text-align: center;">所属题库</th>
                                <th  style="text-align: center;">试卷标题</th>
                                <th  style="width: 150px; text-align: center;">考试时长(分)</th>
                                <th  style="width: 150px; text-align: center;">总分(分)</th>
                                <th  style="width: 150px; text-align: center;">当前选题数量(个)</th>
                                <th  style="width: 200px; text-align: center;">创建日期</th>
                                <th  style="width: 120px; text-align: center;">选题状态</th>
                                <th  style="width: 150px; text-align: center;">操作</th>
                            </tr>
                            </thead>
                            <foreach name="paperArr" item="vo">
                                <tr>
                                    <td>{pigcms{$vo.id}</td>
                                    <td>{pigcms{$vo.subject_name}</td>
                                    <td><a href="{pigcms{:U('Exam/choice_news',array('id'=>$vo['id']))}">{pigcms{$vo.title}</a></td>
                                    <td>{pigcms{$vo.testtime}</td>
                                    <td>{pigcms{$vo.totalper}</td>
                                    <td>{pigcms{$vo.c_num}</td>
                                    <td>{pigcms{$vo.start_time|date="Y-m-d H:i:s",###}</td>
                                    <td><if condition="$vo['c_score'] neq $vo['totalper']"><span style="color: #cc463d">未完成</span><else /><span style="color: #0dcecb;">完成</span></if></td>
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu pull-left" role="menu" style="position:relative;">
                                                <if condition="$vo['c_score'] eq $vo['totalper']">
                                                    <li>
                                                        <a data-toggle="modal" data-target="#common_modal" href="{pigcms{:U('Exam/create_text_news',array('id'=>$vo['id']))}">
                                                            <i class="icon-docs"></i> 加入考试列表 </a>
                                                    </li>
                                                </if>
                                                <li>
                                                    <a href="{pigcms{:U('Exam/choice_news',array('id'=>$vo['id']))}">
                                                        <i class="icon-docs"></i> 查看选题 </a>
                                                </li>
                                                <li>
                                                    <a href="{pigcms{:U('Exam/paper_edit_news',array('id'=>$vo['id']))}">
    ...
    function delete_pr_info(obj) {
        layer.msg('你确定要删除么？本操作不可恢复！', {
            ...
            ,yes: function(index){
                ...
                //通过ajax异步删除
                $.ajax({
                    url:"{pigcms{:U('Exam/paper_del_news')}",
                    data:{'id':id},
                    type:'post',
