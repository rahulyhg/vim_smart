                            ...
                            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                                ...
                                <foreach name="store_list" item="vo">
                                    <tr class="odd gradeX">
                                        ...
                                        <td class="center">{pigcms{$vo.sort}</td>
                                        <td class="center">{pigcms{$vo.name}</td>
                                        <td class="center">{pigcms{$vo.contact_name}</td>
                                        <td>{pigcms{$vo.phone}</td>
                                        <td>{pigcms{$vo.area_name} - {pigcms{$vo.adress}</td>
                                        <td>
                                            <a href="{pigcms{$Think.config.site_url}/index.php?g=Index&c=Recognition&a=see_qrcode&type=meal&id={pigcms{$vo['store_id']}" class="see_qrcode">查看二维码</a>
                                        </td>
                                        <td>
                                            <a style="width:80px;" title="查看店铺订单" href="{pigcms{:U('order',array('store_id'=>$vo['store_id']))}">查看店铺订单</a>
                                        </td>
                                        <td>
                                            <a style="width: 60px;" title="商品分类" href="{pigcms{:U('meal_sort',array('store_id'=>$vo['store_id']))}">商品分类</a>
                                        </td>
                                    </tr>
                                </foreach>
                                </tbody>
