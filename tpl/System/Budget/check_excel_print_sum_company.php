...
<block name="table-toolbar-left">
    ...
    <div class="btn-group">
        <a href="{pigcms{:U('Budget/ajax_excel_print',array('company_id'=>$company_id,'year'=>$year))}">
...
<block name="body">
    <div class="tabbable-custom nav-justified" style="width:100%;overflow-x: scroll;">
        ...
        <table class="table table-striped table-bordered table-hover" style="overflow-x: auto" id="sample_1" >
            <thead>
            <tr>
                <td colspan="{pigcms{:count($data)*2+5}" align="center" style="font-size: 25px">{pigcms{$year}年{pigcms{$title}预算执行汇总表</td>
            </tr>
            <tr>
                ...
                <foreach name="data" item="vo">
                    <th width="10%" colspan="2">{pigcms{$vo['village_name']}</th>
                </foreach>
            </tr>
            <tr>
                <th width="5%">预算金额</th>
            ...
            <tr>
                ...
                <td>{pigcms{:number_format($sum['input']['sum_money'],2)}</td>
                <td>{pigcms{:number_format($sum['input']['sum_sum'],2)}</td>
                <foreach name="data" item="vo1">
                    <if condition="is_array($vo1)">
                    <td>{pigcms{:number_format($vo1['input']['sum_money'],2)}</td>
                    <td>{pigcms{:number_format($vo1['input']['sum_sum'],2)}</td>
                    </if>
                </foreach>
            </tr>
            <volist name="sum['input']['1']['children']" id="vo" >
                <tr>
                    <td>{pigcms{$i}</td>
                    ...
                    <foreach name="data" item="vo1" key="key1">
                                <td><div class="tagDiv">{pigcms{:number_format($vo1['input']['1']['children'][$key]['sum_money'],2)}</div></td>
                                <td><div class="tagDiv">{pigcms{:number_format($vo1['input']['1']['children'][$key]['sum_sum'],2)}</div></td>
                    </foreach>
                </tr>
            </volist>
            <tr>
                ...
                <td>{pigcms{:number_format($sum['output']['sum_money'],2)}</td>
                <td>{pigcms{:number_format($sum['output']['sum_sum'],2)}</td>
                <foreach name="data" item="vo">
                    <td>{pigcms{:number_format($vo['output']['sum_money'],2)}</td>
                    <td>{pigcms{:number_format($vo['output']['sum_sum'],2)}</td>
                </foreach>
            </tr>
            <foreach name="sum['output']" item="vo" key="ke">
                <volist name="vo['children']" id="vo1" key="k">
                    <tr>
                        ...
                        <foreach name="data" item="value" key="key1">

                            <td><div class="tagDiv">{pigcms{:number_format($value['output'][$ke]['children'][$key]['sum_money'],2)}</div></td>
                            <td><div class="tagDiv">{pigcms{:number_format($value['output'][$ke]['children'][$key]['sum_sum'],2)}</div></td>
                        </foreach>
                    </tr>
                </volist>
                <if condition="is_array($vo)">
                    <tr>
                        ...
                        <foreach name="data" item="value">
                            <td><div class="tagDiv">{pigcms{:number_format($value['output'][$ke]['sum_money'],2)}</div></td>
                            <td><div class="tagDiv">{pigcms{:number_format($value['output'][$ke]['sum_sum'],2)}</div></td>
                        </foreach>
                    </tr>
                </if>
            </foreach>
            <tr>
                <td>三</td>
                <td colspan="2">净收支</td>
                <td>{pigcms{:number_format($sum['sum']['sum_money'],2)}</td>
                <td>{pigcms{:number_format($sum['sum']['sum_sum'],2)}</td>
                <foreach name="data" item="vo">
                    <td>{pigcms{:number_format($vo['sum']['sum_money'],2)}</td>
                    <td>{pigcms{:number_format($vo['sum']['sum_sum'],2)}</td>
                </foreach>
            </tr>
            </tbody>
        </table>
