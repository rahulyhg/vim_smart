							...
							<tbody>
								...
								<volist name="order_list" id="vo">
									<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
										...
										<td width="120">
											<if condition="$vo['pay_status'] eq 0"><span style="color:blue">未支付</span>
											<elseif condition="$vo['pay_status'] eq 1" /><span style="color:green">已支付</span>
											<elseif condition="$vo['pay_status'] eq 3" /><span style="color:red">支付超时或失败</span>
											</if>
										</td>
										<td width="90">
											<if condition="$vo['order_status'] eq 0"><span style="color:red">订单失效</span>
											<elseif condition="$vo['order_status'] eq 1" /><span style="color:blue">商家未确认</span>
											<elseif condition="$vo['order_status'] eq 2" /><span style="color:green">商家已确认</span>
											<elseif condition="$vo['order_status'] eq 3" /><span style="color:green">正在配送</span>
											<elseif condition="$vo['order_status'] eq 4" /><span style="color:green">订单已配送</span>
											<elseif condition="$vo['order_status'] eq 5" /><span style="color:green">用户已确认</span>
											</if>
										</td>
										<td width="90">
