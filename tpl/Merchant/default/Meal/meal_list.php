										...
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<td class="button-column">
												<label class="waimaistatus" style="display:inline-block;">
													<input name="switch-field-1" class="ace ace-switch ace-switch-6" type="checkbox" data-id="{pigcms{$vo.meal_id}" <if condition="$vo['waimai_status'] eq 1">checked="checked" data-status="OPEN"<else/>data-status="CLOSED"</if>/>
													<span class="lbl"></span>
												</label>
											</td>
											<td class="button-column">
...
function waimaiStatus(dom1, dom2, status1, status2, attribute){
	$(dom1).each(function(){
		...
		$.ajax({
			url:"{pigcms{:U('Meal/waimai_status')}",
			type:"post",
