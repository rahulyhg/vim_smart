		...
		<div class="page-content-area">
			<div class="row">
				<div class="col-xs-12" style="padding-left:0px;padding-right:0px;">
					<if condition="$_GET['page'] lt 2">
						...
						<!--div class="alert alert-danger" style="margin-top:5px;margin-bottom:0px;">
							<button type="button" class="close" data-dismiss="alert">
								<i class="ace-icon fa fa-times"></i><a href="{pigcms{:U('Customer/fans_list', array('from_merchant' => 3))}"><input type="radio" name="fans" />
							</button>
					...
					<div class="alert" style="margin-top:5px;margin-bottom:0px;">
						<volist name="count_list" id="cl">
						<if condition="$cl['from_merchant'] eq 0"><label><input type="radio" name="fans" value="0" <if condition="$from_merchant eq 0">checked</if> />扫描商家产品二维码({pigcms{$cl['count']})</label>　
						<elseif condition="$cl['from_merchant'] eq 1" /><label><input type="radio" name="fans" value="1" <if condition="$from_merchant eq 1">checked</if> />扫描商家二维码({pigcms{$cl['count']})</label>　
						<elseif condition="$cl['from_merchant'] eq 2" /><label><input type="radio" name="fans" value="2" <if condition="$from_merchant eq 2">checked</if> />平台赠送({pigcms{$cl['count']})</label>　
						<else /><label><input type="radio" name="fans" value="3" <if condition="$from_merchant eq 3">checked</if> />扫描产品推广二维码({pigcms{$cl['count']})</label>
						</if>
						</volist>
					</div>
								...
								<if condition="$fans_list">
									<volist name="fans_list" id="vo">
										<tr class="<if condition="$i%2 eq 0">odd<else/>even</if>">
											...
											<else />
											<td colspan="7"><span style="color: green">等待粉丝注册完善个人资料</span></td>
											</if>
											<td>{pigcms{$vo.dateline|date='Y-m-d H:i:s',###}</td>
											<if condition="$vo['last_time'] gt 0">
											<td>{pigcms{$vo.last_time|date='Y-m-d H:i:s',###}</td>
											<else />
											<td><span style="color: red">暂未登陆</span></td>
											</if>
...
<script type="text/javascript">
	$(function(){
		...
		$('input[type=radio]').click(function(){
			location.href="{pigcms{:U('Customer/fans_list')}&from_merchant="+$('input[type=radio]:checked').val();
		});
	});
</script>
