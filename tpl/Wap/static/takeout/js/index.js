	...
	rendStoreList: function(){
		...
		_html = iTemplate.makeList(TPL, that.res.data, function(k,v){
			return {
				...
				dist_str:v.distjuli > 0 ? (function(){ var _distjuli=v.distjuli>1000? ((v.distjuli/1000).toFixed(2)+' KM'):(parseInt(v.distjuli)+ " M"); return '<i class="ico_addres"></i>距离：'+_distjuli })() : (function(){
					if(v.dist > 0){
				...
				/*dist_str:v.dist ? (function(){
					var _dist = v.dist>1000? ((v.dist/1000).toFixed(2)+' KM'):(parseInt(v.dist)+ " M");
					return '<i class="ico_addres"></i>距离：'+_dist;
				})() : (v.distjuli > 0 ? '<i class="ico_addres"></i>距离：'+v.distjuli :'')*/
			}
		});
