                ...
                <div class="item-detail">
                        <p class="item-price-sell">
                            <span class="item-name">
                                <span class="gtt">预约用户名：</span>{{ item.name}}
                            </span>
                            <span class="item-price">
                                <span class="gtt">用户手机：</span>{{ item.contact}}
                            </span>
                        </p>
                        <p class="item-sell">
                            <span class="gtt">预约类型：</span>{{ item.meal_name}}
                        </p>
                        <p class="item-sell" v-if="item.appointment_start_time != '1970-01-01 08:00'">
                            <span class="gtt">起始时间：</span>{{ item.appointment_start_time}}
                        </p>
                        <p class="item-sell" v-if="item.appointment_end_time != '1970-01-01 08:00'">
                            <span class="gtt">结束时间：</span>{{ item.appointment_end_time}}
                        </p>
                        <p class="item-sell">
                            <span class="gtt">当前状态：</span>
                            <span >
                                <font style="color:red" v-if="item.is_read == 0"  >未处理</font>
                                <font style="color:blue" v-else >已处理</font>
                            </span>
                        </p>
                    <p class="item-img">
                        <a class="" href="javascript::">
                            <span class="item-img1" pid="{{ item.pigcms_id }}" v-on:click="set_read(index)" v-show="item.is_read==0">
        ...
        methods: {
            ...
            set_read:function(i){
                if(window.confirm("你确认标记为已读")){
                    //异步获取数据
                    var data;
                    var url =  "{pigcms{:U('is_read')}";
                    var pigcms_id = this.items[i].pigcms_id;
                    $.ajax({
                        url:url,
