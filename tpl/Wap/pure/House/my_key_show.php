...
<head>
	...
	<link rel="stylesheet" href="{pigcms{$static_path}css/bootstrap.min.css" type="text/css"></link>
...
a, a:visited, a:hover {color:#0697dc;}
.gre2x {
    float: right;
    width: 35%;
    height: 40px;
...
function weuiSwitch(){
	$('.weui_switch').one('click',function(){		
		...
		$.ajax({
			'url':"{pigcms{:U('House/control_show')}",
			...
			$.ajax({
				...
				'success':function(msg){
					if(msg.code_error==0){
						...
						//TODO:下列代码为了提高定位的准确性，在进入页面的时候再次定位筛选了位置。
						weuiSwitch();
						/*$.ajax({
							'url':"{pigcms{:U('House/control_show_ajax')}",
							'data':{},
							'type':'POST',
							'dataType':'JSON',
							'success':function(msg){
...
<if condition="$key neq 1">
    ...
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" onClick="window.location.href =window.location.href;">
        <div class="modal-body">
            <img src="{pigcms{$static_path}images/kill.png" width="100%" height="100%" />
        </div>
    </div>
    <header class="pageSliderHide"><div id="backBtn"></div>智能门禁</header>
    <div class="control_result">
        <volist name="control_result" id="vo" key="k">
            <div class="zt">{pigcms{$vo.beforparent}</div>
            <div class="zttt">{pigcms{$vo.range}</div><div style="clear:both"></div>
            <div class="shtx_dkt">
                <volist name="vo.beforchild" id="val">
                    <input type="hidden" name="controlNum" value="" id="controlNum">
                    <div class="shtx_dk">
                        <volist name="val.afterchild" id="child">
                            <div class="shtx_xmt">
                                <div class="gre"><span style="color:#aaaaaa; font-size:14px;">[{pigcms{$val.afterparent}]</span> <span style="font-size:16px; color:#000000;">{pigcms{$child.ac_name}</span></div>
                                <div class="gre2x">
                                    <div class="bd spacing"><a href="javascript:;" id="showDialog2"><input class="weui_switch" type="checkbox" data_acval="{pigcms{$child.ac_id}" data_nodeid="{pigcms{$child.nodeid}" data_sensorid="{pigcms{$child.sensorid}" data_village="{pigcms{$child.village_id}"/></a></div>
                                </div>
                                <div class="both"></div>
                            </div>
                        </volist>
    ...
    <div class="kdw"><a href="javascript:;" class="fee_blue" data-toggle="modal" data-target="#myModal">帮TA开门</a></div>
...
<else/>
    ...
    <header class="pageSliderHide"><div id="backBtn"></div><!--广发银行大厦-->{pigcms{$now_village.village_name}</header>
    <!--<div class="shtx_dk">
    ...
    <volist name="control_result" id="vo" key="k">
        <div class="zt">{pigcms{$vo.parent}</div>
        <div class="shtx_dkt">
            <input type="hidden" name="controlNum" value="" id="controlNum">
            <div class="shtx_dk">
                <volist name="vo.child" id="child">
                    <div class="shtx_xmt">
                        <div class="gre">{pigcms{$child.ac_name}</div>
                        <div class="gre2x">
                            <div class="bd spacing"><a href="javascript:;" id="showDialog2"><input class="weui_switch" type="checkbox" data_acval="{pigcms{$child.ac_id}" data_nodeid="{pigcms{$child.nodeid}" data_sensorid="{pigcms{$child.sensorid}"/></a></div>
    ...
    <div class="kdw"><if condition="$role neq 2"><a href="javascript:;" class="fee_blue" data-toggle="modal" data-target="#myModal">帮TA开门</a></if></div>
    <div class="kdw2"><a href="javascript:;" class="weui_btn weui_btn_warn_blue eww">返 回</a></div>
