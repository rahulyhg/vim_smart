...
$(window).load(function(){
	//alert("{pigcms{$msg}");	//提示信息
	//alert("{pigcms{$url}");
	//WeixinJSBridge.call("closeWindow");	//关闭
	//window.close();
	var msg_data="{pigcms{$msg}";
	if(msg_data){
		layer.open({content:"网络卡顿请重试！",shadeClose:false,btn:['确定'],yes:function(){
			var url_data="{pigcms{$url}";			
			if(url_data){
				window.location.href="{pigcms{:U($url,array('village_id'=>$now_village['village_id'],'ac_id'=>$ac_id))}";
			}else{
				WeixinJSBridge.call("closeWindow");	//关闭
			}		
		}});
