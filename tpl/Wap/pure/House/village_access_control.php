...
<head>
	...
	<script type="text/javascript" src="{pigcms{$static_path}js/village_control.js" charset="utf-8"></script>
...
<div class="sfrz_zkd"><img src="{pigcms{$static_path}images/qq.jpg" style="width:100%;" /></div>
<form id="access_form" onSubmit="return false;">
	<div class="shtx_dk">
		<div class="shtx_xm">
			...
			<div class="shtx_kk"><input name="truename" type="text" value="{pigcms{$user_info.name}" style="color:#ababab; border:none; width:90%; font-size:14px;" placeholder="请输入真实姓名"/></div>
			<div style="float:right; color:#FF0000; padding-top:13px; font-size:14px; padding-right:15px;">*</div>
			<div class="both"></div>
		</div>
        <if condition="$village_type neq 1">
		<div class="shtx_xm">
				...
				<div class="weui_cell_bd weui_cell_primary">
					<select class="weui_select" name="company_id" id="company_id"><!--此处的class要看下，实现下拉样式-->
						...
						<volist name="company_list" id="vo">
							<option value="{pigcms{$vo['company_id']}" <if condition="$user_info['company_id'] eq $vo['company_id']">selected</if> >{pigcms{$vo.company_first}-{pigcms{$vo.company_name}</option>
						</volist>
					</select>
				</div>
			</div>
		...
		<div class="shtx_xm">
			...
			<div class="shtx_kk"><input type="number" name="phone" value="{pigcms{$user_info.phone}" style="color:#ababab;border:none;width:90%;font-size:14px;" placeholder="请输入手机号码" onFocus="focusPhone()" onBlur="blurPhone()"/></div>
			<div style="float:right; color:#FF0000; padding-top:13px; font-size:14px; padding-right:15px;">*</div>
			<div class="both"></div>
		</div>
		<div class="shtx_xm">
			<div class="shtx_pic"><img src="{pigcms{$static_path}images/re.jpg" style="width:14px; height:18px; margin-top:8px;"/></div>
			<div class="shtx_kk" style="width:80%;">
				...
				<button type="button" onClick="sendsms(this)" class="btn-weak" style="height:25px;background-color:#878787;color:#FFFFFF;border:none;" id="send-code">获取验证码</button>
			</div>
			<div style="float:right; color:#FF0000; padding-top:13px; font-size:14px; padding-right:15px;">*</div>
			<div class="both"></div>
		</div>
	</div>
    <div class="shtx_dk2" <if condition="$village_type eq 1">style="display:none;"</if>>
		<!--<div class="shtx_xm">
			<div class="shtx_pic"><img src="{pigcms{$static_path}images/q3.jpg" style="width:12px; height:18px; margin-top:8px;"/></div>
			<div class="shtx_kk">
				<input name="company_id" type="hidden" value="{pigcms{$user_info.company_id}"/>
		...
		<!--<div class="shtx_xm">
			...
			<div class="shtx_kk"><input type="text" name="youaddress" value="{pigcms{$user_info.address}" style="color:#ababab; border:none; width:90%; font-size:14px;" placeholder="公司地址"/></div>
			<div class="both"></div>
		</div>-->
		<div class="shtx_xm">
			<div class="shtx_pic"><img src="{pigcms{$static_path}images/ff1.png" style="width:14px; height:20px; margin-top:6px;"/></div>
			<div class="weui_cell weui_cell_select">
				<div class="weui_cell_bd weui_cell_primary">
		...
		<if condition="$user_info.card_type neq '4'">
		<div class="shtx_xm shtx_usernum">
			<div class="shtx_pic"><img src="{pigcms{$static_path}images/ff2.png" style="width:17px; height:22px; margin-top:5px;"/></div>
			<div class="shtx_kk"><input name="usernum" type="text" value="{pigcms{$user_info.usernum}" style="color:#ababab; border:none; width:90%; font-size:14px;" placeholder="请输入证件号"/></div>
			<div style="float:right; color:#FF0000; padding-top:13px; font-size:14px; padding-right:15px;">*</div>
			<div class="both"></div>
		</div></if>
			...
			<div class="weui_cell_bd weui_cell_primary">
				<div class="weui_uploader">
					<div class="weui_uploader_hd weui_cell">
						<div class="shtx_pic"><img src="{pigcms{$static_path}images/lrr.png" style="width:17px; height:17px; margin-top:9px; margin-left:2px;"/></div>
	...
	<div class="zkd"><a href="javascript:;" class="weui_btn weui_btn_warn_blue">提交</a></div>
</form>
...
<div class="zdb2">客服电话：<a href="tel:027-87779655" style="color:#0697dc;">027-87779655</a></div>
</body>
<script type="text/javascript" src="{pigcms{$static_public}js/date/valid.js"></script>
<script type="text/javascript">
    ...
    var url_data="{pigcms{:U('House/SmsCodeverify',array('village_id'=>$now_village['village_id']))}";
    $(function(){
                    ...
                    $.post(window.location.href,$('#access_form').serialize(),function(result){
                        if(result.err_code==0){
                            ...
                            layer.open({content:'提交成功!',shadeClose:false,btn:['确定'],yes:function(){
                                window.location.href="{pigcms{:U('House/village_access_next',array('village_id'=>$now_village['village_id']))}";
                            }});
                        }else{
