...
<div id="container">
    <div id="scroller" class="village_my">
        <nav class="me_pay">
            <section><p>房屋门牌号：</p>{pigcms{$now_room['room_name']}</section>
            <section><p>所属期数：</p>{pigcms{$now_room['project_desc']}</section>
            <volist name="pay_list" id="vo">
                <if condition="$vo['type'] eq 'property'or $vo['type'] eq 'park' ">
                    <section class="link-url" data-url="{pigcms{$vo.url}"><img src="{pigcms{$static_path}images/house/{pigcms{$vo.type}.png"/><p>{pigcms{$vo.name}</p><em><if condition="$vo['money'] gt 0">(已缴费至：{pigcms{$vo.money})<else/>(暂无缴费)</if></em></section>
                    <else/>
