...
var hui = (function(selector){
    ...
    _hui.prompt_two = function(msg, btnName, callBack,placeholder, val,placeholder1, val1, callBack2){
        if(!btnName){btnName = ['取消','确定'];}
        if(!placeholder){placeholder = '';}
        if(!val){val = '';}
        hui.dialogBase();
        hui.dialogDom.innerHTML = '<div id="hui-dialog-in"><div id="hui-dialog-msg" style="padding-bottom:12px;">'+msg+'</div><div id="hui-dialog-input-in"><input type="text" id="hui-dialog-input" placeholder="'+placeholder+'" value="'+val+'" /></div><br/><div id="hui-dialog-input-in1"><input type="text" id="hui-dialog-input1" placeholder="'+placeholder1+'" value="'+val1+'" /></div><div style="height:15px;"></div><div id="hui-dialog-btn-line"><div>'+btnName[0]+'</div><div>'+btnName[1]+'</div></div></div>';
        var btns = document.getElementById('hui-dialog-btn-line').getElementsByTagName('div');
        btns[0].onclick = function(){
            if(callBack2){callBack2();}
            hui.dialogClose();
        ...
        btns[1].onclick = function(){
            if(callBack){callBack(document.getElementById("hui-dialog-input").value,document.getElementById("hui-dialog-input1").value);}
            //hui.dialogClose();
        };
    };
	/* loading */
