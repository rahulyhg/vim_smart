...
function appendItemHtml(has_more, list){
	...
	for(var i=0;i<list.length;i++){
		...
		var content="<li class='item-list-container'>"+
			"<a class=''href='"+detailurl+"&village_id="+item.village_id+"&log_id="+item.log_id+"'><div class='item-detail'>"+
			"<p class='item-price-sell'>"+
			"<span class='item-name'><span class='gtt'>姓名：</span>"+item.name+"</span>"+
			"</p>"+
			"<p class='item-sell'><span class='gtt'>开门时间：</span>"+item.created+"</p>"+
			"<p class='item-img'><span><a class=''href='"+detailurl+"&village_id="+item.village_id+"&log_id="+item.log_id+"'><img src='"+staticpath+"images/xqt_blue.png' width='50'height='23'/></a></span>"+"</p>"
			"</div>"+
