...
<div class="nav_img">
	<img src="{pigcms{$user_info.avatar}">
</div>
</div>
	...
	<div class="dingdang">
		...
		<div class="action_border">
			<div class="weui_cell weui_cell_select weui_select_after"  style="background: #fff;">
				...
				<input type="hidden" name="uid" value="{pigcms{$user_info.uid}">
				<div class=" weui_cell_primary">             
					<span class="weui_select" name="merid" style="color:#464646;font-size: 27px;">{pigcms{$user_info.nickname}</span>
				</div>
			</div>
		</div>
		</div>
		...
		<div class="dingdang_create2 " id="cou" style="margin-top: -90px;">
			<div class="action_border">
				<div class="weui_cell weui_cell_select weui_select_after"  style="background: #fff;">
					<div class="weui_cell_hd" style="padding-left: 15px;color:#464646;font-size: 27px;padding-left: 29px;">优惠金额（<?=$discount_info['ds_scale']*10?>折优惠）{pigcms{$discount_info.ds_name}</div>
					<input type="hidden" name="ds_id" value="{pigcms{$discount_info.ds_id}">
...
<script type="text/javascript">
		...
		var ds_scale="{pigcms{$discount_info.ds_scale}";	//折扣
		//alert(ds_scale);
		function setYH(){
			var cou_money = "0";
			var pay_money = "";
						...
						setInterval(function(){		//定时轮询判断订单是否已付款
							$.ajax({
								...
								success: function(res){
									if(res.code==1){	//确认支付后
									   //alert(res.msg);
									   window.location.href="{pigcms{$config['site_url']}/wap.php?g=Wap&c=My&a=check_retrun&userContent="+res.msg; 
									}else{
										//alert(res.msg);
									}
								},
