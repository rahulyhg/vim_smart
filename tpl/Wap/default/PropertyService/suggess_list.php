...
<div class="zk">
    <div class="zk2" >
        <div class="sm">
            ...
            <div class="kk2"><span style="font-size: 16px;">意见反馈({pigcms{$count})</span></div>
        ...
        <div style="width:100%;" id="henfan">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" id="hehe">
                <tr>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;">建议人</td>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;width: 22%;">建议内容</td>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;width: 20%;">审核状态</td>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;width: 28%;">提交时间<br /><span style="font-size: 12px;"></span></td>
                </tr>
                <foreach name="list" item="vo">
                        <tr>
                            <td height="40" align="center" style="font-size:12px; color:#515455; border-bottom:1px #efefef solid;"><if condition="$vo['name'] eq ''"><span style="font-size: 12px;">该神秘人未透露姓名</span><else />{pigcms{$vo.name|substr=###,0,15}</if></td>
                            <td height="40" align="center" style="font-size:14px; color:#515455; border-bottom:1px #efefef solid;"><span style="font-size: 12px;">{pigcms{$vo.content|substr=###,0,15}</span></td>
                            <td height="40" align="center" style="font-size:12px; color:#515455; border-bottom:1px #efefef solid;"><if condition="$vo['is_read'] eq 0">未处理<else />已处理</if></td>
                            <td height="40" align="center" style="font-size:12px; color:#515455; border-bottom:1px #efefef solid;">{pigcms{$vo.time|date="Y-m-d H:i",###}</td>
                        </tr>
                </foreach>
            </table>
       </div>
...
<script>
    ...
    function sea() {
        var search = $("#searchInput").val();
        window.location.href = "{pigcms{:U('suggess_list')}"+"&point_status="+point_status+"&search="+search+"&more=1"+"&ym="+time+"&village_id="+village_id;
    }
    ...
    $(document.body).infinite().on("infinite", function() {
        ...
        setTimeout(function() {
            ...
            gt = parseInt(lt)-1;
            $.ajax({
                url:"{pigcms{:U('suggess_list_ajax')}",
                type:"get",
