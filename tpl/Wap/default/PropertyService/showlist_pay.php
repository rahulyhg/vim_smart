...
<div class="zk">
    <div class="zk2" >
        <div class="sm">
            ...
            <div class="kk2"><span style="font-size: 16px;">订单记录({pigcms{$count})</span></div>
        ...
        <div style="width:100%;" id="henfan">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" id="hehe">
                <tr>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;">消费者</td>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;width: 22%;">车牌号</td>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;width: 20%;">实缴金额</td>
                    <td height="40" align="center" style="border-bottom:1px #e1e1e1 solid; font-size:16px; color:#909090;width: 28%;">支付时间<br /><span style="font-size: 12px;">({pigcms{$begin_time})</span></td>
                </tr>
                <foreach name="list" item="vo">
                    <tr>
                        ...
                        <td height="40" align="center" style="font-size:14px; color:#515455; border-bottom:1px #efefef solid;">{pigcms{$vo.pay_loan}</td>
                        <td height="40" align="center" style="font-size:14px; color:#515455; border-bottom:1px #efefef solid;">{pigcms{$vo.pay_time|date="H:i",###}</td>
                    </tr>
                </foreach>
            </table>
        </div>
...
<script>
    ...
    var work_time = "{pigcms{$work_time}";
    var lt = 0;
    var gt = 0;
    ...
    function sea() {
        var search = $("#searchInput").val();
        window.location.href = "{pigcms{:U('showlist_pay')}"+"&point_status="+point_status+"&search="+search+"&ym="+time+"&work_time="+work_time+"&village_id="+village_id;
    }
    ...
    $(document.body).infinite().on("infinite", function() {
        ...
        setTimeout(function() {
            ...
            gt = parseInt(lt)-1;
            $.ajax({
                url:"{pigcms{:U('showlist_pay_ajax')}",
                type:"get",
                data:"point_status="+point_status+"&search="+search+"&ym="+time+"&more="+gt+"&work_time="+work_time+"&village_id="+village_id,
                // dataType:'json',
