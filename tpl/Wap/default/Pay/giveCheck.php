...
<div id="innerHtml">
	...
	<div class="skr">
		<div class="skr_1"><span class="sk">收款商家</span><span class="xm">{pigcms{$view_content.title}</span></div>
	</div>
	<if condition="$view_content['mer_name']">
	<div class="skr">
		<div class="skr_1"><span class="sk">优惠提供方</span><span class="xm">{pigcms{$view_content.mer_name}</span></div>
	</div>
	</if>
	...
	<div class="skr" style="margin-top: 10px;">
		<a href="{pigcms{$view_content.ds_url}"><img src="{pigcms{$view_content.ds_img}" style="width:100%;height:150px;border: 0;"></a>
	</div>
</div>
<script type="text/javascript">
	$(function(){
		var money = "{pigcms{$view_content.money}";
		var mer_name = "{pigcms{$view_content.title}";
		var out_trade_no = "{pigcms{$view_content.out_trade_no}";
		var cou_money = "{pigcms{$view_content.cou_money}";
		var ds_type = "{pigcms{$view_content.ds_type}";
		var meal_add = "{pigcms{$view_content.meal_add}";
		var ds_uid = "{pigcms{$view_content.ds_uid}";
		var ds_merId = "{pigcms{$view_content.ds_merId}";
		var mer_id = "{pigcms{$view_content.mer_id}";		
		var first_head="{pigcms{$view_content.first_head}";
		$.ajax({
