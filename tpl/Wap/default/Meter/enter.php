        ...
        <div class="weui-cell">
            ...
            <div class="weui-cell__ft">
                <a href="{pigcms{:U('meter_record',array('meter_code'=>$meter_info['meter_code'],'meter_hash'=>$meter_info['meter_hash']))}">
                    <span style="color: blue;text-align: center;">{pigcms{$meter_info.meter_code}<span><br />
                    <span style="color: blue;text-align: center;">({pigcms{$meter_info.rate}倍)<span>
                </a>
            </div>
    ...
    var model = new Vue({
        ...
        methods: {
            ...
            modal_update_cousume:function(){
                var self = this;
                var default_val = this.last_month_total;
                $.prompt({
                    title: '修改往期总用量',
                    input: default_val,
                    empty: false, // 是否允许为空
