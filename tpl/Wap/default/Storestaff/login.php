...
<body id="index" data-com="pagecommon">
		...
		<div id="w_login" style="display:none; position:absolute;width: 100%;height:300px;z-index:10;background: #f0efed;">
			<div id="normal-fieldset" class="normal-fieldset" style="height: 100%;">
				...
				<dl class="list">
						<dd class="dd-padding">
							<label class="mt"><i class="bank-icon icon" ></i><span class="pay-wrapper"><span style="width: 200px;">用户名：&nbsp;{pigcms{$wei_login.username}</span><input type="hidden" style="right: 0px;"  id="mt" value="{pigcms{$wei_login['id']}" <if condition="$i eq 1">checked="checked"</if> name="pay_type"></span></label>
						</dd>
				</dl>
			</div>
		...
		<a href="javascript:void(0);" id="w_login" style="display:none;"  onclick="window.top.artiframe('/admin.php?g=Wap&amp;c=Storestaff&amp;a=selectlogin&amp;openid={pigcms{$openid}','编辑商户信息',600,450,true,false,false,editbtn,'edit',true);">编辑</a>
		<script src="{pigcms{:C('JQUERY_FILE')}"></script>
		<script src="{pigcms{$static_public}js/laytpl.js"></script>
	    <script src="{pigcms{$static_path}layer/layer.m.js"></script>
		<script type="text/javascript">
//			判断是否是微信登录
			var wei_o = "{pigcms{$wei_o}";
			if(wei_o=="1"){
				$("#w_login").show();
				$("#fanhui").show();
			}
	...
	$("#wei_login").click(function(){
		var op_id = $("#mt").val();
		var url = "{pigcms{:U('Storestaff/selectlogin')}";
		$.ajax({
			url:url,
			data:{
				"op_id" : op_id
