...
class usercontrol extends base {
	...
	function onsynlogin() {
		...
		if($this->app['synlogin']) {
			if($this->user = $_ENV['users']->get_user_by_uid($uid)) {
				$synstr = '';
...
//			return $status;
//		}
		$uid = $_ENV['users']->add_user($username, $password, $email, 0, $questionid, $answer, $regip);
		return $uid;
	}
...
//			return $status;
//		}
		$status = $_ENV['users']->edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw, $questionid, $answer);
	...
	function onlogin() {
		...
		$answer = $this->input('answer');
		if($isuid == 1) {
			$user = $_ENV['users']->get_user_by_uid($username);
		} elseif($isuid == 2) {
			$user = $_ENV['users']->get_user_by_email($username);
		} else {
			$user = $_ENV['users']->get_user_by_username($username);
		}
		...
		if(empty($user)) {
			...
			$status = -2;
		} elseif($checkques && $user['secques'] != '' && $user['secques'] != $_ENV['users']->quescrypt($questionid, $answer)) {
			$status = -3;
		} else {
			$status = $user['uid'];
		}
		$merge = $status != -1 && !$isuid && $_ENV['users']->check_mergeuser($username) ? 1 : 0;
		return array($status, $user['username'], $password, $user['phone'], $merge);
	}
	...
	function onget_user() {
		...
		$username = $this->input('username');
		if(!$this->input('isuid')) {
			$status = $_ENV['users']->get_user_by_username($username);
		} else {
			$status = $_ENV['users']->get_user_by_uid($username);
		}
		if($status) {
	...
	function ondelete() {
		$this->init_input();
		$uid = $this->input('uid');
		return $_ENV['users']->delete_user($uid);
	}
	...
	function onmerge() {
		...
		$uid = $_ENV['users']->add_user($newusername, $password, $email, $uid);
	...
	function _check_username($username) {
		...
		if(!$_ENV['users']->check_username($username)) {
			return UC_USER_CHECK_USERNAME_FAILED;
		} elseif(!$_ENV['users']->check_usernamecensor($username)) {
			return UC_USER_USERNAME_BADWORD;
		} elseif($_ENV['users']->check_usernameexists($username)) {
	...
	function _check_email($email, $username = '') {
		...
		if(!$_ENV['users']->check_emailformat($email)) {
			return UC_USER_EMAIL_FORMAT_ILLEGAL;
		} elseif(!$_ENV['users']->check_emailaccess($email)) {
			return UC_USER_EMAIL_ACCESS_ILLEGAL;
		} elseif(!$this->settings['doublee'] && $_ENV['users']->check_emailexists($email, $username)) {
