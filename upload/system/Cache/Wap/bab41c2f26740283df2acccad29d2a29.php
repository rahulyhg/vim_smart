...
<body id="index">
		...
		<?php if($follow_list): ?><div style="margin-top:.2rem;">
		    <dl class="list" id="orders">
		    	<dd>
		    		<dl>
		    			<?php if(is_array($follow_list)): $i = 0; $__LIST__ = $follow_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><dd class="dealcard dd-padding" onclick="window.location.href = '<?php echo ($order['url']); ?>';">
					            <div class="dealcard-img imgbox">
					            	<img src="<?php echo ($order["img"]); ?>" style="width:100%;height:100%;"/>
					            </div>
			                    <div class="dealcard-block-right">
			                        <div class="dealcard-brand single-line"><?php echo ($order['name']); ?></div>
			                        <small>粉丝数：<?php echo ($order['fans_count']); ?></small>
			                        <br/>
			                        <small>
			                        </small>
			                        <weak>
		                        		<a class="btn btn-weak color-gray" href="javascript:drop_confirm('你确定要取消关注该商家吗？', '<?php echo U('My/cancel_follow',array('mer_id' => $order['mer_id']));?>');">取消关注</a>
			                        </weak>
			                    </div>
