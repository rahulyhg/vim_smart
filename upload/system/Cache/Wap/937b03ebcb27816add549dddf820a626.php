...
<head>
    <meta charset="utf-8"/>
	<?php if($is_wexin_browser): ?><title><?php echo ($config["group_alias_name"]); ?>列表</title>
	<?php else: ?>
		<title><?php echo ($now_category["cat_name"]); echo ($config["group_alias_name"]); ?>列表_<?php echo ($config["site_name"]); ?></title><?php endif; ?>
    ...
    <link href="<?php echo ($static_path); ?>css/eve.7c92a906.css?1" rel="stylesheet"/>
...
<body id="index">
        <div id="container">
			...
			<div class="nav-bar">
			    <ul class="nav">
		            ...
		            <?php if($all_area_list): ?><li class="dropdown-toggle caret biz subway" data-nav="biz"><span class="nav-head-name"><?php if($now_area): echo ($now_area["area_name"]); else: ?>全城<?php endif; ?></span></li><?php endif; ?>
			                    ...
			                    <ul>
			                        <li class="category-wrapper">
			                            <ul class="dropdown-list">
			                            	<li data-category-id="-1" <?php if(empty($top_category)): ?>class="active"<?php endif; ?>><span>全部分类</span></li>
			                            	<?php if(is_array($all_category_list)): $i = 0; $__LIST__ = $all_category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li data-category-id="<?php echo ($vo["cat_url"]); ?>" <?php if($vo['cat_count'] > 1): ?>data-has-sub="true"<?php else: ?>onclick="list_location($(this));return false;"<?php endif; ?> class="<?php if($vo['cat_count'] > 1): ?>right-arrow-point-right<?php endif; ?> <?php if($top_category['cat_url'] == $vo['cat_url']): ?>active<?php endif; ?>">
			                            			...
			                            			<div class="sub_cat hide" style="display:none;">
			                            				<?php if($vo['cat_count'] > 1): ?><ul class="dropdown-list">
				                            					<li data-category-id="<?php echo ($vo["cat_url"]); ?>" onclick="list_location($(this));return false;"><div><span class="sub-name">全部</span></div></li>
				                            					<?php if(is_array($vo['category_list'])): $j = 0; $__LIST__ = $vo['category_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($j % 2 );++$j;?><li data-category-id="<?php echo ($voo["cat_url"]); ?>" onclick="list_location($(this));return false;"><div><span class="sub-name"><?php echo ($voo["cat_name"]); ?></span></div></li><?php endforeach; endif; else: echo "" ;endif; ?>
			                            					</ul><?php endif; ?>
			                            			</div>
			...
			<div class="deal-container">
				<div class="deals-list" id="deals">
					...
					<?php else: ?>	
						<div class="no-deals">暂无此类<?php echo ($config["group_alias_name"]); ?>，请查看其他分类</div><?php endif; ?>
				</div>
		...
		<script src="<?php echo ($static_public); ?>js/fastclick.js"></script>
		<script>
		$(function(){
		...
		<script>
			var location_url = "<?php echo U('Group/index');?>";
