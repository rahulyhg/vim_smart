...
<ul class="round">
<li><?php if($cardsCount == 0): ?><a href="javascript:void(0);"><span>您还没有领取任何会员卡</span></a><?php else: ?><a href="/wap.php?g=Wap&c=Card&a=index&token=<?php echo ($token); ?>&mycard=1"><span>我的会员卡<em class="ok"><?php echo ($cardsCount); ?></em></span></a><?php endif; ?></li>
</ul>
<ul class="round">
<?php if($allCardsCount != 0): if(is_array($allCards)): $i = 0; $__LIST__ = $allCards;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><li class="dandanb"><a href="/wap.php?g=Wap&c=Card&a=card&token=<?php echo ($token); ?>&cardid=<?php echo ($c["id"]); ?>"><span class="none"><img src="<?php echo ($c["logo"]); ?>"><h2><?php echo ($c["cardname"]); ?></h2>
...
<!--li class="address"><a href="/wap.php?g=Wap&c=Card&a=companyMap&token=<?php echo ($token); ?>&companyid=<?php echo ($thisCompany["mer_id"]); ?>"><span><?php if($thisCompany["address"] != ''): echo ($thisCompany["address"]); else: ?>商家未设置地址<?php endif; ?></span></a></li-->
<li class="detail"><a href="/wap.php?g=Wap&c=Card&a=companyDetail&token=<?php echo ($token); ?>&companyid=<?php echo ($thisCompany["mer_id"]); ?>"><span>查看商家详情</span></a></li>
</ul>
</div>
<script>
