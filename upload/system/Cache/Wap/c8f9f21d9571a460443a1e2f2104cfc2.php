...
<html>
	...
	<body id="index">
        ...
        <div id="container">
			...
			<?php if($activity_list): ?><dl class="list qianggou activity_list">
					<dd>
						<dl>
							<dd class="dd-padding" style="padding:.28rem .2rem .28rem .08rem;">
								<div>
									<strong><a href="<?php echo U('Wapactivity/index');?>">限时活动</a></strong><space></space>
									<strong class="activity-right" style="float:right;color:#ee4339;">
										距结束：<span id="time_d"><?php echo ($time_array['d']); ?></span>天&nbsp;<span id="time_h"><?php echo ($time_array['h']); ?></span>:<span id="time_m"><?php echo ($time_array['m']); ?></span>:<span id="time_s"><?php echo ($time_array['s']); ?></span>
									</strong>
								</div>
							</dd>
							<dd>
								<?php if(is_array($activity_list)): $i = 0; $__LIST__ = $activity_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="qianggoucard">		
										<a href="<?php echo U('Wapactivity/detail',array('id'=>$vo['pigcms_id']));?>">		            	
											<div class="img-container"><img src="<?php echo ($vo["list_pic"]); ?>" alt="<?php echo ($vo["name"]); ?>"/></div>
											<div class="brand"><?php echo ($vo["name"]); ?></div>
											<div class="campaign-price" style="color:#FF658E">已参与 <?php echo ($vo["part_count"]); ?></div>
										</a>
									</div><?php endforeach; endif; else: echo "" ;endif; ?>
							</dd>
						</dl>
							...
							<dd>
								<?php if(is_array($classify_Zcategorys)): $i = 0; $__LIST__ = $classify_Zcategorys;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="classify_f_div <?php if($i == 1): ?>on<?php endif; ?>">
									...
									<ul <?php if($i != 1): ?>style="display:none;"<?php endif; ?>>
										...
										<?php if($vo['subEmptyCount']): $__FOR_START_716013077__=0;$__FOR_END_716013077__=$vo['subEmptyCount'];for($i=$__FOR_START_716013077__;$i < $__FOR_END_716013077__;$i+=1){ ?><li></li><?php } endif; ?>
