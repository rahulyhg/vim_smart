...
<link href="<?php echo ($static_path); ?>userinfo/fans.css" rel="stylesheet" type="text/css"> 
...
onblur="check3(this)" name="phone"  class="px" id="phone" value="<?php echo ($info["phone"]); ?>"  type="text" placeholder="手机号码"></td>
</tr>
</table>
</li><?php endif; ?>
<?php if($fans["id"] == '' and $is_check == '1'): ?><li>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="kuang">
<tr>
...
<td><input name="truename" onfocus="check2(this)" onblur="check4(this)" type="text" class="px" id="truename" value="<?php echo ($info["truename"]); ?>" placeholder="请输入您的真实姓名"></td>
</tr>
</table>
</li><?php endif; ?>
...
<div class="por"><img src="<?php if($info["portrait"] != ''): echo ($info["portrait"]); else: ?>/static/images/portrait.jpg<?php endif; ?>" id="portrait_src" /></div>
<div style="clear:both"></div>
或者选择下面头像
<div style="margin:10px 0 20px 0" id="pors">
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/1.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/2.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/3.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/4.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/5.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/6.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/7.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/8.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/9.jpg" /></div>
<div class="por"><img onclick="selectpor(this)" src="<?php echo ($static_path); ?>images/portrait/10.jpg" /></div>
<div style="clear:both"></div>
</div>
</li>
</ul><?php endif; ?>
...
$('.is_check').click(function(){
	...
	reg=/^0{0,1}(13[0-9]|145|15[0-9]|18[0-9])[0-9]{8}$/i;
	 if(!reg.test(phone)){   
		...
		var num = $('#num').html();
		if(num == ''){
			$.post('wap.php?g=Wap&c=Userinfo&a=get_code&token=<?php echo ($token); ?>&cardid=<?php echo ($cardid); ?>', {phone:phone},
			function(data) {
				if(data.error == 0){
					$('#num').html('60');
...
<?php if($cardid != '' && $conf['truename'] == 0 && $cardInfo == false): else: ?>	
    if (truename == '') {
		alert("请输入您的真实姓名");
        return
    }<?php endif; ?>
...
//         paypass   : paypass,
    ...
    $.post('wap.php?g=Wap&c=Userinfo&a=index&token=<?php echo ($_GET['token']); ?>&cardid=<?php echo ($_GET['cardid']); ?>', submitData,
    function(data) {
        if(data==1){			 
			alert('个人信息保存成功');
			...
			alert('成功领取了会员卡');
			location.href = "<?php echo U('Card/card',array('token'=>$_GET['token'],'cardid'=>$_GET['cardid']));?>";
		}else if(data==3){
			alert('该商家会员卡缺货了');
		}else if(data==4){
			alert('您的积分不够领取改会员卡');
