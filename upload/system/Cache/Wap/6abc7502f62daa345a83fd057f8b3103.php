...
<div class="nav_img">
	...
	<img src="<?php echo ($view_content["img"]); ?>">
</div>
<div class="nav_shngjia" style="display:none;"><span>向<!--武汉大头仔餐饮管理有限公司--><?php echo ($view_content["title"]); ?>付款</span></div>
</div>
<div class="dingdanghao">
	<!--<?php if($view_content["mer_id"] == '' ): ?><div class="weui_cell weui_cell_select weui_select_after" style="text-align:center;" id="shanjia">
		<div class="weui_cell_bd weui_cell_primary" >
				<select class="weui_select" name="merid" style="font-size:14px;color:#464646;" id="youhui" >
					<option value="">请选择商家</option>
					<?php if(is_array($mer)): $i = 0; $__LIST__ = $mer;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["mer_id"]); ?>"><div style="float: right;width: 20px;background:url('<?php echo ($vo["adverimgurl"]); ?>') "></div><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
		</div>
	</div><?php endif; ?>-->
	   
	<form action="<?php echo U('Pay/givejs');?>" method="post" id="submit" onsubmit="return checkForm(this);">
	<div class="dingdang">
		  <?php if($view_content["mer_id"] == '' ): ?><div class="dingdang_create2 " id="shanjia" >
		...
		<div style="padding-bottom:4px;padding: 35px 30px;"><span style="color:#464646;font-size:27px;">金额</span>
			<input type="hidden" name="mer_id" id="mer_id" value="<?php echo ($view_content["mer_id"]); ?>">
		</div>
...
<!--						<?php if(is_array($dataip)): $i = 0; $__LIST__ = $dataip;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>-->
<!--							<option value="<?php echo ($vo["ds_id"]); ?>"><?php echo ($vo["ds_name"]); ?></option>-->
...
<div class="weui_opr_area" >
		        ...
		        <a href="<?php echo U('My/recharge');?>" class="weui_btn weui_btn_default" style="margin-top:10px;">充值</a>
                </p>
            </div>
	</div>
...
<script type="text/javascript">
	...
	$(function(){
		var cou_list = '<?php echo ($data_vip); ?>';
		var cou_html = '';
