...
<body id="index">
		...
		<dl class="list" style="margin-top:0rem;">
		    <dd>
				<ul class="orderindex">
					<li><a href="<?php echo U('My/card_list');?>" class="react <?php if(empty($_GET['use'])): ?>hover<?php endif; ?>">
						<i class="text-icon">⌺</i>
						<span>未使用</span>
					</a>
					</li><li><a href="<?php echo U('My/card_list',array('use'=>1));?>" class="react <?php if($_GET['use'] == '1'): ?>hover<?php endif; ?>">
						<i class="text-icon">⌸</i>
						<span>已使用</span>
					</a>
					</li>
		...
		<?php if(is_array($card_list)): $i = 0; $__LIST__ = $card_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dl class="list <?php if($vo['record_id'] == $_GET['card_id']): ?>active<?php endif; ?>">
		        <dd class="address-wrapper">
		        	<a class="react" href="<?php echo U('Card/coupon',array('token'=>$vo['token'],'cardid'=>$vo['cardid'],'type'=>2,'is_use'=>intval($_GET['use'])));?>">
			            <div class="address-container">
							<div class="kv-line">
			                    <h6>商家：</h6><p><?php echo ($vo["name"]); ?></p>
			                </div>
			                <div class="kv-line">
			                    <h6>标题：</h6><p><?php echo ($vo["title"]); ?></p>
			                </div>
			                <div class="kv-line">
