...
<head>
    <meta charset="utf-8"/>
	<title><?php echo ($config["meal_alias_name"]); ?>订单列表</title>
...
<body id="index">
        ...
        <dl class="list" style="margin-top:0rem;">
		    <dd>
				<ul class="orderindex">
					<li><a href="<?php echo U('My/meal_order_list');?>" class="react <?php if(empty($_GET['status'])): ?>hover<?php endif; ?>">
						<i class="text-icon">⌺</i>
						<span>全部</span>
					</a>
					</li><li><a href="<?php echo U('My/meal_order_list',array('status'=>-1));?>" class="react <?php if($_GET['status'] == '-1'): ?>hover<?php endif; ?>">
						<i class="text-icon">⌸</i>
						<span>待付款</span>
					</a>
					</li><li><a href="<?php echo U('My/meal_order_list',array('status'=>1));?>" class="react <?php if($_GET['status'] == '1'): ?>hover<?php endif; ?>">
					...
					</li><li><a href="<?php echo U('My/meal_order_list',array('status'=>2));?>" class="react <?php if($_GET['status'] == '2'): ?>hover<?php endif; ?>">
						<i class="text-icon">⌹</i>
						<span>待评价</span>
					</a>
					</li>
		...
		<?php if($order_list): ?><div style="margin-top:.2rem;">
		    			...
		    			<?php if(is_array($order_list)): $i = 0; $__LIST__ = $order_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i; if($order['meal_type']): ?><dd class="dealcard dd-padding" onclick="window.location.href = '<?php echo U('Takeout/order_detail', array('mer_id' => $order['mer_id'], 'store_id' => $order['store_id'], 'order_id' => $order['order_id']));?>';">
					        ...
					        <dd class="dealcard dd-padding" onclick="window.location.href = '<?php echo U('Food/order_detail', array('mer_id' => $order['mer_id'], 'store_id' => $order['store_id'], 'order_id' => $order['order_id']));?>';"><?php endif; ?>
			                    ...
			                    <div class="dealcard-block-right">
			                        ...
			                        <br/>
			                        <small>总&nbsp;&nbsp;&nbsp;&nbsp;价：<?php if($order['total_price'] > 0): echo ($order['total_price'] - $order['minus_price']); else: echo ($order['price']); endif; ?> 元&nbsp;&nbsp;数量：<?php echo ($order['total']); ?></small>　<small style="color: green"><?php if($order['meal_type']): ?>外卖<?php else: echo ($config["meal_alias_name"]); endif; ?></small>
			                        <weak>
									<?php if($order['status'] == 3): ?><a class="btn btn-weak color-gray">已取消并退款</a>
			                        	<?php elseif(empty($order['paid'])): ?>
			                        		<?php if($order['meal_type'] == 1): ?><a class="btn btn-weak color-strong" href="<?php echo U('Pay/check',array('order_id' => $order['order_id'], 'type'=>'takeout'));?>">付款</a>
			                        		<?php else: ?>
			                        		<a class="btn btn-weak color-strong" href="<?php echo U('Pay/check',array('order_id' => $order['order_id'], 'type'=>'food'));?>">付款</a><?php endif; ?>
			                        	...
			                        	<?php elseif($order['paid'] == 1 AND empty($order['status'])): ?>
