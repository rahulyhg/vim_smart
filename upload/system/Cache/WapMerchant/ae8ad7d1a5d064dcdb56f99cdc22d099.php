...
<header class="pigcms-header mm-slideout">
	...
	<a href="#slide_menu" id="pigcms-header-left">
		...
		<a  href="<?php echo U('Index/store');?>" id="pigcms-header-right">添加店铺</a>
	...
	<div class="staff-list-wrap">
		...
		<div class="pigcms-container">
			...
			<div class="staff-status-container">
				...
				<p class="staff-status staff-status-cur">
					...
					全部<span>(<?php echo ($count['all']); ?>)</span>
			...
			<div class="staff-status-container">
				...
				<p class="staff-status">
					...
					正常<span>(<?php echo ($count['status1']); ?>)</span>
			...
			<div class="staff-status-container">
				...
				<p class="staff-status">
					...
					待审核<span>(<?php echo ($count['status2']); ?>)</span>
		...
		<div id="staff-list-wrapper" class='pigcms-main'>
			...
			<div id="staff-list-scroller">
				...
				<?php if(is_array($store)): $i = 0; $__LIST__ = $store;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><li class='staff-list-li'>
							...
							<div class='staff-list-info'>
							   ...
							   <div>
								...
								<span class='staff-phone'><?php echo ($vv["name"]); ?></span>

								<span class='staff-type' style='color:#f26f27'><?php if($vv['status'] == 1): ?>正常<?php elseif($vv['status'] == 4): ?>禁用<?php else: ?>审核中<?php endif; ?></span></div>

								<div style="padding:5px;" id="managelist">

								<div><a href="<?php echo U('Index/tablelist',array('store_id'=>$vv['store_id']));?>" style="margin:5px 0;">桌台管理</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:;" onClick="See_ErWM(<?php echo ($vv['store_id']); ?>)">店铺二维码</a>

								</div>

								<div style="margin-top:7px;"><a href="<?php echo U('Index/sortlist',array('store_id'=>$vv['store_id']));?>">分类管理</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('Index/storemeal',array('store_id'=>$vv['store_id']));?>"><?php echo ($config["meal_alias_name"]); ?>信息管理</a></div>
							 ...
							 <div class='staff-list-operation'>
								...
								<a href="<?php echo U('Index/store',array('store_id'=>$vv['store_id']));?>" class='staff-operation-remove' >
							   ...
							   <a href="<?php echo U('Index/table_add',array('store_id'=>$vv['store_id']));?>" class='staff-operation-remove' >
							   ...
							   <a href="<?php echo U('Index/sort_add',array('store_id'=>$vv['store_id']));?>" class='staff-operation-remove' >
...
var ewmUrl="<?php echo U('Index/erwm');?>";
...
$.post(ewmUrl,{sid:id,type:'meal'},function(ret){
	  ...
	  if(!ret.error_code && ret.qrcode){
		 ...
		 layer.open({title:['店铺二维码：','background-color:#fb4746;color:#fff;'],content:'<div ><img src="'+ret.qrcode+'" style="width:'+imgw+'px"></div>',btn: ['确定'],end:function(){}});
		 ...
		 layer.open({title:['错误提示：','background-color:#fb4746;color:#fff;'],content:'二维码获取失败！',btn: ['确定'],end:function(){}});
