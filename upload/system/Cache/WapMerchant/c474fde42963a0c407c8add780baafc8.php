...
<body>
	<header class="pigcms-header mm-slideout">
		...
		<a  href="<?php echo U('Index/hardware_add');?>" id="pigcms-header-right">添加设备</a>
	</header>
		...
		<div id="staff-list-scroller">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li class="staff-list-li">
					<p class="staff-list-info">
						<span class="staff-name" style='width:2６px;'><span style='color:red'>设备(<?php echo ($i); ?>)</span></span>
						<span class="staff-type">绑定账号：<?php echo ($list["username"]); ?></span><br/><br/>
						<a href="tel:<?php echo ($list["mp"]); ?>" class="staff-phone">手机号：<?php echo ($list["mp"]); ?></a>&nbsp;&nbsp;<span class="staff-type">终端号：<?php echo ($list["mcode"]); ?></span>
					</p>
					<p class="staff-list-operation">
					  <a href="<?php echo U('Index/hardware_add',array('id'=>$list['pigcms_id']));?>" class="staff-operation-remove">
							<i class="iconfont icon-edit"></i>
							<span> 编辑</span>
						</a>
						<br/>
						<br/>
						<a href="<?php echo U('Index/hardware_dell',array('id'=>$list['pigcms_id']));?>" class="staff-operation-remove" onclick="delcfm()">
