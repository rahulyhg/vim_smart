 ...
 var  meal_alias_name = "<?php echo ($config["meal_alias_name"]); ?>";
	...
	<form id="myform" method="post" action="<?php echo U('Merchant/store_amend');?>" frame="true" refresh="true">
		...
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			...
			<?php if($config['store_open_waimai']): ?><tr>
					<th width="80"><?php echo ($config["waimai_alias_name"]); ?>功能</th>
					<td>
						<span class="cb-enable"><label class="cb-enable <?php if($store['have_waimai'] == 1): ?>selected<?php endif; ?>"><span>开启</span><input type="radio" name="have_waimai" value="1" <?php if($store['have_waimai'] == 1): ?>checked="checked"<?php endif; ?> /></label></span>
						<span class="cb-disable"><label class="cb-disable <?php if($store['have_waimai'] == 0): ?>selected<?php endif; ?>"><span>关闭</span><input type="radio" name="have_waimai" value="0" <?php if($store['have_waimai'] == 0): ?>checked="checked"<?php endif; ?>/></label></span>
					</td>
				</tr><?php endif; ?>
			<tr>
				<th width="80">店铺状态</th>
