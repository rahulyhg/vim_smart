...
<body style="background:#fff;padding:20px 20px;">
	...
	<ul class="modules">
		...
		<li>
			...
			<div class="mright">
				<?php if($m['sub']): ?><a href="<?php if (!$m['linkurl']){ echo '?g=System&c=LinkPC&a=' . $m['module'] . '&iskeyword=' . intval($_GET['iskeyword']); }else{ echo $m['linkurl'];}?>">详细</a><?php endif; ?>
				<?php if($m['canselected']): ?><a href="javascript:;" onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])){ echo $m['linkcode']; }else{ echo $m['keyword']; }?>')" style="margin-left:14px;">选中</a><?php endif; ?>
			</div>
