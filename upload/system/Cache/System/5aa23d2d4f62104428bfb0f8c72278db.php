...
<html xmlns="http://www.w3.org/1999/xhtml">
 ...
 var  meal_alias_name = "<?php echo ($config["meal_alias_name"]); ?>";
	...
	<form id="myform" method="post" action="<?php echo U('Classify/cat_amend');?>" enctype="multipart/form-data">
		<input type="hidden" name="cid" value="<?php echo ($now_category['cid']); ?>"/>
		<table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
			<tr>
				<th width="80">分类名称</th>
				<td><input type="text" class="input fl" name="cat_name" id="cat_name" onBlur="TextSEO(this.value)" value="<?php echo ($now_category["cat_name"]); ?>" size="25" placeholder="" validate="maxlength:20,required:true" tips=""/></td>
			</tr>
			<tr>
			...
			<?php if(!empty($now_category['cat_pic'])): ?><tr>
					<th width="80">分类现图</th>
			...
			<?php if($now_category['subdir'] != 3): ?><tr>
				<th width="80">分类排序</th>
			...
			<?php if($now_category['subdir'] == 2): ?><tr>
				<th width="80">允许发布时上传图片</th>
				<td>
					<span class="cb-enable"><label class="cb-enable <?php if($now_category['isupimg'] == 1): ?>selected<?php endif; ?>"><span>允许</span><input type="radio" name="isupimg" value="1" <?php if($now_category['isupimg'] == 1): ?>checked="checked"<?php endif; ?> /></label></span>
					<span class="cb-disable"><label class="cb-disable <?php if($now_category['isupimg'] == 0): ?>selected<?php endif; ?>"><span>不允许</span><input type="radio" name="isupimg" value="0" <?php if($now_category['isupimg'] == 0): ?>checked="checked"<?php endif; ?>/></label></span>
				</td>
			</tr><?php endif; ?>
		<tr><th width="80">SEO标题：</th>
		<td><input type="text" tips="一般不超过80个字符！" validate="" style="width:260px;"  id="seo_title" name="seo_title" class="input fl" value="<?php echo ($now_category["seo_title"]); ?>"></td></tr>
		<tr><th width="80">SEO关键词：</th><td><input type="text" tips="一般不超过100个字符！" validate="" style="width:260px;"  id="seo_keywords" name="seo_keywords" class="input fl" value="<?php echo ($now_category["seo_keywords"]); ?>"></td></tr>
		<tr><th width="80">SEO描述：</th><td><textarea tips="一般不超过200个字符！" validate="" id="seo_description" name="seo_description" style="width:250px;height:90px;"><?php echo ($now_category["seo_description"]); ?></textarea></td></tr>
		</table>
