	...
	<body width="100%" <?php if($bg_color): ?>style="background:<?php echo ($bg_color); ?>;"<?php endif; ?>>
		<div class="mainbox">
			<div id="nav" class="mainnav_title">
				<ul>
					<?php if(empty($now_category)): ?><a href="<?php echo U('Appoint/index');?>" class="on">分类列表</a>|
						<a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Appoint/cat_add',array('cat_fid'=>intval($_GET['cat_fid'])));?>','添加主分类',480,260,true,false,false,addbtn,'add',true);">添加主分类</a>
					<?php else: ?>
						<a href="<?php echo U('Appoint/index');?>">分类列表</a>|
						<a href="<?php echo U('Appoint/index',array('cat_fid'=>$_GET['cat_fid']));?>" class="on"><?php echo ($now_category["cat_name"]); ?> - 子分类列表</a>|
						<a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Appoint/cat_add',array('cat_fid'=>intval($_GET['cat_fid'])));?>','添加子分类',520,370,true,false,false,addbtn,'add',true);">添加子分类</a><?php endif; ?>
				</ul>
			</div>
					...
					<table width="100%" cellspacing="0">
						...
						<tbody>
							<?php if(is_array($category_list)): if(is_array($category_list)): $i = 0; $__LIST__ = $category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<?php if(empty($_GET['cat_fid'])): ?><td><a href="<?php echo U('Appoint/index',array('cat_fid'=>$vo['cat_id']));?>">查看子分类</a></td><?php endif; ?>
										<td><a href="javascript:void(0);" onclick="window.top.artiframe('<?php echo U('Appoint/cue_field',array('cat_id'=>$vo['cat_id']));?>','预约表单填写项',580,420,true,false,false,false,'detail',true);">预约表单填写项</a></td>
