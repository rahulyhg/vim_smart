...
<form id="myform" method="post" action="<?php echo U('House/savemenu');?>" frame="true" refresh="true">
    ...
    <table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
        <?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rowset): $mod = ($i % 2 );++$i;?><tr>
                <th width="20%">
                    <label><input type="checkbox" class="menu_<?php echo ($rowset['id']); ?> father_menu" value="<?php echo ($rowset['id']); ?>" name="menus[]" <?php if(in_array($rowset['id'], $role['menus'])): ?>checked<?php endif; ?>/>&nbsp;<?php echo ($rowset['name']); ?></label>
                </th>
                <td>
                    <?php if(is_array($rowset['lists'])): $i = 0; $__LIST__ = $rowset['lists'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><label><input type="checkbox" class="child_menu_<?php echo ($row['fid']); ?> child_menu" value="<?php echo ($row['id']); ?>"  name="menus[]" data-fid="<?php echo ($row['fid']); ?>"  <?php if(in_array($row['id'], $role['menus'])): ?>checked<?php endif; ?> />&nbsp;<?php echo ($row['name']); ?></label>　<?php endforeach; endif; else: echo "" ;endif; ?>
                </td>
