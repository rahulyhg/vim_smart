...
<link rel="stylesheet" type="text/css" href="<?php echo ($static_path); ?>css/style_2_common.css" />
<link href="<?php echo ($static_path); ?>css/style.css" rel="stylesheet" type="text/css" />
<link rel="stylesheet" type="text/css" href="<?php echo ($static_path); ?>css/cymain.css" />

<script src="<?php echo ($static_path); ?>js/common.js" type="text/javascript"></script>
<script src="<?php echo ($static_path); ?>js/jquery.min.js" type="text/javascript"></script>
...
<body style="background:#fff;padding:20px 20px;">
	...
	<form id="selectimg">
		<table class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>
				<td><?php echo ($m['pigcms_id']); ?></td>
				<td><img src="<?php echo ($m['pic']); ?>" style="width:70px;"/><br/></td>
				<td><input type="text" name="img_remark"  value="<?php echo ($m['img_remark']); ?>" style="border:1px solid #999;"></td>
				<td class="norightborder"><input type="checkbox" name="pic" value="<?php echo ($m["pic"]); ?>">选中|<a href="javascript:void(0)" onclick="deleteImg(this)">删除</a></td>
	...
	$(document).ready(function() {
		$("input[name='img_remark']").change(function(event) {
			$.post('<?php echo U('ImageSelect/img_option');?>', {id:$(this).parents().prev().prev().html(),text: $(this).val()}, function(data, textStatus, xhr) {});
		});
	});		
	function fun(){
		// 返回数据到主页面
		...
		if(domid=='upload_pic_ul'){
			...
			if(img_count+ids>5){
				...
				$.post('<?php echo U('ImageSelect/check_thumb_exist');?>', {image_path:image_path,type:type}, function(data, textStatus, xhr) {setTimeout("art.dialog.close()", 100 )});
		...
		}else if(domid=='gallery'){
			$.post('<?php echo U('Frontmanag/insert_gallery');?>', {cyid:dom.name,image_path:image_path}, function(data, textStatus, xhr) {setTimeout("art.dialog.close()", 100 )});
		}else if(domid == 'bgs'){
			if(ids>1){
				alert('只能选一个');
				$("input[name='pic']:checkbox").attr('checked','false');
	...
	function deleteImg(obj){
		if(confirm('确定要删除图片吗？')){
			$.post('<?php echo U('ImageSelect/img_option');?>', {id:$(obj).parents().prev().prev().prev().html(),op: 'del'}, function(data, textStatus, xhr) { window.location.reload();});
		}
	}
