...
<!--<div class="page-container">-->
    ...
    <style type="text/css">
        ...
        <div class="btn-group" style="margin-top:10px;">
            <a href="<?php echo U('watch_company_one');?>">
    ...
    <table class="table table-striped table-bordered table-hover" id="sample_1" >
        ...
        <tbody>
        <?php if(is_array($predict_list)): $i = 0; $__LIST__ = $predict_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><td>
                <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                    <input type="checkbox" class="checkboxes" value="<?php echo ($vo["predict_id"]); ?>" />
                    <span></span>
                </label>
            </td>
            <td><div class="shopNameDiv"><?php echo ($vo["year"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["village_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["company_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["admin_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo date('Y-m-d H:i:s',$vo['updatetime']);?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["check_admin_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo $vo['checktime']?date('Y-m-d H:i:s',$vo['checktime']):'';?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["status_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["remark"]); ?></div></td>
            <td>
                <?php if($vo['is_apply'] == 1): ?><span style="color: red">已应用</span>
                    <?php else: ?>
                    <span style="color: green">未应用</span><?php endif; ?>
            </td>
            <td class="button-column noExl">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-left" role="menu" style="position:relative; margin-left:-90px;">
                        <li>
                            <a href="<?php echo U('Budget_predict/watch_predict_one',array('id'=>$vo['predict_id']));?>" >
                                <i class="icon-tag"></i> 查看详情 </a>
                        </li>
                        <?php if(!$isLiu): ?><li>
                            <a href="<?php echo U('Budget_predict/output_excel_one',array('id'=>$vo['predict_id']));?>" >
                                <i class="icon-tag"></i> 表格导出 </a>
                        </li><?php endif; ?>
                        <?php if(in_array($vo['status'],$action_now['status'])): ?><li>
                            <a data-toggle="modal" data-target="#common_modal" href="<?php echo U('Budget_predict/check_predict_one',array('id'=>$vo['predict_id']));?>">
                                <i class="icon-tag"></i> 审核条目/快捷审核
                            </a>
                            </li><?php endif; ?>
                            <li onclick="apply_predict_one(<?php echo ($vo['predict_id']); ?>)">
...
<!--/底部文件-->
    ...
    <script>
        ...
        $('#predict_status').val('<?php echo ($predict_status); ?>');
        ...
        function apply_predict_one(id) {
            swal({
                ...
                if(iscom){
                    ...
                    window.location.href='<?php echo U("Budget_predict/apply_predict_one");?>&id='+id;
                }else{
                    swal.close();
                }
