...
<!--头部设置-->
<?php
$title = array( 'title'=>'在线预约', 'describe'=>'', ); $breadcrumb = array( array('物业服务','#'), array('在线预约','#'), ); ?>
<!--头部设置结束-->
...
<!--                <li>-->
<!--                    <a href="<?php echo U('',array('village_id'=>$vo['village_id']));?>">-->
...
<!--            //编号	排序	商品名称	价格	单位	销售量	最后操作时间	状态	操作-->
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr class="<?php if($i%2 == 0): ?>odd<?php else: ?>even<?php endif; ?>">
            <td>
                <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
            ...
            <td><div class="tagDiv"><?php echo ($row["pigcms_id"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($row["name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($row["contact"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($row["village_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($row["meal_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($row["price"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($row["unit"]); ?></div></td>
            <td>
                <div class="tagDiv">
                    <?php if($row['appointment_start_time'] > 0): echo (date("Y-m-d H:i",$row["appointment_start_time"])); endif; ?>
                </div>
            </td>
            <td>
                <div class="tagDiv">
                    <?php if($row['appointment_end_time'] > 0): echo (date("Y-m-d H:i",$row["appointment_end_time"])); endif; ?>
                </div>
            </td>
            <td>
                <div class="tagDiv"><?php echo (date("Y-m-d H:i",$row["time"])); ?></div>
            </td>
            <td>
                <div class="tagDiv">
                    <?php if($row['is_read'] == 0): ?><span style="color: red">未处理</span>
                    <?php else: ?>
                        <span style="color: green">已处理</span><?php endif; ?>
                </div>
            </td>
<!--            <td class="button-column">-->
<!--                <a href="<?php echo U('appointment_info',array('pigcms_id'=>$row['pigcms_id']));?>"  style="width: 60px;" class="label label-sm label-info handle_btn show_info" title="查看详情">查看详情</a>-->
<!--                <?php if($row['is_read'] == 0): ?>-->
<!--                    <a style="width:100px;"  pigcms_id="<?php echo ($row["pigcms_id"]); ?>" class="label label-sm label-info" title="已处理" href="javascript:;" onclick="read(this)" bindid='<?php echo ($vo["bind_id"]); ?>' pid="<?php echo ($vo["pid"]); ?>">标记为已处理</a>-->
<!--<?php endif; ?>-->
<!--                <a class="del label label-sm label-info handle_btn" href="<?php echo U('appointment_del',array('pigcms_id'=>$row['pigcms_id']));?>"  style="width: 60px;" class="label label-sm label-info handle_btn" title="删除">删除</a>-->
<!--            </td>-->
            <td class="button-column">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-left" role="menu" style="position: absolute;">
                        <li>
                            <a href="<?php echo U('appointment_info',array('pigcms_id'=>$row['pigcms_id']));?>"  class="label-sm handle_btn show_info" title="查看详情">
                                <i class="icon-docs"></i>查看详情
                            </a>
                        </li>
                        <?php if($row['is_read'] == 0): ?><li>
                            <a pigcms_id="<?php echo ($row["pigcms_id"]); ?>" title="已处理" href="javascript:;" onclick="read(this)" bindid='<?php echo ($vo["bind_id"]); ?>' pid="<?php echo ($vo["pid"]); ?>"> <i class="icon-docs"></i>标记为已处理</a>
                        </li><?php endif; ?>
                        <li>
                            <a class="del" href="<?php echo U('appointment_del',array('pigcms_id'=>$row['pigcms_id']));?>" class="label label-sm label-info handle_btn" title="删除"> <i class="icon-docs"></i>删除</a>
                        </li>
                    </ul>
                </div>

                <!--<a style="width: 60px;" class="label label-sm label-info" title="编辑" href="<?php echo U('edit',array('pigcms_id'=>$vo['pigcms_id'],'usernum'=>$vo['usernum']));?>">编辑</a>
                <a style="width: 60px;" class="label label-sm label-info" title="缴费明细" href="<?php echo U('order_history',array('bind_id'=>$vo['pigcms_id']));?>">缴费明细</a>
                <a style="width: 60px;" class="label label-sm label-info handle_btn" title="抄表二维码"
                   href="http://www.hdhsmart.com/shequ.php?g=House&c=Index&a=re_setmeter_qr&usernum=<?php echo ($vo['usernum']); ?>"
                   id="<?php echo ($vo["pigcms_id"]); ?>"
    ...
    <?php if($repair_list['totalPage'] > 1): echo ($repair_list["pagebar"]); endif; ?>
</div>
<!--业务区结束-->
...
<script>
    ...
    function read(obj){
        ...
        if(confirm('您确定要标记为已处理？')){
            $.post("<?php echo U('is_read');?>",{pigcms_id:pigcms_id},function(result){
