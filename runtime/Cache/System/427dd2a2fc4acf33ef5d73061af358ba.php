...
<!--<div class="page-container">-->
    ...
    <!--<div class="btn-group">
        <a href="<?php echo U('Budget/add_type');?>">
                ...
                <span>
                    ...
                    <div class="btn-group">
                        <select id="datetimepicker"  class="form-control" placeholder="" name="startDate" v-model="year" onchange="change_url('year',this.options[this.options.selectedIndex].value)">
                            <?php $__FOR_START_14695__=2017;$__FOR_END_14695__=date('Y')+2;for($i=$__FOR_START_14695__;$i < $__FOR_END_14695__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?></option><?php } ?>
                        </select>
                    </div>
                </span>
            </span>
    ...
    <div class="tabbable-custom nav-justified">
        <table class="table table-striped table-bordered table-hover" id="sample_1" >
            <thead>
            <tr>
                <td colspan="7" align="center" style="font-size: 25px"><?php echo ($year); ?>年<?php echo ($title1); ?>收支预算对比计算表</td>
            </tr>
            <tr>
                <th width="5%">序号</th>
            ...
            <tbody>
            <?php if(is_array($money_list)): $ke = 0; $__LIST__ = $money_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($ke % 2 );++$ke;?><tr>
                    <td rowspan="<?php echo ($vo["count"]); ?>" style="text-align:center;vertical-align:middle;"><?php echo ($ke); ?></td>
                    <td rowspan="<?php echo ($vo["count"]); ?>" style="text-align:center;vertical-align:middle;"><?php echo ($vo['type_name']); ?></td>
                    <?php if(is_array($vo['children'])): $k = 0; $__LIST__ = $vo['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($k % 2 );++$k; if($k == 1): else: ?>
                <tr><?php endif; ?>
                    <td rowspan="<?php echo ($vo1["count"]); ?>" style="text-align:center;vertical-align:middle;"><div class="tagDiv"><?php echo ($vo1['type_name']); ?></div></td>
                    <?php if(is_array($vo1['children'])): $k2 = 0; $__LIST__ = $vo1['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($k2 % 2 );++$k2; if($k2 == 1): else: ?>
                <tr><?php endif; ?>
                    <td style="text-align:center;vertical-align:middle;"><div class="tagDiv"><?php echo ($vo2['type_name']); ?></div></td>
                    <td><div ondblclick="open_url(<?php echo ($vo2['type_id']); ?>)" title="双击修改详细"   style="width: 100%;"><div class="tagDiv"><?php echo number_format($vo2['data']['money_sum'],2);?></div></div></td>
                    <td><div ondblclick="open_url(<?php echo ($vo2['type_id']); ?>)" title="双击修改详细"   style="width: 100%;"><div class="tagDiv"><?php echo number_format($vo2['data']['sum'],2);?></div></div></td>
                    <td><div ondblclick="open_url(<?php echo ($vo2['type_id']); ?>)" title="双击修改详细"   style="width: 100%;"><div class="tagDiv"><?php echo number_format(($vo2['data']['money_sum']-$vo2['data']['sum']),2);?></div></div></td><?php endforeach; endif; else: echo "" ;endif; ?>
...
<!--/底部文件-->
    ...
    <script>
        ...
        new Vue({
            el:"#filter",
            data:{
                list:<?php echo ($project_list); ?>,
                    company_id:<?php echo ($company_id); ?>,
                        project_id_change:'<?php echo ($project_id_change); ?>',
                        year:'<?php echo ($year); ?>',
                    },
                    computed: {
        ...
        function change_url(type,val) {
            console.log(val);
            window.location.href='<?php echo U('Budget/check_money_list_change');?>&'+type+'='+val;
        }
        function open_url(type_id) {
            var url='<?php echo U('Budget/ajax_change_money',array('project_id_change'=>$project_id_change,'year'=>$year));?>';
