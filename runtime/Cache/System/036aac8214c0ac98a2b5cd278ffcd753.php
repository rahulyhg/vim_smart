...
<!--头部设置-->
<?php
$title = array( 'title'=>'楼层管理', 'describe'=>'', ); $breadcrumb = array( array('物业服务','#'), array('巡检管理','#'), ); ?>
<!--头部设置结束-->
...
<div class="table-toolbar">
    <div class="btn-group">
        <div class="col-md-6">
            <div class="btn-group">
                <a href="<?php echo U('PropertyService/point_safety_add');?>">
                    <button id="sample_editable_1_new" class="btn sbold green">添加消防点
                        <i class="fa fa-plus"></i>
                    </button>
                </a>
            </div>
    ...
    <div class="btn-group">
        <?php if($_GET['is_del'] == 0): ?><a href="<?php echo U('',array('is_del'=>1));?>">
                <button id="sample_editable_1_new" class="btn sbold green">查看已停用巡检点
                </button>
            </a>
        <?php else: ?>
...
<div id="shopList" class="grid-view">
    <table class="table table-striped table-bordered table-hover" id="sample_1" >
        ...
        <tbody>
        <?php if(is_array($pointArray)): $i = 0; $__LIST__ = $pointArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="<?php if($i%2 == 0): ?>odd<?php else: ?>even<?php endif; ?>">
                <td><div class="tagDiv"><?php echo ($vo["id"]); ?></div></td>
                <td><div class="tagDiv"><?php echo ($vo["room_name"]); ?></div></td>
            <td><div class="tagDiv"><?php if($vo['name'] != ''): echo ($vo["name"]); else: echo ($vo["orientation"]); endif; ?></div></td>
                <td><div class="shopNameDiv"><?php echo ($vo["village_name"]); ?></div></td>
                <td><div class="tagDiv"><?php echo ($vo["desc"]); ?></div></td>
                <?php if($vo['is_del'] == 0): ?><td>
                    <button class="btn btn-sm green btn-outline filter-submit margin-bottom" id="point_type_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["is_del"]); ?>" onclick="changeType(this)">启用</button>
                    <input type="hidden" id="point_type_<?php echo ($vo["id"]); ?>_id" value="<?php echo ($vo['id']); ?>">                  
                    </td>
                <?php else: ?>
                    <td>
                    <button class="btn btn-sm red btn-outline filter-submit margin-bottom" id="point_type_<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["is_del"]); ?>" onclick="changeType(this)">停用</button>
                    <input type="hidden" id="point_type_<?php echo ($vo["id"]); ?>_id" value="<?php echo ($vo['id']); ?>">
                    </td><?php endif; ?>
                ...
                <ul class="dropdown-menu pull-left" role="menu" style="position:relative;">
                    <li>
                        <a href="<?php echo U('point_safety_update',array('id'=>$vo['id']));?>">
                            <i class="icon-tag"></i> 编辑 </a>
                    </li>
                    <li>
                        <a href="<?php echo U('qrcode_safety_point',array('rid'=>$vo['rid']));?>"
                           data-toggle="modal"
                           data-target="#modal_<?php echo ($vo["rid"]); ?>">
                            <i class="icon-tag"></i> 巡检二维码 </a>
                    </li>
                </ul>
                </div>
                </td>
            ...
            <!--弹出层容器-->
            <div class="modal fade" tabindex="-1" role="dialog" id="modal_<?php echo ($vo["rid"]); ?>">
