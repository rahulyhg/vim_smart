                    ...
                    <div class="portlet-body">
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="<?php echo U('Index/system_menu_add_news');?>">
                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <tbody>
                            <?php if(is_array($menu_array)): foreach($menu_array as $key=>$vo): ?><tr class="odd gradeX">
                                    <td>
                                        <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                    ...
                                    <td>
                                            <input type="number" style="width: 75px" class="form-control" id="<?php echo ($vo["id"]); ?>" onblur="sort_fid_menu(this)" value="<?php echo ($vo["sort"]); ?>" <?php if($vo['fid'] != 0 or $vo['auth_area'] != 0): ?>readonly = "readonly"<?php endif; ?>/>
                                            <span></span>
                                    </td>
                                    <td><?php echo ($vo["name"]); ?></td>
                                    <td><?php echo ($vo["fid_name"]); ?></td>
                                    <td><?php echo ($vo["module"]); ?></td>
                                    <td><?php echo ($vo["controller"]); ?></td>
                                    <td><?php echo ($vo["action"]); ?></td>
                                    <!--<?php if($vo['fid'] == 0): ?><td colspan="3" style="color: darkred">顶级菜单没有此类型数据</td>
                                        <?php else: ?>
                                        <td><?php echo ($vo["module"]); ?></td>
                                        <td><?php echo ($vo["controller"]); ?></td>
                                        <td><?php echo ($vo["action"]); ?></td><?php endif; ?>-->
                                    <?php if($vo["is_show"] == 1): ?><td style="color: green;cursor:pointer" onclick="hidden_all_choose_menu(this)" id="<?php echo ($vo["id"]); ?>">显示</td>
                                    <?php else: ?>
                                    <td style="color: red;cursor:pointer" id="<?php echo ($vo["id"]); ?>" onclick="show_all_choose_menu(this)">未显示</td><?php endif; ?>
                                    <?php if($vo["auth_type"] == 0): ?><td style="color: blue">菜单权限</td>
                                        <?php elseif($vo["auth_type"] == 1): ?>
                                        <td style="color: yellow">逻辑权限</td>
                                        <?php elseif($vo["auth_type"] == 2): ?>
                                        <td style="color: red">显示权限</td>
                                        <?php elseif($vo["auth_type"] == 3): ?>
                                        <td style="color: orange">外部链接</td>
                                        <?php elseif($vo["auth_type"] == 4): ?>
                                        <td style="color: purple">模块显示</td>
                                        <?php elseif($vo["auth_type"] == 5): ?>
                                        <td style="color: purple">后台管理</td><?php endif; ?>
                                    <?php if($vo["auth_area"] == 0): ?><td style="color: #DB78A1">后台权限</td>
                                        <?php elseif($vo["auth_area"] == 1): ?>
                                        <td style="color: yellowgreen">Wap端权限</td>
                                        <?php elseif($vo["auth_area"] == 2): ?>
                                        <td style="color: #ab1e1e">项目权限</td><?php endif; ?>
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu pull-left" role="menu" style="position:relative;">
                                                <li>
                                                    <a href="<?php echo U('system_menu_edit_news',array('id'=>$vo['id']));?>">
                                                        <i class="icon-tag"></i> 更新 </a>
                                                </li>
                                                <li>
                                                    <a href="<?php echo U('system_menu_deleted_news',array('id'=>$vo['id']));?>">
...
<script type="text/javascript">
    ...
    function delete_pr_info(obj){
        layer.msg('你确定要删除吗？', {
            ...
            ,yes: function(index){
                ...
                //通过ajax异步删除
                $.ajax({
                    url:"<?php echo U('Index/log_del_new');?>",
    ...
    function hidden_all_choose_menu(obj){
        ...
        $.ajax({
            url:"<?php echo U(change_show_hide);?>",
            type:'post',
