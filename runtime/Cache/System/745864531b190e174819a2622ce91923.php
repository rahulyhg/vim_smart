...
<!--头部设置-->
<?php
$title = array( 'title'=>'楼层管理', 'describe'=>'', ); $breadcrumb = array( array('物业服务','#'), array('巡更管理','#'), ); ?>
<!--头部设置结束-->
...
<div class="table-toolbar">
    <div class="btn-group">
        <div class="col-md-6">
            <div class="btn-group">
                <a href="<?php echo U('PropertyService/point_add');?>">
        ...
        <?php if(is_array($pointArray)): $i = 0; $__LIST__ = $pointArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="<?php if($i%2 == 0): ?>odd<?php else: ?>even<?php endif; ?>">
            ...
            <td><div class="tagDiv"><?php if($vo['name'] != ''): echo ($vo["name"]); else: echo ($vo["orientation"]); endif; ?></div></td>
                ...
                <ul class="dropdown-menu pull-left" role="menu" style="position:relative;">
                    <li>
                        <a href="<?php echo U('point_update',array('id'=>$vo['id']));?>">
                            <i class="icon-tag"></i> 编辑 </a>
                    </li>
                    <li>
                        <a href="<?php echo U('qrcode_point',array('rid'=>$vo['rid']));?>"
                           data-toggle="modal"
...
<script>
    ...
    //点位的状态：开启与关闭
    // $("#point_type_<?php echo ($vo["id"]); ?>").click(function(){
    //     var is_del = $("#point_type_<?php echo ($vo["id"]); ?>").val();
    //     var point_id = $("#point_id_<?php echo ($vo["id"]); ?>").val();
    //     console.log(is_del);
    //     console.log(point_id);
    //     // $.ajax({
    //     //      url:'<?php echo U("point_type");?>',
