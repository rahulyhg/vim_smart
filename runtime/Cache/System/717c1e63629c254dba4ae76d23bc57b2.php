...
<!--头部设置-->
<?php
$title = array( 'title'=>'投诉建议', 'describe'=>'', ); $breadcrumb = array( array('物业服务','#'), array('投诉建议','#'), ); ?>
<!--头部设置结束-->
...
<div id="shopList" class="grid-view">
    <table class="table table-striped table-bordered table-hover" id="sample_1">
        ...
        <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="<?php if($i%2 == 0): ?>odd<?php else: ?>even<?php endif; ?>">
        <td><div class="tagDiv"><?php echo ($vo["pigcms_id"]); ?></div></td>
        <td><div class="tagDiv"><?php echo ($vo["name"]); ?></div></td>
        <td><div class="tagDiv"><?php echo ($vo["content"]); ?></div></td>
        <td><div class="shopNameDiv"><?php echo (date('Y-m-d H:i:s',$vo["time"])); ?></div></td>
<!--        <td class="button-column">-->
<!--            <?php if($vo['is_read'] == 0): ?>-->
<!--                <a style="width:100px;" class="label label-sm label-info" title="已处理" href="javascript:;" onclick="read(this)" bindid='<?php echo ($vo["bind_id"]); ?>' pid="<?php echo ($vo['pigcms_id']); ?>">标记为已处理</a>-->
<!--<?php endif; ?>-->
<!--            <a style="width: 60px;" class="label label-sm label-info handle_btn" title="查看详情" href="<?php echo U('appointment_info',array('pigcms_id'=>$vo['pigcms_id']));?>">查看详情</a>-->
<!--        </td>-->
                <td>
                    <?php if($vo['is_read']): ?><span class="text-info">已处理</span>
                        <?php else: ?>
                        <span style="color:orangered">未处理</span><?php endif; ?>
                </td>
        <td class="button-column">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu pull-left" role="menu" style="position: absolute;">
                    <li>
                        <a class="handle_btn" title="查看详情" href="<?php echo U('appointment_info',array('pigcms_id'=>$vo['pigcms_id']));?>">
                            <i class="icon-docs"></i>查看详情
                        </a>
                    </li>
                    <?php if($vo['is_read'] == 0): ?><li>
                            <a  title="已处理" href="javascript:;" onclick="read(this)" bindid='<?php echo ($vo["bind_id"]); ?>' pid="<?php echo ($vo['pigcms_id']); ?>">
                                <i class="icon-docs"></i>标记为已处理
                            </a>
                        </li><?php endif; ?>
                    <li>
                        <a class="del" href="<?php echo U('appointment_del',array('pigcms_id'=>$vo['pigcms_id']));?>" title="删除">
