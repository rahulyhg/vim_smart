...
<!--<div class="page-container">-->
    ...
    <div class="btn-group">
        <a href="<?php echo U('update_consume');?>">
    ...
    <div class="btn-group">
        <a href="<?php echo U('meter_config_list');?>">
    ...
    <div class="btn-group">
        <a href="<?php echo U('meters_record_list');?>">
            <button id="sample_editable_1_new" class="btn sbold green">工程设备列表
            </button>
        </a>
    </div>
...
<!--<?php endif; ?>-->
    ...
    <table class="table table-striped table-bordered table-hover" id="sample_1">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($row["meter_floor"]); ?></td>
                <td><?php echo ($row["meter_type_name"]); ?></td>
                <td><?php echo ($row["price_type_name"]); ?></td>
                <?php if($row['is_record'] == 1): ?><td><?php echo ($row["last_total_consume"]); ?></td>
                    <td><?php echo ($row["total_consume"]); ?></td>
                    <?php else: ?>
                    <td><?php echo ($row["end_num"]); ?></td>
                    <td>未抄录</td><?php endif; ?>

                <td><?php echo ($row["consume"]); ?></td>
                <td>
                    <?php if($_GET['is_del'] == 0): ?><button class="btn btn-sm green btn-outline filter-submit margin-bottom" id="meter_type_<?php echo ($row["id"]); ?>" value="<?php echo ($row["is_del"]); ?>" onclick="changeType(this)">启用</button>
                        <input type="hidden" id="meter_type_<?php echo ($row["id"]); ?>_id" value="<?php echo ($row['id']); ?>">
                        <!-- <a  href="<?php echo U('meter_logic_del',array('meter_hash'=>$row['meter_hash']));?>">
                        <button class="btn btn-sm green btn-outline filter-submit margin-bottom">启用</button>
                        </a> -->
                    <?php else: ?>
                        <button class="btn btn-sm red btn-outline filter-submit margin-bottom" id="meter_type_<?php echo ($row["id"]); ?>" value="<?php echo ($row["is_del"]); ?>" onclick="changeType(this)">停用</button>
                        <input type="hidden" id="meter_type_<?php echo ($row["id"]); ?>_id" value="<?php echo ($row['id']); ?>">
                        <!-- <a href="<?php echo U('meter_return',array('meter_hash'=>$row['meter_hash']));?>">
                ...
                <td class="button-column">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu pull-left" role="menu" style="position: absolute; margin-left:-90px;">
                            <li>
                                <?php if($village_type == 0): ?><a data-toggle="modal" data-target="#common_modal"  href="<?php echo U('modal_meter_bind_room',array('meter_hash'=>$row['meter_hash']));?>">
                                        <i class="icon-docs"></i> 绑定房间
                                    </a>
                                    <?php else: ?>
                                    <a data-toggle="modal" data-target="#common_modal"  href="<?php echo U('modal_meter_bind_room_sq',array('meter_hash'=>$row['meter_hash']));?>">
                                        <i class="icon-docs"></i> 绑定房间
                                    </a><?php endif; ?>
                            </li>
                            <li>
                                <a data-toggle="modal" data-target="#common_modal"  href="<?php echo U('modal_meter_qr',array('meter_hash'=>$row['meter_hash']));?>">
                                    <i class="icon-docs"></i> 二维码
                                </a>
                            </li>
                            <?php if($row['is_update'] == 0): ?><li>
                                    <a  href="<?php echo U('modal_meter_update',array('meter_hash'=>$row['meter_hash']));?>">
                                        <i class="icon-docs"></i> 修改设备
                                    </a>
                                </li><?php endif; ?>
                            <!-- <li>
                                <?php if($_GET['is_del'] == 1): ?><a  href="<?php echo U('meter_return',array('meter_hash'=>$row['meter_hash']));?>">
