...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group" style="margin-top:10px;">
            <form action="<?php echo U('Budget_predict/watch_predict_one');?>" method="post">
                    ...
                    <div class="btn-group">
                        <select name="id" id="record_status"  class="form-inline search" style="line-height:34px; height:34px; border:1px #CCCCCC solid; margin-right:7px; margin-top:-1px;"><!--onchange="change_url('record_status',this.options[this.options.selectedIndex].value)"-->
                            <?php if(is_array($predict_list)): foreach($predict_list as $k=>$vo): ?><option value="<?php echo ($vo["predict_id"]); ?>" <?php if($vo['predict_id'] == $predict_id): ?>selected<?php endif; ?>><?php echo ($vo["village_name"]); ?></option><?php endforeach; endif; ?>
                        </select>
                    </div>
                </span>
                ...
                <div class="row">
                    <div class="tabbable-custom nav-justified">
                        <ul class="nav nav-tabs nav-justified">
                            <?php if(is_array($data_type)): foreach($data_type as $k=>$vo): if($k == 4): else: ?>
                                    <li <?php if($k == 1): ?>class="active"<?php endif; ?>>
                                        <a href="#tab_<?php echo ($k); ?>" data-toggle="tab" class="active"> <?php echo ($vo['info']['type_name']); ?>明细表</a>
                                    </li><?php endif; endforeach; endif; ?>
                            <!--<li>
                        ...
                        <div class="tab-content">
                            ...
                            <?php if(is_array($data_type)): foreach($data_type as $k=>$vo): if($k == 1): ?><div class='tab-pane active tab-pane fad' id="tab_<?php echo ($k); ?>">
                                            ...
                                            <table class="table table-striped table-bordered table-hover table-checkable order-column">
                                                ...
                                                <?php if($vo['children']): if(is_array($vo['children'])): foreach($vo['children'] as $k1=>$vo1): if($k1 == 0): else: ?>
                                                            <?php if(is_array($vo1['children'])): $k2 = 0; $__LIST__ = $vo1['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($k2 % 2 );++$k2;?><tr>
                                                                    <?php if($vo1['type_name'] == '派遣和劳务支出'): if($k2 == 1): ?><td rowspan="<?php echo count($vo1['children'])+1;?>" style="text-align:center;vertical-align:middle;"><?php echo ($vo1['type_name']); ?></td><?php endif; ?>
                                                                    ...
                                                                    <td title="<?php echo ($vo['data'][$k1]['children'][$vo2['type_id']]['type_data']['remark']); ?>"><?php echo ($vo['data'][$k1]['children'][$vo2['type_id']]['type_data']['remark']); ?></td><?php endif; ?>
                                                                </tr><?php endforeach; endif; else: echo "" ;endif; endif; endforeach; endif; ?>
                                                    <?php else: endif; ?>
