...
<!--<div class="page-container">-->
    ...
    <?php if($is_record != -1): ?><div class="btn-group">
            <a href="<?php echo U('',array_merge($_GET,array('is_record'=>-1)));?>">
                <button id="sample_editable_1_new" class="btn sbold green">返回
                </button>
            </a>
        </div><?php endif; ?>
    <div class="btn-group">
        <a href="<?php echo U('Room/meterlist_news');?>">
            <button id="sample_editable_1_new" class="btn sbold green">设备管理（<?php echo count($list);?>）
            </button>
        </a>
    </div>
    <?php if($is_record != 0): ?><div class="btn-group">
            <a href="<?php echo U('',array_merge($_GET,array('is_record'=>0)));?>">
                <button id="sample_editable_1_new" class="btn sbold green">查看未抄录设备（<?php echo ($no_record_count); ?>）
                </button>
            </a>
        </div><?php endif; ?>
    <?php if($is_record != 1): ?><div class="btn-group">
            <a href="<?php echo U('',array_merge($_GET,array('is_record'=>1)));?>">
                <button id="sample_editable_1_new" class="btn sbold green">查看已抄录设备（<?php echo ($is_record_count); ?>）
                </button>
            </a>
        </div><?php endif; ?>
    <div class="btn-group">
        <a href="<?php echo U('Meter/meter_import_record');?>">
            <button id="sample_editable_1_new" class="btn sbold green">抄表记录初始化导入
            </button>
        </a>
    </div>
    <div class="btn-group">
        <a href="<?php echo U('Meter/output_meter_record_list');?>">
    ...
    <h4 class="text-muted" style="display:inline-block">
        &nbsp
        <?php switch($is_record): case "-1": ?>总设备数：<?php break;?>
            <?php case "0": ?>未抄录设备:<?php break;?>
            <?php case "1": ?>已抄录设备:<?php break; endswitch;?>
        <?php echo count($list);?>
    ...
    <table class="table table-striped table-bordered table-hover" id="sample_1">
        <thead>
            <tr>
                ...
                <th>上次止码</th>
                <th>本月止码</th>
                <?php if($is_record != 0): ?><th>抄录人</th>
                <th>当月用量</th>
                <th>抄录时间</th><?php endif; ?>
                <th>操作</th>
            </tr>
        </thead>
        <tbody>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                ...
                <?php if($row['is_record'] == 1): ?><td><?php echo ($row["last_total_consume"]); ?></td>
                    ...
                    <td>未抄录</td><?php endif; ?>
                <?php if($is_record != 0): ?><td><?php echo ($row["realname"]); ?></td>
                <td><?php echo ($row["consume"]); ?></td>
                <td>
                    <?php if($row['create_time'] != '' ): echo (date("Y-m-d",$row["create_time"])); ?>
                        <?php else: ?>
                        当月未抄录<?php endif; ?>
                ...
                <td class="button-column">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu pull-left" role="menu" style="position: absolute; margin-left:-90px;">
                            ...
                            <?php if($row['record_id']): ?><li>
                                    <a target="_blank" href="<?php echo U('edit_record',array('record_id'=>$row['record_id']));?>">
