...
<!--<div class="page-container">-->
    ...
    <div class="btn-group">
        <a href="<?php echo U('Budget_predict/predict_in');?>" >
    ...
    <div class="btn-group">
        <a href="<?php echo U('Budget/check_excel_print');?>" >
            <button  class="btn sbold red" >
                预算执行总表
            </button>
        </a>
                ...
                <span>
                    <div class="btn-group">
                        <select id="datetimepicker"  class="form-control" placeholder="" name="startDate" onchange="change_url('year',this.options[this.options.selectedIndex].value)">
                            <?php $__FOR_START_8265__=2017;$__FOR_END_8265__=date('Y')+2;for($i=$__FOR_START_8265__;$i < $__FOR_END_8265__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?></option><?php } ?>
                        </select>
                    </div>
                    <div class="btn-group">
    ...
    <div class="tabbable-custom nav-justified" style="width:100%;overflow-x: scroll;">
        ...
        <div style="width: 100%;"><?php echo ($year); ?>年<?php echo ($title1); ?>预算编制汇总表</div>
        <table class="table table-striped table-bordered table-hover" style="overflow-x: auto" id="sample_1" >
            ...
            <tr>
                ...
                <?php if(is_array($predict_list)): foreach($predict_list as $key=>$vo): ?><th width="10%"><?php echo ($vo['village_name']); ?></th><?php endforeach; endif; ?>
                <th width="20%">总计</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                ...
                <?php $cache_sum=0;?>
                <?php if(is_array($predict_list)): foreach($predict_list as $key=>$vo1): $cache_sum +=$vo1['sum']['input']['sum_sum'];?>
                        <td><?php echo number_format($vo1['sum']['input']['sum_sum'],2);?></td><?php endforeach; endif; ?>
                <th><div class="tagDiv"><?php echo number_format($cache_sum,2);?></div></th>
            </tr>
            <?php if(is_array($type_list['input']['1']['children'])): $i = 0; $__LIST__ = $type_list['input']['1']['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($i); ?></td>
                    <td colspan="2"><?php echo ($vo['type_name']); ?></td>
                    <?php $cache_sum=0;?>
                    <?php if(is_array($predict_list)): foreach($predict_list as $key1=>$vo1): $cache_sum +=$vo1['sum']['input']['1']['children'][$key]['sum_sum'];?>
                        <td><div class="tagDiv"><?php echo number_format($vo1['sum']['input']['1']['children'][$key]['sum_sum'],2);?></div></td><?php endforeach; endif; ?>
                    <td><div class="tagDiv"><?php echo number_format($cache_sum,2);?></div></td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <tr>
                <td>二</td>
                <td colspan="2">支出合计</td>
                <?php $cache_sum=0;?>
                <?php if(is_array($predict_list)): foreach($predict_list as $key=>$vo): $cache_sum +=$vo['sum']['output']['sum_sum'];?>
                    <td><?php echo number_format($vo['sum']['output']['sum_sum'],2);?></td><?php endforeach; endif; ?>
                <td><div class="tagDiv"><?php echo number_format($cache_sum,2);?></div></td>
            </tr>
            <?php if(is_array($type_list['output'])): foreach($type_list['output'] as $ke=>$vo): $cache_key1=key($vo['children']);$change++;?>
                <?php if(is_array($vo['children'])): foreach($vo['children'] as $ke1=>$vo1): ?><tr>
                        <?php if($ke1 == $cache_key1): ?><td rowspan="<?php echo count($vo['children'])+1;?>" style="text-align:center;vertical-align:middle;"><?php echo ($change); ?></td>
                        ...
                        <?php $cache_sum=0;?>
                        <?php if(is_array($predict_list)): foreach($predict_list as $key1=>$value): $cache_sum +=$value['sum']['output'][$ke]['children'][$ke1]['sum_sum'];?>
                            <td><div class="tagDiv"><?php echo number_format($value['sum']['output'][$ke]['children'][$ke1]['sum_sum'],2);?></div></td><?php endforeach; endif; ?>
                        <td><div class="tagDiv"><?php echo number_format($cache_sum,2);?></div></td>
                    </tr><?php endforeach; endif; ?>
                <?php if(is_array($vo)): ?><tr>
                        <td>小计</td>
                        <?php $cache_sum=0;?>
                        <?php if(is_array($predict_list)): foreach($predict_list as $key=>$value): $cache_sum +=$value['sum']['output'][$ke]['sum_sum'];?>
                            <td><div class="tagDiv"><?php echo number_format($value['sum']['output'][$ke]['sum_sum'],2);?></div></td><?php endforeach; endif; ?>
                        <td><div class="tagDiv"><?php echo number_format($cache_sum,2);?></div></td>
            ...
            <tr>
                ...
                <td colspan="2">净收支</td>
                <?php $cache_sum=0;?>
                <?php if(is_array($predict_list)): foreach($predict_list as $key=>$vo): $cache_sum +=$vo['sum']['sum']['sum_sum'];?>
                    <td><?php echo number_format($vo['sum']['sum']['sum_sum'],2);?></td><?php endforeach; endif; ?>
                <td><div class="tagDiv"><?php echo number_format($cache_sum,2);?></div></td>
            </tr>
            </tbody>
        </table>
...
<!--/底部文件-->
    ...
    <script>
        ...
        sessionStorage.setItem('company_id',<?php echo ($company_id); ?>);
        sessionStorage.setItem('year',<?php echo ($year); ?>);
        function change_url(type,val) {
            sessionStorage.setItem(type,val);
            window.location.href='<?php echo U('watch_company_one');?>&company_id='+sessionStorage.getItem('company_id')+'&year='+sessionStorage.getItem('year');
        }
        $("#sample_editable_1_new").click(function () {
            $("#sample_1").table2excel({
                exclude  : ".noExl", //过滤位置的 css 类名
                filename : "<?php echo ($year); ?>年<?php echo ($title1); ?>预算编制汇总表.xls", //文件名称
