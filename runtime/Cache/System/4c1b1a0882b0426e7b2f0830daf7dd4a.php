                ...
                <span>
                    <div class="btn-group">
                        <select id="datetimepicker"  class="form-control" placeholder="" name="startDate" onchange="change_urls('year',this.options[this.options.selectedIndex].value)">
                            <?php $__FOR_START_604__=2017;$__FOR_END_604__=date('Y')+2;for($i=$__FOR_START_604__;$i < $__FOR_END_604__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?></option><?php } ?>
                        </select>
                    </div>
                    <div class="btn-group">
    ...
    <div class="tabbable-custom nav-justified" style="width:100%;overflow-x: scroll;">
        <ul class="nav nav-tabs nav-justified">
            <?php if(is_array($type_list)): $i = 0; $__LIST__ = $type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li <?php if($_GET['type'] == $vo['type_id']): ?>class="active"<?php endif; ?>>
                <a href="<?php echo U('Budget/check_excel_print',array('type'=>$vo['type_id'],'role_id'=>$role_id));?>" onclick="loading();"> <?php echo ($vo['type_name']); ?> </a>
                </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
        <table class="table table-striped table-bordered table-hover" id="sample_1" >
            ...
            <tr>
                <th>合计</th>
                <?php $__FOR_START_3293__=1;$__FOR_END_3293__=13;for($i=$__FOR_START_3293__;$i < $__FOR_END_3293__;$i+=1){ ?><td width="5%"><?php echo ($i); ?>月</td><?php } ?>
            </tr>
            </thead>
            <tbody>
            <?php if(is_array($data)): foreach($data as $key=>$vo): if($key !== 'sum'): if(is_array($vo['children'])): $k = 0; $__LIST__ = $vo['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($k % 2 );++$k;?><tr>
                            ...
                            <?php $__FOR_START_2668__=1;$__FOR_END_2668__=13;for($i=$__FOR_START_2668__;$i < $__FOR_END_2668__;$i+=1){ if($vo1['type_data'][$i]): ?><td title="双击点击查看明细"  ondblclick="open_url(<?php echo ($key); ?>,<?php echo sprintf('%02d',$i);?>)"><?php echo number_format($vo1['type_data'][$i],2);?></td>
                                    <?php else: ?>
                    ...
                    <tr>
                        ...
                        <?php $__FOR_START_30747__=1;$__FOR_END_30747__=13;for($i=$__FOR_START_30747__;$i < $__FOR_END_30747__;$i+=1){ ?><td><?php echo number_format($vo[$i],2);?></td><?php } ?>
                        <td></td>
...
<!--/底部文件-->
    ...
    <script>
        ...
        function change_urls(type,val) {
            ...
            window.location.href='<?php echo U('Budget/check_excel_print',array('type'=>$_GET['type']));?>&'+type+'='+val+'&role_id='+role_id;
        }
