...
<!--<div class="page-container">-->
    ...
    <div class="btn-group">
        ...
        <span id="filter">
                <span>
                    ...
                    <div class="btn-group">
                        <input type="text" id="datetimepicker"  class="form-control" placeholder="" name="startDate" value="<?php echo ($month); ?>" onchange="change_url('month',this.value)">
                    </div>-->
                    <div class="btn-group">
                    ...
                    <div class="btn-group">
                        <select name="type" id="type"  class="form-control search">
                                ...
                                <?php if(is_array($fee_type_list)): foreach($fee_type_list as $key=>$value): ?><option value="<?php echo ($key); ?>"><?php echo ($value); ?></option><?php endforeach; endif; ?>
                        </select>
                    </div>
                    <div class="btn-group">
                    ...
                    <div class="btn-group">
                        <input type="text" id="datetimepicker"  class="form-inline" placeholder="" name="starttime" value="<?php echo ($time['start']); ?>" style="line-height:34px; height:34px; border:1px #CCCCCC solid; margin-right:7px; padding-left:5px;">
                         至&nbsp;
                        <input type="text" id="datetimepicker1"  class="form-inline" placeholder="" name="endtime" value="<?php echo ($time['end']); ?>" style="line-height:34px; height:34px; border:1px #CCCCCC solid; margin-right:7px; padding-left:5px;">
                    </div>
                </span>
    ...
    <div style="width:100%;overflow-x: scroll;">
        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sum" style="width: 100%; margin-top:20px;table-layout: fixed;">
            ...
            <tr>
                ...
                <?php if(is_array($type_list)): foreach($type_list as $key=>$value): ?><th style="width: 150px"><?php echo ($value['otherfee_type_name']); ?>/笔数</th><?php endforeach; endif; ?>
            </tr>
            </thead>
            <tbody>
            <?php if(is_array($fee_sum_list)): foreach($fee_sum_list as $key=>$value): ?><tr>
                    <th><?php echo ($value['type_name']); ?></th>
                    <th><span id="sum_cash" style="font-size:16px; color:#f36a5a"><?php echo number_format($value['sum_cash'],2);?>元/<?php echo ($value['sum_count']); ?></span></th>
                    <?php if(is_array($type_list)): foreach($type_list as $key1=>$value1): ?><th><?php echo number_format($value['sum_list'][$value1['otherfee_type_id']]['sum_cash'],2);?>元/<?php echo ($value['sum_list'][$value1['otherfee_type_id']]['sum_count']); ?></th><?php endforeach; endif; ?>
    ...
    <table class="table table-striped table-bordered table-hover" id="sample_1" >
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><td>
            ...
            <td><div class="tagDiv"><?php echo ($vo["owner"]); ?></div></td>
            <td><div class="shopNameDiv"><?php echo ($vo["type_name"]); ?></div></td>
            <td><div class="shopNameDiv"><?php echo ($vo["fee_type_name"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["pay_receive"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["pay_true"]); ?></div></td>
            <?php if($vo['status'] == 0): ?><td><div class="tagDiv" style="color: red" <?php if($vo['receipt_request']): ?>title="处理时间：<?php echo date('Y-m-d H:i:s',$vo['receipt_request']['check_time']);?>
                        申请备注:<?php echo ($vo['receipt_request']['remark']); ?>
                        处理备注:<?php echo ($vo['receipt_request']['check_remark']); ?>"<?php endif; ?>><?php echo ($vo["status_name"]); ?></div></td>
                <?php else: ?>
                <td><div class="tagDiv"><?php echo ($vo["create_time"]); ?>,</div><div <?php if($vo['receipt_request']): ?>title="处理时间：<?php echo date('Y-m-d H:i:s',$vo['receipt_request']['check_time']);?>
                        申请备注:<?php echo ($vo['receipt_request']['remark']); ?>
                        处理备注:<?php echo ($vo['receipt_request']['check_remark']); ?>"<?php endif; ?> style="color: red"><?php echo ($vo['status_name_remark']); ?></div></td><?php endif; ?>
            <td><div class="tagDiv"><?php echo ($vo["changetime"]); ?></div></td>
            <td><div class="tagDiv"><?php echo ($vo["remark"]); ?></div></td>
            <td class="button-column">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-left" role="menu" style="position:relative;">
                        ...
                        <li>
                            <a href="javascript:;" target="_blank" onclick="sub_one('<?php echo ($vo["pigcms_id"]); ?>-<?php echo ($vo["type_id"]); ?>')">
                                <i class="icon-tag"></i> 打印收据 </a>
                        </li>
                        <li>
                            <a href="javascript:;" target="_blank" onclick="update_one('<?php echo ($vo["pigcms_id"]); ?>-<?php echo ($vo["type_id"]); ?>')">
                                <i class="icon-tag"></i> 修改此缴费 </a>
                        </li>
                        <?php if($is_pm): ?><li>
                                <a href="javascript:;" target="_blank" onclick="delete_one('<?php echo ($vo["pigcms_id"]); ?>-<?php echo ($vo["type_id"]); ?>')">
                                    <i class="icon-tag"></i> 删除此缴费 </a>
                            </li>
                            <?php else: ?>
                            <li>
                                <a href="javascript:;" target="_blank" onclick="delete_request_one('<?php echo ($vo["pigcms_id"]); ?>-<?php echo ($vo["type_id"]); ?>')">
                                    <i class="icon-tag"></i> 申请删除此缴费 </a>
                            </li><?php endif; ?>
                        <?php if($is_fm): ?><li>
                                <a href="javascript:;" target="_blank" onclick="refund_all_one('<?php echo ($vo["pigcms_id"]); ?>-<?php echo ($vo["type_id"]); ?>')">
                                    <i class="icon-tag"></i> 全部退款 </a>
                            </li>
                            <li>
                                <a href="javascript:;" target="_blank" onclick="refund_one('<?php echo ($vo["pigcms_id"]); ?>-<?php echo ($vo["type_id"]); ?>','<?php echo ($vo["pay_true"]); ?>')">
...
<!--/底部文件-->
    ...
    <script>
        ...
        function update_one(id) {
            window.location.href="<?php echo U('Receipt/update_receipt');?>&id="+id;
        }
        /**
         * @author zhukeqin
         * @param id
        ...
        function delete_one(id) {
            ...
            swal({
                ...
                function(){
                    $.ajax({
                        url:'<?php echo U("Receipt/ajax_delete_fee");?>',
                        data:{"ids":ids},
                        type:'POST',
                        dataType:'json',
                        success:function (res) {
            ...
            swal({
                ...
                function(inputValue){
                    ...
                    if (inputValue === "") {
                        ...
                        $.ajax({
                            url:'<?php echo U("Receipt/ajax_delete_request_fee");?>',
        ...
        function refund_one(id,money) {
            ...
            swal({
                ...
                function(inputValue){
                    $.ajax({
                        url:'<?php echo U("Receipt/ajax_refund_fee");?>',
        ...
        function openPostWindow(url,idStr,yms,project_id){
            ...
            $('#print_status').val('<?php echo ($print_status); ?>');
            $('#type').val('<?php echo ($type); ?>');
            $('#pay_status').val('<?php echo ($pay_status); ?>');

        function change_url(type,val) {
            console.log(val);
            window.location.href='<?php echo U('Receipt/receipt_list');?>&'+type+'='+val;
        }
        $("#download").click(function () {
            $("#sum").table2excel({
                exclude  : ".noExl", //过滤位置的 css 类名
                filename : "<?php echo ($village_name); echo ($project_name); ?>合计笔数统计表<?php echo ($time['start']); ?>-<?php echo ($time['end']); ?>.xls", //文件名称
