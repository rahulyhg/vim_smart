...
<!--<div class="page-container">-->
    ...
    <div class="btn-group">
        <a href="<?php echo U('Budget/ajax_village_excel_print',array('year'=>$year));?>">
                ...
                <span>
                    <div class="btn-group">
                        <select id="datetimepicker"  class="form-control" placeholder="" name="startDate" onchange="change_url('year',this.options[this.options.selectedIndex].value)">
                            <?php $__FOR_START_12257__=2017;$__FOR_END_12257__=date('Y')+2;for($i=$__FOR_START_12257__;$i < $__FOR_END_12257__;$i+=1){ ?><option value="<?php echo ($i); ?>"><?php echo ($i); ?>年</option><?php } ?>
                        </select>
                    </div>
                </span>
            </span>
    ...
    <div class="tabbable-custom nav-justified">
        <ul class="nav nav-tabs nav-justified">
            <li class="active">
                <a href="<?php echo U('Budget/village_excel_print',array('type'=>'sum'));?>" onclick="loading();"> 执行主表 </a>
            </li>
            <?php if(is_array($type_list)): $i = 0; $__LIST__ = $type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                    <a href="<?php echo U('Budget/village_excel_print',array('type'=>$vo['type_id']));?>" onclick="loading();"> <?php echo ($vo['type_name']); ?> </a>
                </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
        <table class="table table-striped table-bordered table-hover" id="sample_1" >
            <thead>
            <tr>
                <td colspan="7" align="center" style="font-size: 25px"><?php echo ($year); ?>年<?php echo ($title1); ?>预算编制汇总表</td>
            </tr>
            <tr>
                <th width="5%">序号</th>
            ...
            <tr>
                ...
                <td><?php echo number_format($data['input']['sum_money_no'],2);?></td>
                <td><?php echo number_format($data['input']['sum_sum'],2);?></td>
                <td><?php echo number_format($data['input']['difference_no'],2);?></td>
                <td>含税金额</td>
            </tr>
            <?php if(is_array($data['input']['1']['children'])): $i = 0; $__LIST__ = $data['input']['1']['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><div class="tagDiv"><?php echo number_format($vo['sum_money_no'],2);?></div></td>
                    <td><div class="tagDiv"><?php echo number_format($vo['sum_sum'],2);?></div></td>
                    <td><div class="tagDiv"><?php echo number_format($vo['difference_no'],2);?></div></td>
                    <td><div class="tagDiv"><?php echo ($vo["type_remark"]); ?></div></td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <tr>
                <td>二</td>
                <td colspan="2">支出合计</td>
                <td><?php echo number_format($data['output']['sum_money_no'],2);?></td>
                <td><?php echo number_format($data['output']['sum_sum'],2);?></td>
                <td><?php echo number_format($data['output']['difference_no'],2);?></td>
                <td>含税金额</td>
            </tr>
            <?php if(is_array($data['output'])): foreach($data['output'] as $ke=>$vo): if(is_array($vo['children'])): $k = 0; $__LIST__ = $vo['children'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($k % 2 );++$k;?><tr>
                        ...
                        <td><div class="tagDiv"><?php echo number_format($vo1['sum_money_no'],2);?></div></td>
                        <td><div class="tagDiv"><?php echo number_format($vo1['sum_sum'],2);?></div></td>
                        <td><div class="tagDiv"><?php echo number_format($vo1['difference_no'],2);?></div></td>
                        <td><div class="tagDiv"><?php echo ($vo1["type_remark"]); ?></div></td>
                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                <?php if(is_array($vo)): ?><tr>
                        <td>小计</td>
                        <td><div class="tagDiv" id="<?php echo ($key); ?>"><?php echo number_format($vo['sum_money_no'],2);?></div></td>
                        <td><div class="tagDiv"><?php echo number_format($vo['sum_sum'],2);?></div></td>
                        <td><div class="tagDiv"><?php echo number_format($vo['difference_no'],2);?></div></td>
                        <td></td>
            ...
            <tr>
                <td>三</td>
                <td colspan="2">净收支</td>
                <td><?php echo number_format($data['sum']['sum_money_no'],2);?></td>
                <td><?php echo number_format($data['sum']['sum_sum'],2);?></td>
                <td><?php echo number_format($data['sum']['difference_no'],2);?></td>
                <td>含税金额</td>
            </tr>
            </tbody>
        </table>
...
<!--/底部文件-->
    ...
    <script>
        ...
        function change_url(type,val) {
                window.location.href='<?php echo U('Budget/village_excel_print',array('type'=>$_GET['type']));?>&'+type+'='+val;
            }
    </script>
