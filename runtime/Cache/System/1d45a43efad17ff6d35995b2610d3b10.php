...
<!--头部设置-->
<?php
$title = array( 'title'=>'微信消息日志', 'describe'=>'', ); $breadcrumb = array( array('信息发布',U('group_msg_list_news')), array('微信消息日志','#'), ); $add_action = array( ); ?>
<!--头部设置结束-->
...
<table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
    ...
    <tbody>
    <?php if($list): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                <td>
                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                ...
                <td><?php echo ($row["ruser"]); ?></td>
                <td><a href = "<?php echo U('get_msg_log_info',array('log_id'=>$row['id']));?>"
                                       data-toggle="modal"
                                       data-target="#modal_<?php echo ($row["id"]); ?>"
                    >
                        <?php echo ($row["tpl_title"]); ?>
                    </a>
                </td>
                <td><?php echo ($row["errcode"]); ?> : <?php echo ($row["errmsg"]); ?></td>
...
<!--                            <li>-->
<!--                                <a href = "<?php echo U('get_msg_log_info',array('log_id'=>$row['id']));?>"-->
<!--                                   data-toggle="modal"-->
<!--                                   data-target="#modal_<?php echo ($row["id"]); ?>"-->
...
<!--                </td>-->
        ...
        <tr><td class="textcenter pagebar" colspan="16"><?php echo ($page_bar); ?></td></tr>
        <?php else: ?>
        <tr class="odd"><td class="textcenter red" colspan="16" >暂无数据</td></tr><?php endif; ?>
    </tbody>
</table>
<!--业务区结束-->
