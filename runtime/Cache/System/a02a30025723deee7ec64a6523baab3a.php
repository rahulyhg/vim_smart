...
<!--<div class="page-container">-->
    ...
    <?php if($_SESSION['system']['tid']== 0): ?><div class="btn-group">
            <a href="<?php echo U('Room/add_tenant');?>">
                <button class="btn sbold green">添加入驻单位
                    <i class="fa fa-plus"></i>
                </button>
            </a>
            <!--        根据模块区分-->
            <?php switch($_mod): case "property": ?><a href="<?php echo U('Room/bill_preview');?>">
                        <button class="btn sbold green">账单预览</button>
                    </a>
                    <a href="<?php echo U('Room/account_list');?>">
                        <button class="btn sbold green">当月台账</button>
                    </a><?php break;?>
                <?php case "user": if($_out == 0): ?><a href="<?php echo U('Room/tenantlist_news',array('_mod'=>'user','_out'=>1));?>">
                            <button class="btn sbold green">已迁出单位</button>
                        </a>
                        <?php else: ?>
                        <a href="<?php echo U('Room/tenantlist_news',array('_mod'=>'user','_out'=>0));?>">
    ...
    <br/>
    <div class="btn-group">
        <input type="month" name="choose_time" value="<?php echo $_GET['ym']?:date('Y-m') ?>"/>
    </div><?php endif; ?>
...
<!--        入驻状态	入驻单位	业主	总面积	缴费状态	操作-->
        <tr>
            ...
            <?php if($_out == 0): ?><th>房间号</th>
                <th>入驻状态</th><?php endif; ?>
            <?php if($_out == 0): ?><th>入驻单位</th>
                <?php else: ?>
                <th>已迁出单位</th><?php endif; ?>
            <th>联系方式（入住单位）</th>
            <?php if($_out == 0): ?><th>业主</th>
                <th>总面积</th>
                <?php switch($_mod): case "property": ?><th>缴费状态</th><?php break;?>
                    <?php case "user": break; endswitch;?>
                <th>操作</th><?php endif; ?>
        </tr>
        </thead>
        <tbody>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="background-color: #ffffff">
                <td>
                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                ...
                <?php if($_out == 0): ?><td class="tagDiv">
                        <!--排序需要-->
                        <span style="color:#FBFCFD;text-overflow:ellipsis;display:block;width:0px;height:0">
                            <?php echo ($vo["tung_unit"]); echo sprintf("%04d",$vo['room_names']?:9999);?>
                        </span>
                        <!--/排序需要-->
                        <?php echo ($vo["tung_unit"]); ?>
                        <?php echo ($vo["room_names"]); ?>
                    </td>

                    <td class="fstatus">
                        <select_fstatus oids="<?php echo ($vo["oids"]); ?>" fstatus="<?php echo ($vo["fstatus"]); ?>"></select_fstatus>
                    </td><?php endif; ?>

                    <td>
                        <div class="tagDiv"><?php echo ($vo["tenantname"]); ?></div>
                    </td>

                    <td>
                        <div class="tagDiv"><?php echo ($vo["tname"]); ?>|<?php echo ($vo["tphone"]); ?></div>
                    </td>

                <?php if($_out == 0): ?><td>
                        <div class="tagDiv"><?php echo str_replace(',','<br>',$vo['ownernames']);?></div>
                    </td>
                    <td>
                        <div class="tagDiv"><?php echo ($vo["housesize"]); ?> ㎡ </div>
                    </td>
                    <!--                根据模块区分-->
                    <?php switch($_mod): case "property": ?><td>
                                <?php if($vo["is_enter_list"] == 1): ?><div>
                                        <?php if($vo['total_price'] != 0): ?><div>
                                                <a  data-toggle="modal" data-target="#common_modal" class="btn btn-xs yellow-crusta" href="<?php echo U('Room/before_pay',array('pid'=>$vo['pid'],'ym'=>$_GET['ym']));?>" style="    line-height: 30px; padding: 0 10px 0 10px; font-size: 14px;"><span style="font-weight:bold;">¥:<?php echo sprintf('%.2f',$vo['total_price']);?></span></a>
                                            </div>
                                        <?php else: ?>
                                            <button class="btn  blue-steel"><i class="fa fa-check"></i>已缴费</button><?php endif; ?>

                                    </div>
                                <?php else: ?>
                                    <?php if($_SESSION['system']['tid']== 0): ?><a href="<?php echo U('Room/bill_preview',array('tid'=>$vo['pigcms_id']));?>">
                                            <button class="btn sbold green">未确认的账单</button>
                                        </a>
                                    <?php else: ?>
                                            <button class="btn sbold green">未确认的账单</button><?php endif; endif; ?>
                            </td><?php break;?>
                        <?php case "user": break; endswitch;?>
                    <!--                /根据模块区分-->
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu pull-left" role="menu" style="position: absolute; margin-left:-90px;">
                            <?php if($_SESSION['system']['tid']== 0): ?><li>
                                <a data-toggle="modal" data-target="#common_modal"  href="<?php echo U('Room/modal_tenant_bind_room',array('tid'=>$vo['pigcms_id']));?>">
                                    <i class="icon-docs"></i> 绑定房间
                                </a>
                            </li>
                            <li>
                                <a data-toggle="modal" data-target="#common_modal"  href="<?php echo U('Room/modal_tenant_rooms',array('tid'=>$vo['pigcms_id']));?>">
                                    <i class="icon-docs"></i> 房间配置
                                </a>
                            </li>
                            <li>
                                <a data-toggle="modal" data-target="#common_modal"  href="<?php echo U('Room/modal_tenant_meters',array('tid'=>$vo['pigcms_id']));?>">
                                    <i class="icon-docs"></i> 设备配置
                                </a>
                            </li>

                            <li>
                                <a href="<?php echo U('Room/tenant_edit',array('tid'=>$vo['pigcms_id']));?>">
                                    <i class="icon-docs"></i> 编辑
                                </a>
                            </li><?php endif; ?>
                            <!--                根据模块区分-->
                            <?php switch($_mod): case "property": ?><li>
                                        <a href="<?php echo U('Room/pay_history_list',array('tid'=>$vo['pigcms_id']));?>">
                                            <i class="icon-docs"></i> 缴费明细
                                        </a>
                                    </li>
                                    <!--<?php if($vo["is_enter_list"] == 1): ?><li>
                                            <a data-toggle="modal" data-target="#common_modal" href="<?php echo U('PropertyService/choose_pay_type',array('money'=>sprintf('%.2f', $vo['total_property']+$vo['total_other']+$vo['total_electric']+$vo['total_water']),'type'=>'all','pid'=>$vo['pid']));?>">
                                                <i class="icon-docs"></i> 全部缴费
                                            </a>
                                        </li><?php endif; ?>--><?php break;?>
                                <?php case "user": break; endswitch;?>
                            <!--                /根据模块区分-->
                        </ul>
                    </div>
                    ...
                    <td class="button-column">
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu pull-left" role="menu" style="position: absolute; margin-left:-90px;">
                                ...
                                <li>
                                    <a href="<?php echo U('del_tenant_act',array('tid'=>$vo['pigcms_id']));?>" onclick="return window.confirm('警告：数据删除后不恢复，确认删除？')">
