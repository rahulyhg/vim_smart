...
// 		$code = md5($this->_im_appid.$this->from_user_id.$this->from_user_name.$this->from_user_avatar.$this->no_header.$this->back_url.$this->_im_appkey);
		...
		$this->im_url = ($this->config['im_url']).'/?app_id=' . $this->config['im_appid'] . '&openid=' . $_SESSION['openid'] . '&key=' . $key;
...
// 		$im_url = 'http://im.maopan.com/my.php?app_id='.$this->_im_appid.'&from_user_id='.$this->from_user_id.'&from_user_name='.urlencode($this->from_user_name).'&from_user_avatar='.urlencode($this->from_user_avatar).'&no_header='.$this->no_header.'&back_url='.urlencode($this->back_url).'&code='.$code;
...

	{
		...
		if (count($men) < 6) {
			...
			if ($uids) {
				...
				$othermen = D('User')->field('uid, nickname, avatar')->where(array('sex' => 1, 'uid' => array('not in', $uids)))->order('uid DESC')->limit('0,' . $l)->select();
		...
		if (count($women) < 6) {
			...
			if ($uids) {
				...
				$otherwomen = D('User')->field('uid, nickname, avatar')->where(array('sex' => 2, 'uid' => array('not in', $uids),'status'=>1))->order('uid DESC')->limit('0,' . $l)->select();

			} else {

				$otherwomen = D('User')->field('uid, nickname, avatar')->where(array('sex' => 2,'status'=>1))->order('uid DESC')->limit('0,' . $l)->select();
		...
		$count1 = D('User')->where(array('sex' => 1,'status'=>1))->count();

		$this->assign('count1', $count1);

		$count2 = D('User')->where(array('sex' => 2,'status'=>1))->count();
...

	{
		...
		$users = D('User')->field('uid, nickname, avatar')->where(array('sex' => $sex))->order('uid DESC')->limit('0, 24')->select();
