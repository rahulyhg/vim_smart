			...
			if (!$user_info) {    //判断是否已存在
				...
				if ($user_info['role'] <= 1 || $user_info['role'] == "") {    //业主身份
					if ($user_info['ac_status'] == 1 || $user_info['ac_status'] == 3) {    //是业主且已提交智能门禁审核但没得到认可时
						...
						//var_dump($arguments_all_feild);exit;
			...
			if($where_terrace['terrace_class'] == 'Yeelink'){
				...
				if($yeelink->getStatus($access_array['sensorid'])){	//判断开关是否已开
					...
					if($status_data==""){
						...
						import('@.ORG.syslog');
						$log = new syslog('开门异常：sensorid错误','access_control_user_log',$access_array['ac_name']);
				...
				if(!$control_log) {
					import('@.ORG.syslog');
					$log = new syslog('开门异常：记录失败','access_control_user_log',$access_array['ac_name']);
					$data_log=$log->get_data();
					M('operation_log')->data($data_log)->add();
				 ...
				 * */
				import('@.ORG.'.$where_terrace['terrace_class']);
				$keyOb=new $where_terrace['terrace_class']($access_array['unios_pin'],$access_array['unios_act'],$where_terrace['key'],$agv_arr['commandToken'],$where_terrace['url']);
				...
				if(empty($result_json)){
					...
					//开门异常进入后台日志
					import('@.ORG.syslog');
					$log = new syslog('开门异常：服务器未响应','access_control_user_log',$access_array['ac_name']);
					$data_log=$log->get_data();
					M('operation_log')->data($data_log)->add();
					...
					//失败调用返回状态0
					import('@.ORG.syslog');
					$log = new syslog('开门异常：'.$result_json['error'],'access_control_user_log',$access_array['ac_name']);
					$data_log=$log->get_data();
					M('operation_log')->data($data_log)->add();
				...
				if(!$control_log) {
					import('@.ORG.syslog');
					$log = new syslog('开门异常：记录未成功','access_control_user_log',$access_array['ac_name']);
					$data_log=$log->get_data();
					M('operation_log')->data($data_log)->add();
					$this->redirect(U('House/access_control_show_old', array('village_id' => $agv_arr['village_id'], 'ac_id' => $agv_arr['ac_id'], 'msg' => '记录失败')));
				}
			...
			if($where_terrace['terrace_class'] == 'Yeelink'){
				...
				if($yeelink->getStatus($access_array['sensorid'])){	//判断开关是否已开
					...
					if($status_data==""){
						...
						import('@.ORG.syslog');
						$log = new syslog('开门异常：'.$status_data,'access_control_user_log',$access_array['ac_name']);
						$data_log=$log->get_data();
						M('operation_log')->data($data_log)->add();
				...
				if(empty($result_json)){
					//服务器问题调用返回状态500（维护等）
					import('@.ORG.syslog');
					$log = new syslog('开门异常：服务器500','access_control_user_log',$access_array['ac_name']);
					$data_log=$log->get_data();
					M('operation_log')->data($data_log)->add();
