 ...
 class NewsAction extends BaseAction{
	 public function index(){
            ...
            if($cat_id){
                ...
                $news = M('System_news n');
                $where['n.status'] = 1;
                $count_news = $news->where($where)->count();
                import('@.ORG.news_page');
                $p = new Page($count_news, 15,'page');
                $news_title = $news->field('n.id,n.title,n.add_time,c.name')->join(C('DB_PREFIX').'System_news_category c ON c.id = n.category_id ')->where($where)->order('n.sort DESC,n.id DESC')->limit($p->firstRow . ',' . $p->listRows)->select();
                $this->assign('news_title',$news_title);
                $pagebar = $p->show();
                $this->assign('pagebar', $pagebar);
            }
