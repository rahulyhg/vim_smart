...
//                }else{
//                    exit(json_encode(array('error_code' => true, 'msg' => '用户未绑定手机号')));
//                }
            } elseif($uidarray == -1) {
//                解决老用户的问题
...
//                dump($od_uid);exit;
                if(!empty($od_uid)){
                   if($od_uid['pwd'] != md5($pwd)){
                       exit(json_encode(array('error_code' => true, 'msg' => '密码有误！')));
                   }
                    $data_user['phone'] = $phone;
                    $data_user['nickname'] = substr($phone,0,3).'****'.substr($phone,7);
                    $data_user['add_time'] = $data_user['last_time'] = $_SERVER['REQUEST_TIME'];
                    ...
                    if($uid>0){
                        ...
                        exit(json_encode(array('error_code' => true, 'msg' => '账号有误！')));
                    }
                }else{
                    exit(json_encode(array('error_code' => true, 'msg' => '用户不存在,或者被删除')));
                }
            } elseif($uidarray == -2) {
                exit(json_encode(array('error_code' => true, 'msg' => '密码错误')));
            } elseif($uidarray == -3) {
                exit(json_encode(array('error_code' => true, 'msg' => '安全提问错误')));
            } else {
                exit(json_encode(array('error_code' => true, 'msg' => '未知错误')));
            }
...
//                dump($od_uid);exit;
    ...
    public function reg() {
        if (IS_POST) {
            $phone = isset($_POST['phone']) ? $_POST['phone'] : '';
            $pwd = isset($_POST['pwd']) ? $_POST['pwd'] : '';
            $vfycode= isset($_POST['vfycode']) ? $_POST['vfycode'] : '';	//验证码
			//$user_code=M('user_modifypwd')->where(array('telphone'=>$phone,'vfcode'=>$vfycode))->find();
			//if($user_code['vfcode'] && (time()-$user_code['addtime'])<=60*20){	//判断验证码是否已过期或不存在
				...
				exit(json_encode($result));
			//}else{
				//exit(json_encode(array('error_code' =>2,'msg' =>'此验证码已过期或不存在')));
			//}
        } else {
            $pigcms_assign['referer'] = !empty($_GET['referer']) ? strip_tags(htmlspecialchars_decode($_GET['referer'])) : (!empty($_SERVER['HTTP_REFERER']) ? strip_tags($_SERVER['HTTP_REFERER']) : U('Index/Index/index'));
            $pigcms_assign['url_referer'] = urlencode($pigcms_assign['referer']);
    ...
    public function sendsms()
    {
        ...
        if($mode==1){
            $mode=='reg';
            if ($Userarr)
                exit(json_encode(array('error_code' => 3, 'msg' => '手机账号己存在！')));
        }else if($mode==2){
            $mode=='add';
        }
        $user_modifypwdDb = M('User_modifypwd');
        $vcode_count=$user_modifypwdDb->query("SELECT COUNT(*) as total FROM pigcms_user_modifypwd WHERE telphone='$phone' and mode>0 and  FROM_UNIXTIME(ADDTIME)>DATE_SUB(NOW(),INTERVAL 10 MINUTE)");
	    if ($vcode_count[0].total>8) {
            exit(json_encode(array('error_code' => 1, 'msg' => '抱歉，你发送短信太过频繁啦，请过10分钟再试！')));
        }
        $vcode = createRandomStr(6, true, true);
        $content =$this->config['sms_sign'].'您的验证码是：' . $vcode . '。此验证码20分钟内有效，请不要把验证码泄露给其他人。如非本人操作，可不用理会！';
        //exit(json_encode(array('error_code' => 3, 'msg' => $content.'---'.$phone.'---'.$mode)));
        //Sms::sendSms(array('mer_id' => 0, 'store_id' => 0, 'content' => $content, 'mobile' => $phone, 'uid' => 0, 'type' => $mode));
        $addtime = time();
        $expiry = $addtime + 20 * 60; /*             * **二十分钟有效期*** */
        $data = array('telphone' => $phone, 'vfcode' => $vcode, 'expiry' => $expiry, 'addtime' => $addtime,'mode'=>'1');
    ...
    public function Generate() {
        ...
        if (($tmpid > 0) && !empty($vfycode)) {
            ...
            $vcode = createRandomStr(7,false,true);
