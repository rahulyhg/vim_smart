...
class FrontmanagAction extends BaseAction {
    ...
    public function binding(){
        $mer_id = $this->mer_id;
        $uids = M('merchant')->where('mer_id='.$mer_id)->getField('uid');
        $use = array();
        if($uids!="0"){
            $piece = explode(",", $uids);
            $num = count($piece);
            if($num>1){
                ...
                $use = M('user')->where($map)->select();
            }else{
                $use = M('user')->where('uid='.$uids)->select();
            }
        }
        $this->assign('info',$use);
        $this->display();
    }
    /**d
     * 商家用户绑定删除
    ...
    public function bd_del(){
        ...
        $num = count($piece);
        if($num>1){
                $key = array_search($uid,$piece);
                unset($piece[$key]);
                $bd_uid = implode(",",$piece);
                $f = M('merchant')->where('mer_id='.$mer_id)->setField('uid',$bd_uid);
        }else{
            $f = M('merchant')->where('mer_id='.$mer_id)->setField('uid',"0");
        }
        if($f){
            M('user')->where('uid='.$uid)->setField('mer_id',"0");
            $this->success('删除成功',U('Frontmanag/binding'));
        }else{
            $this->error('删除失败');
        }
    }
