...
class WeidianAction extends BaseAction{
	...
	public function create(){
		$this->api_url = $this->config['weidian_url'].'/api/';
		$data = array(
			'token'      => $this->merchant_session['mer_id'],
			'site_url'   => $this->config['site_url'],
			'timestamp'  => $_SERVER['REQUEST_TIME'],
			'wxname'     => $this->merchant_session['name'],
			'phone'     => $this->merchant_session['phone'],
			'payment_url'     => $this->config['site_url'].'/wap.php?c=Pay&a=check&type=weidian',
			'notify_url'     => $this->config['site_url'].'/wap.php?c=Weidian&a=api_msg',
			'login_url'     => $this->config['site_url'].'/wap.php?c=Weidian&a=redirect',
			'server_key' => ''
		);
		$sort_data = $data;
