<?php
    class ImageSelectAction extends  BaseAction{
            ...
            if(IS_POST){
                ...
                foreach($_POST['image_path'] as $v) {
                    ...
                    if (!is_file($upload_dir . $tmp_img_path[1])) {
                        ...
                        file_put_contents($upload_dir . $tmp_img_path[1],$img_content);

                        for ($i = 0, $len = count($thumbMaxWidth); $i < $len; $i++) {
                            if (!is_file($upload_dir . $thumb_prefix[$i] . $tmp_img_path[1])) {
                                Image::thumb($upload_dir . $tmp_img_path[1],$upload_dir . $thumb_prefix[$i] . $tmp_img_path[1], '', $thumbMaxWidth[$i], $thumbMaxHeight[$i], true);
                            }
                        }
        ...
        public function img_option(){
            if(IS_POST){
                if(!empty($_POST['text'])){
                    D('Image')->where('pigcms_id='.$_POST['id'])->setField('img_remark',$_POST['text']);
                }

                if($_POST['op']=='del'){
                    D('Image')->where('pigcms_id='.$_POST['id'])->setField('status',4);
                }
            }
        }
