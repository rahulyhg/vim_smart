...
class CountAction extends BaseAction{
	...
	public function bill()
	{
		...
		$percent = '';
		$type_name=array('meal'=>'快店','group'=>'团购','weidian'=>'微店','appoint'=>'预约','wxapp'=>'营销','store'=>'收银');
		$time = '';
		...
		$s=D($_GET['type'].'_order')->field('pay_time')->where($where)->limit(1)->find();
		...
		$result = D("Meal_order")->get_order_by_mer_id($mer_id,0,$type,$time);
		//dump(D());
	...
	public function storebill()
	{
		$mer_id = intval($this->merchant_session['mer_id']);
		$percent = '';
		$type_name=array('meal'=>'快店','group'=>'团购','weidian'=>'微店','appoint'=>'预约','store'=>'收银');
		$time = '';
		$where['pay_time']=array('neq',0);
		$where['paid']=1;
		$where['mer_id']=$_GET['mer_id'];
		...
		$result = D("Meal_order")->get_order_by_mer_id($mer_id,1,$type,$time,$store_id);
