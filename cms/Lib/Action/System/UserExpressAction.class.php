...
/**
 * o2o 快递管理模块
 * Class UserExpressAction
 * @update-time: 2017-06-12 10:05:07
 * @author: 王亚雄
 */
class UserExpressAction extends BaseAction {
    ...
    /**
     * 寄件管理
     * @update-time: 2017-05-25 15:04:00
     * @author: 王亚雄
     */
    public function shipping(){
        $model = M('express_order');
        //条件
        $map = array();
        $map['is_del'] = array('eq',0);
        $map['od.is_pay'] = array('eq',1);
        //若get到user_id 则使用user_id 作为过滤条件
        if($user_id = I('get.user_id',0,'intval')){
            $map['od.user_id'] = array('eq',$user_id);
        }
        //显示字段
        $field = array(
            'od.order_id',
        ...
        $list = $model->alias('od')
            ->field($field)
            ->join('LEFT JOIN __USER_ADRESS__ bad ON bad.adress_id = od.billing_adid')
            ->join('LEFT JOIN __USER_ADRESS__ sad ON sad.adress_id = od.shipping_adid')
            ->join('LEFT JOIN __USER__ u ON u.uid = od.user_id')
            ->where($map)
        ...
        $this->display('shipping');

    }

    /**
     * 寄件订单逻辑删除
     * @update-time: 2017-06-05 11:35:08
     * @author: 王亚雄
     */
    public function del_shipping(){
        $order_id = $_GET['order_id']?:0;
        if(!$order_id) $this->err("order_id不存在");
        $model = M('express_order','pigcms_');
        $re = $model->where('order_id=%d',$order_id)->setField('is_del',1);
        if($re!==false){
            $this->suc("删除成功");
        }else{
            $this->err("删除失败");
    ...
    /**
     * 寄件管理绑定快递单号 弹出层拉取内容页面
     * @update-time: 2017-06-05 11:35:13
     * @author: 王亚雄
     */
    public function modal_bind_emsorderid(){

        $order_id = intval($_GET['order_id']) ?: 0;
        if(!$order_id)  $this->error('order_id 不存在！');
        $field = array(
            'od.order_id',
            'od.goods_type_name',
            'od.pay_type_name',
    ...
    /**
     * 寄件管理绑定快递单号 执行绑定操作
     * @update-time: 2017-06-05 11:35:16
     * @author: 王亚雄
     */
    public function bind_emsorderid_act(){
        //pigcms_express_order 主键
        $order_id = $_GET['order_id']?:0;
        //快递单号
        $ems_order_id = $_GET['ems_order_id']?:0;
        //验证
        if(!$order_id) $this->err("order_id不存在");
        if(!$ems_order_id) $this->err("ems_order_id不存在");
        //数据库模型
        $data['ems_order_id'] = $ems_order_id;

        //快递信息 可为空
        //寄件人信息
        $billArr['name'] = $_GET['billing_name']?:null;
        $billArr['phone'] = $_GET['billing_phone']?:null;
        $billArr['position'] = $_GET['billing_position']?:null;
        $billArr['detail'] = $_GET['billing_detail']?:null;
        $billArr['uid'] = $_GET['uid']?:null;
        $billing_id = M('user_adress')->add($billArr);


        //取件人信息
        $shippArr['name'] = $_GET['shipping_name']?:null;
        $shippArr['phone'] = $_GET['shipping_phone']?:null;
        $shippArr['detail'] = $_GET['shipping_detail']?:null;
        if ($_GET['province_id'] && $_GET['province_id'] && $_GET['city_id']) {
            $shippArr['position'] = $_GET['province_id'].' '.$_GET['city_id'].' '.$_GET['area_id'];
        }
        $shipping_id = M('user_adress')->add($shippArr);

        if ($billing_id) {
            $data['billing_adid'] = $billing_id;
        }
        if ($shipping_id) {
            $data['shipping_adid'] = $shipping_id;
        }

        if ($_GET['goods_type_name']) {
            $data['goods_type_name'] = $_GET['goods_type_name'];
        }

        $model = M('express_order','pigcms_');
        //更新
        $re = $model->where(array('order_id'=>$order_id))->save($data);
        if($re!==false){
            echo json_encode(array('err'=>0,'msg'=>array('billing_adid'=>$billing_id,'shipping_adid'=>$shipping_id)));
        }else{
            $this->err("更新失败");
        }
    }
...
//        $single_jsonResult = $this->submitEOrder($jsonEorder);
    ...
    /**
     * 寄件管理查看详情弹出层
     * @update-time: 2017-06-05 11:35:19
     * @author: 王亚雄
     */
    public function modal_detail(){
        $order_id = $_GET['order_id']?:0;
        if(!$order_id) $this->err("快递单号获取失败！");
        ...
        $map['is_del'] = array('eq',0);
        $map['od.is_pay'] = array('eq',1);
        $map['od.order_id'] = array('eq',$order_id);
        //显示字段
        $field = array(
            'od.order_id',
            'od.goods_type_name',
        ...
        $this->display('modal_detail');
    }


    /**
     * 顺丰模板
     * @update-time: 2017-06-05 11:35:32
     * @author: 王亚雄
     */
    public function SF(){
    ...
    /**
     * 打印pdf
     * @update-time: 2017-06-05 11:35:42
     * @author: 王亚雄
     */
    public function SF_PDF(){
        $info = stripslashes(html_entity_decode($_COOKIE['order_id_' . $_GET['order_id']]));
        $info = json_decode($info,true);
        $this->assign('info',$info);
        $out = $this->fetch('SF_PDF');

        $mpdf = new mPDF('zh-CN', $format = 'A4-L', $default_font_size = 0, $default_font = '', $mgl = 15, $mgr = 15, $mgt = 16, $mgb = 16, $mgh = 9, $mgf = 9, $orientation = 'L');
    ...
    public function send_msg(){
        ...
        $field = array(
            ...
            'sad.position' => 'sad_position',
            'u.openid',
            'exp.name'=>'express_name',
            'exp.code'=>'express_code',
            'exp.coding'=>'express_coding',
        );
        //获取数据
        $info = $model->alias('od')
            ->field($field)
            ...
            ->join('LEFT JOIN __USER_ADRESS__ sad ON sad.adress_id = od.shipping_adid')
            ->join('LEFT JOIN __USER__ u ON u.uid = od.user_id')
            ->join('LEFT JOIN __EXPRESS__ exp ON exp.id = od.express_id')
            ->where($map)
            ->find();
        //工作人员信息
        $realname = $_SESSION['system']['realname'];
        $village_id = $_SESSION['system']['village_id'];
        $village_name = D('house_village')->where(array('village_id'=>$village_id))->find()['village_name'];

        $url = C('WEB_DOMAIN').'/wap.php?&g=Wap&c=PropertyService&a=getOrderTracesByJson&coding='.$info['express_coding'].'&order_id='.$info['ems_order_id'];

        $msg_u =array(
            'url'=>$url,
            'first_value'=>"您寄送的【".$info['ems_order_id']."】已被【".$village_name."】-【".$realname."】成功揽件",
            'keyword1_value'=>$info['express_name'],
            'keyword2_value'=>$info['order_on'],
            'keyword3_value'=>$info['ems_order_id'],
            'keyword4_value'=>time(),
        );
        $openid[] = $info['openid'];
    ...
    //快递寄件推送
    public function auto_send_message($admin_user,$msg_u){
        //制作本地推送内容
        foreach ($admin_user as $value){
            $href = $msg_u['url'];
            $data=array(
                'touser'=>$value,
                'template_id'=>"0Pmyv8hslqhpKpIi47duFxPZIDf8UMPF0QzcOLaQD9w",
                'url'=>$href,
                'data'=>array(
                    'first'=>array(
                        'value'=>urlencode($msg_u['first_value']),
                        'color'=>"#029700",
                    ),
                    'keyword1'=>array(
                        'value'=>urlencode($msg_u['keyword1_value']),
                        'color'=>"#000000",
                    ),
                    'keyword2'=>array(
                        'value'=>urlencode($msg_u['keyword2_value']),
                        'color'=>"#000000",
                    ),
                    'keyword3'=>array(
                        'value'=>urlencode($msg_u['keyword3_value']),
                        'color'=>"#000000",
                    ),
                    'keyword4'=>array(
                        'value'=>urlencode($msg_u['keyword4_value']),
                        'color'=>"#000000",
                    ),
                    'remark'=>array(
                        'value'=>'感谢您使用!',
    ...
    public function update(){
        ...
        $detail = I('get.detail','','htmlspecialchars');
        $data = array(
            'phone'=>$phone,
            'name'=>$name,
            'detail'=>$detail
        ...
        //更新
        $re = $model->where('adress_id=%d',$adid)->save($data);
        if($re!==false){
            $this->suc("成功");
        }else{
            $this->err("编辑失败");
    ...
    /**
     * 收件箱
     * @update-time: 2017-05-25 15:04:03
     * @author: 王亚雄
     */
    ...
    public function user(){
        ...
        $model = M('express_order','pigcms_');
        //寄件列表用户视图
        $express_user_view = $model
            ->field(array(
                'user_id',
                'max(create_time)'=>'last_billing_time'
            ))
            ->group('user_id')
            ->select(false);
        ...
        $field = array(
            ...
            'u.nickname',
            'u.phone',
            'eu.last_billing_time'
        );
        //条件
        $map = array();
        //总条数
        $count = M('user','pigcms_')->alias('u')
            ->join("right join $express_user_view as eu on eu.user_id=u.uid")
            ->where($map)
            ->count();
        //分页
        $page = new Page($count,10);
        //当页数据
        $list = M('user','pigcms_')->alias('u')
            ->field($field)
            ->join("right join $express_user_view as eu on eu.user_id=u.uid")
            ->where($map)
            ->limit($page->firstRow,$page->listRows)
            ->order('eu.user_id desc')
            ->select();
        //dump($list); exit();
        //dump(mysql_error());exit();
        $this->assign('list',$list);
        $this->assign('pagebar',$page->show());
        $this->display('user');
     ...
     *  通过快递单号获取物流轨迹信息
     */
    public function modal_express_trail(){

        $order_id = I('get.order_id');
        //获取快递单号
        $LogisticCode =M('express_order','pigcms_')->where('order_id=%d',$order_id)->getField('ems_order_id');

        if(!$LogisticCode){
            $this->assign('error',"订单号还未绑定！");
        }

        $model = new UserExpressModel();
        //获取物流公司信息
        $express_company_info = $model->get_express_company_info($LogisticCode);
        //物流公司编码
        $ShipperCode = $express_company_info['Shippers'][0]['ShipperCode'];
        //获取物流轨迹信息
        $trail_info = $model->get_trail_info($LogisticCode,$ShipperCode);
        //发生错误1
        if(!$trail_info['Success']){
            $this->assign('error',$trail_info['Reason']);
        }
        //发生错误2
        if($trail_info['State']!=2&&$trail_info['State']!=3){
            $this->assign('error',$trail_info['Reason']);
        }

        $this->assign('info',array_reverse($trail_info['Traces']));

        $this->display('modal_express_trail');
    ...
    public function test(){
        $model = new UserExpressModel();
        $a = $model->get_express_company_info('506727623700');
        $ShipperCode = $a['Shippers'][0]['ShipperCode'];
        dump($ShipperCode);
        $b = $model->get_trail_info(506727623700,$ShipperCode);
        dump($b);
    }
