...

{
    ...
    public function index_news()
    {
	 ...
	 public function other(){
        $count = D("Sms_record")->where("mer_id=0")->count();
        import('@.ORG.system_page');
        $p = new Page($count, 20);
        $mod = new Model();
        $sql = "SELECT * FROM ". C('DB_PREFIX') . "sms_record where mer_id=0 ORDER BY pigcms_id DESC LIMIT {$p->firstRow}, {$p->listRows}";
        $res = $mod->query($sql);
