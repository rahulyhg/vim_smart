...
//http://www.hdhsmart.com/upload/images/classify/20170707/classify_20170707164956.jpg
   //屏掉
    public $collectUrl = 'http://xxxx-server.meihua.com';
    public $img_dir = 'upload/classify/';
    ...
    public function index_news() {
        ...
        $category_list = $database_Classify_category->field(true)->where($condition)->order('`cat_sort` DESC,`cid` ASC')->limit(100)->select();
        ...
        if (!empty($now_category['cat_field'])) {
            ...
            foreach ($now_category['cat_field'] as $key => $value) {
                ...
                if ($value['use_field'] == 'area') {
                    ...
                    $now_category['cat_field'][$key]['name'] = '区域';
...
//                    $mc = 0;
    ...
    public function checkList_news() {
        ...
        $tmpdatas = $userinputDb->field('id,uid,cid,fcid,sub3dir,title,lxname,lxtel,addtime,updatetime')->where($wherestr)->order('updatetime desc')->select();
    ...
    /**
     * 编辑弹出层
     * @update-time: 2017-07-06 09:40:53
     * @author: 王亚雄
     */
    public function infoedit() {
        ...
        //创建html
        $iptm = new ClassifyInputModel($tmpdata['cid']);
        foreach($content as &$c){
            $c['html'] = $iptm->get_input_by_url($c['url'],$c['vv']);
        }
        unset($c);
        $this->assign('content', $content);
        $this->display();
    ...
    /**
     * 编辑执行操作
     * @update-time: 2017-07-06 09:40:57
     * @author: 王亚雄
     */
    public function infoedit_act(){
        //dump($_POST);exit();
        //模型
        $model = M('classify_userinput');
        $id = I('post.vid');
        //老的数据
        $info = $model->where('id=%d',$id)->find();
        //老的content数据
        $content_arr = unserialize($info['content']);
        //新的数据，未组装
        $new_info = $_POST;
        //临时容器
        $tmp = array();
        //content字段数据组装
        foreach($new_info['k'] as $key => $tn){

            //属性名称
            $tmp[$key]['tn'] = htmlspecialchars($tn);
            //属性值
            if( $content_arr[$key]['type']==1
                && isset($content_arr[$key]['unit'])
                && !empty($content_arr[$key]['unit'])
            ){//值带单位的时候
                $tmp[$key]['vv'] = trim(htmlspecialchars(explode('/',$new_info['v'][$key])[0]));
            }else{
                $tmp[$key]['vv'] = trim(htmlspecialchars($new_info['v'][$key]));
            }


        }
        //新的content数据（array）
        $new_content_arr =array_map("array_merge",$content_arr,$tmp);

        //新的数据
        $new_info = array(
             "title"  =>  I('post.title'),
             "lxname"  => I('post.lxname'),
             "lxtel"  => I('post.lxtel'),
             "description"  => I('post.description'),
             "content"=>serialize($new_content_arr),
             "imgs"=>serialize(I('post.bd_imgs'))
        );

        //更新数据
        $res = $model->where('id=%d',$id)->save($new_info);
        if($res!==false){
            $this->success("编辑成功");
        }else{
            $this->error("编辑失败 ".mysql_error());
        }
