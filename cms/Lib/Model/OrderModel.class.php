...
class OrderModel extends Model
{
	...
	public function get_order_by_mer_id($mer_id, $type = 'meal', $is_system = false,$time,$is_pay_bill)
	{
		...
		switch ($type) {
			...
			case 'appoint':
				...
				$p = new Page($count, 20);
				$sql = "SELECT o.order_id, o.appoint_type as order_name, o.uid,o.mer_id, o.store_id, 1 as total, (pay_money+balance_pay) as price, o.payment_money as order_price, order_time as dateline, paid, pay_type, pay_time, third_id, balance_pay, pay_money as payment_money, merchant_balance, is_pay_bill FROM ". C('DB_PREFIX') . "appoint_order o LEFT JOIN ". C('DB_PREFIX') ."appoint a ON o.appoint_id = a.appoint_id  WHERE a.payment_status=1 AND  o.mer_id={$mer_id} AND paid=1 AND is_own=0 AND pay_type<>'offline' AND service_status=1".$time_condition;
				$sql .= " ORDER BY order_id DESC LIMIT {$p->firstRow}, {$p->listRows}";
				$order_list = $mode->query($sql);
			...
			case 'store':
				$db = D('Store_order');
				$count = $db->where("mer_id={$mer_id} AND paid=1 AND is_own=0 AND refund=0")->count();
				$p = new Page($count, 20);
	...
	public function export_order_by_mid($mer_id, $type = 'meal', $is_pay_bill = 0)
	{
		...
		switch ($type) {
			...
			case 'appoint':
				$db = D('Appoint_order');
				$sql = "SELECT order_id, order_id as orderid, appoint_id as order_name, uid, mer_id, store_id, 1 as total, (pay_money+balance_pay) as price, payment_money as order_price, order_time as add_time, paid, pay_type, pay_time, third_id, balance_pay, pay_money as payment_money, merchant_balance, is_pay_bill FROM ". C('DB_PREFIX') . "appoint_order WHERE is_pay_bill=0 AND mer_id={$mer_id} AND paid=1 AND is_own=0 AND pay_time<'{$time}' AND pay_type<>'offline' AND service_status=1";
				$sql .= " ORDER BY order_id DESC ";
				$order_list = $mode->query($sql);
				break;
			case 'store':
				$db = D('Store_order');
				$sql = "SELECT order_id, orderid, name as order_name, 0 as uid, mer_id, store_id, 1 as total, price, total_price as order_price, dateline, paid, pay_type, pay_time, third_id, balance_pay, payment_money, merchant_balance, is_pay_bill FROM ". C('DB_PREFIX') . "store_order WHERE is_pay_bill=0 AND mer_id={$mer_id} AND paid=1 AND is_own=0 AND refund=0";
				$sql .= " ORDER BY order_id DESC";
				$order_list = $mode->query($sql);
				break;
		}
