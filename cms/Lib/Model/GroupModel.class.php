...
// 				$group_list[$key]['wx_cheap'] = floatval($value['wx_cheap']);
	...
	public function get_single_store_group_list($store_id,$limit=6,$is_wap=false){
		...
		$group_list = D('')->field('`g`.`group_id`,`g`.`cat_id`,`g`.`cat_fid`,`g`.`name` AS `group_name`,`g`.`price`,`g`.`pic`,`g`.`old_price`,`g`.`wx_cheap`,`g`.`sale_count`,`g`.`end_time`,`g`.`begin_time`')->table(array(C('DB_PREFIX').'group'=>'g',C('DB_PREFIX').'group_store'=>'gc'))->where("`gc`.`city_id`='".C('config.now_city')."' AND `g`.`group_id`=`gc`.`group_id` AND `gc`.`store_id`='$store_id' AND `g`.`status`='1' AND `g`.`type`='1' AND `g`.`end_time`>'$now_time'")->order('`g`.`sort` DESC,`g`.`group_id` DESC')->limit($limit)->select();// AND `g`.`begin_time`<'$now_time'
	...
	public function wap_get_group_list_by_catid($get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
		...
		$condition_where = "`m`.`city_id`='".C('config.now_city')."' AND `g`.`mer_id`=`m`.`mer_id` AND `g`.`status`='1' AND `g`.`type`='1' AND `m`.`status`='1' AND `g`.`end_time`>'$now_time'";// AND `g`.`begin_time`<'$now_time'
...
// 				$order = 'juli asc,`g`.`group_id` DESC';
	...
	public function wap_get_storeList_by_catid($get_grouplist_catid,$get_grouplist_catfid,$area_id,$order, $lat = 0, $long = 0, $circle_id = 0){
		...
		$condition_where = "`gc`.`city_id`='".C('config.now_city')."' AND `g`.`status`='1' AND `g`.`type`='1' AND `s`.`status`='1' AND `g`.`end_time`>'$now_time' AND `gc`.`group_id`=`g`.`group_id` AND `gc`.`store_id`=`s`.`store_id`";// AND `g`.`begin_time`<'$now_time'
	...
	public function get_qrcode($id){
		...
		$now_group = $this->field('`group_id`,`qrcode_id`,`mer_id`')->where($condition_group)->find();
		if(empty($now_group)){
