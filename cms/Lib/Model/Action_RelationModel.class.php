 ...
 */
    class Action_RelationModel extends Model{
        ...
        public function add_user_action($order_id,$order_type){
            ...
            if($order_type=='meal'){
                $sId    =   M($order_type.'_order')->where(array('order_id'=>$order_id))->field(array('uid','store_id'))->find();
                ...
                if($storeId){
                    ...
                    $order_type =   'meal_store';
                }else{
                    return  array('error' => 5,'msg'=>'商铺没有分类');
                }
            }else{
                $sId    =   M($order_type.'_order')->where(array('order_id'=>$order_id))->field(array($order_type.'_id','uid','store_id'))->find();
                if($sId[$order_type.'_id']){
                    $sCatId  =   M($order_type)->where(array($order_type.'_id'=>$sId[$order_type.'_id']))->field('cat_id')->find();
                }else{
                    return  array('error' => 4,'msg'=>'订单号不存在');
                }
            }
            if($sCatId['cat_id']){
                $aCate  =   M($order_type.'_category')->where($sCatId)->field(array('cat_fid','cat_name','cat_url','cat_id'))->find();
                ...
                if($aCate['cat_fid']){
                    $aFCate  =   M($order_type.'_category')->where(array('cat_id'=>$aCate['cat_fid']))->field(array('cat_name','cat_url','cat_id'))->find();
        ...
        public  function    add_category($cate_name){
            ...
            if($sName['action_id']){
                return  array('error' => 2,'msg'=>'已经存在','id'=>$sName['rela_id']);
            }else{
        ...
        public  function    is_relation($cate_name){
            foreach($cate_name as $k=>$v){
                $sName  =   M('action_relation')->where(array('cat_type'=>$this->order_type,'cat_id'=>$v['cat_id']))->field('*')->select();
                if($sName[0]['rela_id']){
                    $aName[$k]  =   $sName;
                }else{
                    $aName[$k]  =   null;
        ...
        public  function    is_one_relation($cate_name){
            $sName  =   M('action_relation')->where(array('cat_type'=>$this->order_type,'cat_id'=>$cate_name['cat_id']))->field('*')->select();
            if(empty($sName)){
                ...
                if($add_category['error']!=1 && $add_category['error']!=3){
                    ...
                    $sAdd   =   M('action_relation')->add($aName);
                    $asName[0]   =   $aName;
