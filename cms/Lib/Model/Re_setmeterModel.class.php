...
class Re_setmeterModel extends Model{
    ...
    public function get_meter_record($meter_hash,$year){
        ...
        for ($i=1;$i<=12;$i++){
            ...
            $degree=($end-$start)*$meter_info['rate'];
     ...
     * 批量获取抄表信息
     */
    public function get_meter_record_list($village_id,$year,$search=''){
     ...
     * 批量获取抄表信息
     */
    public function get_meter_record_list1($village_id,$year,$search=''){
        if(empty($village_id)) return '请选择项目';
        if(empty($year)) $year=date('Y');
        if(empty($month)) $year=date('m');
        $where=array();
        if(!empty($search)) $where=$search;
        $where['is_del']=0;
        $where['village_id']=$village_id;
