...
class Yeelink{
     ...
     * @param $status
     */
    public function __construct($apikey="",$deviceid="",$url=""){
        $this->apikey = $apikey;
    ...
    public function saveSensor($ac_control,$sensorid='',$ac_type){
        ...
        if($sensorid!=""){
            $method = "PUT";
            $url = $this->url . $this->deviceid . '/sensor/' . $sensorid ;
            $json = '{
            ...
            $method = "POST";
            $url = $this->url.$this->deviceid.'/sensors/';
            $json = '{
    ...
    public function yeelink($sensorid='',$status=''){
        //创建传感器
        $url = $this->url . $this->deviceid . '/sensor/' . $sensorid . '/datapoints/';
        $curl = curl_init();
        //0关闭,1开启
        $json = '{
                "value":' .$status. '
    ...
    public function getStatus($sensorid=''){
        //创建传感器
        $url = $this->url . $this->deviceid . '/sensor/' . $sensorid . '/datapoint/';
        $curl = curl_init();
        //0关闭,1开启
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_HEADER, 0);
