...
// 	}
	...
	public static function print_format($order_id, $table = 'meal_order')
	{
		$str_format = array();
		$order = D(ucfirst($table))->field(true)->where(array('order_id' => $order_id))->find();
		if (is_array($order)) {
					...
					if ($meal_ids) {
						...
						foreach ($list as $l) {
							if (isset($str_format[$mid_pid[$l['id']]])) {
								$str_format[$mid_pid[$l['id']]] .= chr(10) . $l['name'] . ": ￥" . $l['price'] . " * " . $l['num'];
				...
				return $format_str;
                            ...
                            if($order['cue_field']){
                                ...
                                foreach($cue_field as $v){
                                    $format_str .= chr(10) . '客户地址：'.$v['address']. chr(32) .$v['value'];
                                }
                            }
