...
// 		$alipay_config['transport']    = 'http';
		...
		if($verify_result) {//验证成功
			...
			if( ! empty($doc->getElementsByTagName( "notify" )->item(0)->nodeValue) ) {
				...
				$order_param['pay_money'] = $order_id_arr[2];
...
// 			}
	...
	}		/* 商户后台充值	* @time 2016-04-13	* @author	小邓  <969101097@qq.com>*/	public function recharge_pay($out_trade_no){		import("@.ORG.pay.Alipay.alipay_submit");		$alipay_config['partner']		= $this->pay_config['pay_alipay_pid'];		$alipay_config['seller_email']	= $this->pay_config['pay_alipay_name'];		$alipay_config['key']			= $this->pay_config['pay_alipay_key'];		$alipay_config['sign_type']    = 'MD5';		$alipay_config['input_charset']= 'utf-8';		$alipay_config['transport']    = 'http';		$parameter = array(			"service" => "create_direct_pay_by_user",			"partner" => $this->pay_config['pay_alipay_pid'],			"seller_email" => $this->pay_config['pay_alipay_name'],			"payment_type"	=> '1',			"notify_url"	=> C('config.site_url').'/merchant.php?g=Merchant&c=Recharge&a=alipayNotify',	//异步通知			"return_url"	=> C('config.site_url').'/merchant.php?g=Merchant&c=Recharge&a=alipayReturn',	//同步通知			"out_trade_no"	=> $out_trade_no,			"subject"	=> '商户后台充值',			"total_fee"	=> $this->pay_money,			//"total_fee"	=> '0.01',			"body"	=> '',			"show_url"	=> C('config.site_url'),			"anti_phishing_key"	=> '',			"exter_invoke_ip"	=> '',			"_input_charset"	=> 'utf-8'		);		$alipaySubmit = new AlipaySubmit($alipay_config);		$form = $alipaySubmit->buildRequestForm($parameter,"get", "确认");		return $form;	}
}
?>