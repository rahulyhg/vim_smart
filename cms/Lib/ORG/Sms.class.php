...
// 		if (preg_match("/^13[0-9]{1}[0-9]{8}$|15[01236789]{1}[0-9]{8}$|18[01236789]{1}[0-9]{8}$/", $mobilephone)) {
	...
	/*public function sendSms($data = array(), $send_time = '', $charset = 'utf-8', $id_code = '')
	{
		if ($data) {
			...
			$sms_uid = C('config.sms_uid');
			$sms_pwd = C('config.sms_pwd');
			if($sms_pwd = C('config.sms_pwd')){
				$sms_pwd = md5($sms_pwd);
			}
			$smsapi_senturl = str_replace("{uid}",$sms_uid,$smsapi_senturl);
			$smsapi_senturl = str_replace("{pwd}",$sms_pwd,$smsapi_senturl);
			$smsapi_senturl = str_replace("{moblie}", $mobile,$smsapi_senturl);
			$smsapi_senturl = str_replace("{content}",urlencode($content.$site_name),$smsapi_senturl);
			
			
			$return = file_get_contents($smsapi_senturl);
			$sms_status = 0;
