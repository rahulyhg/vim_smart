...
class TagLibPigcms extends TagLib {
	...
	public function _adver($attr,$content) {
        ...
        $parseStr .= 'if(count('.$name.')==0) : echo "<div class=\"'.$tag['cat_key'].'_empty\">列表为空</div>" ;';
        $parseStr .= 'else: ';
        $parseStr .= 'foreach('.$name.' as $key=>$vo): ';
		$parseStr .= '++$'.$key.';?>';
        $parseStr .= $this->tpl->parse($content);
        $parseStr .= '<?php endforeach; endif; else: echo "<div class=\"'.$tag['cat_key'].'_empty\">列表为空</div>" ;endif; ?>';
        $_iterateParseCache[$cacheIterateId] = $parseStr;
        if(!empty($parseStr)) {
            return $parseStr;
        }
